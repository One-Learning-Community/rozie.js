...
On its home framework each of these is a solid pick. The case for Rozie is consistency, coverage, and the deliberate non-singleton shape: no toast component spans every framework with the same API; each ecosystem reimplements the queue, the auto-dismiss timers, hover-pause, and positioning from scratch, and Lit / web components have nothing mainstream at all. `@rozie-ui/toast` gives them all the same `<Toaster>`.
...
- [Toaster — live demo](/components/toast-demo) — the real Vue package running in the page (info / success / error buttons driving the `ref` handle), plus the one `.rozie` source and the six generated outputs.
