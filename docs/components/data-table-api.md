...
| `reorder-change` | Fired when the column order changes. Column reorder is **programmatic only** — an `applyColumnOrder` call or a two-way `columnOrder` write; unlike visibility, resize and pin, no built-in drag or menu affordance ships. Payload: the fresh `ColumnOrderState`. |
...
| `undo` | **Requires `undoable`.** Restore the `data` snapshot from before the most recent mutation (cell/row edit, paste, fill, cut): `undo()`. Replays through the standard write path, so it writes `r-model:data` — firing the `data` slice's per-target model-update event (Lit's `data-change`, React's `onDataChange`, etc.; see [Models](#models-the-twelve-two-way-slices)) — and fires `history-change`. The stack keeps up to `undoLimit` snapshots (oldest evicted). No-op when there is nothing to undo. |
...
`cell`, `colHeader`, `filter` and `editor` each also have a **per-column slot family** — a statically-named fill per `columnId` (`cell-price`, `filter-category`, …) that takes precedence over the matching generic slot in the table below. See [Per-column slot families](/components/data-table-columns#per-column-slot-families) for the full precedence/gate rules and [Per-framework usage code](/components/data-table-usage#per-column-slot-families-cell-id-colheader-id-filter-id-editor-id) for the exact per-target syntax (React/Solid `slots`, Svelte `snippets`, Lit `.rozieSlots`, Vue/Angular native `#cell-<id>` / `[templates]`).

| Slot | Params | Description |
| --- | --- | --- |
| `cell` | `columnId, column, row, value` | Custom cell renderer — switch on `columnId` to vary per column. A column it does not render shows the plain accessor value. **On a group-header row `row` is a [group descriptor](/components/data-table-grouping#the-row-slot-param-on-group-header-rows), not a data record** — guard with `row.isGroupRow` before reading record fields (0.3.2). |
| `cell-<columnId>` | `columnId, column, row, value` | **Family fill** — a static per-column override of `cell` (e.g. `#cell-price`). No gate. See [Per-column slot families](/components/data-table-columns#per-column-slot-families). |
| `colHeader` | `columnId, column, label` | Custom header renderer — switch on `columnId`. Falls back to the plain `header` label. |
| `colHeader-<columnId>` | `columnId, column, label` | **Family fill** — a static per-column override of `colHeader`. No gate. See [Per-column slot families](/components/data-table-columns#per-column-slot-families). |
...
| `selectCell` | `row, checked, toggle` | Override the per-row select checkbox (only when `selectionMode="multiple"`). **On a group-header row `row` is a [group descriptor](/components/data-table-grouping#the-row-slot-param-on-group-header-rows), not a data record** (0.3.2). |
...
| `groupBar` | `grouping, groupableColumns, applyGrouping, clearGrouping` | **Headless** group-bar (only when `groupable`). Receives the ordered `grouping` array, the `groupableColumns` (`[{ id, label }]`), and the `applyGrouping`/`clearGrouping` helpers so a consumer builds any bar/drag UI. `groupableColumns` lists every groupable **leaf** column in declaration order — including leaves nested under a multi-level group header, and excluding the group headers themselves, which carry no accessor and cannot be a grouping key (0.3.2). Opt a leaf out with `groupable: false`. The default render is a non-interactive styled-token reflection (empty when ungrouped); the component ships **no** drag affordance. The React render-prop edge (documented divergence). |
| `filter` | `columnId, value, uniqueValues, minMax, setFilter` | **Headless** faceted-filter UI (only when a column is `filterable`). `value` is the column's current filter value; `uniqueValues` is the cross-filtered distinct values (`unknown[]`, keys only); `minMax` is the cross-filtered numeric `[min, max]` (or `null`); `setFilter(columnId, value)` applies a column filter through the same funnel the built-in input uses (`value` of `null`/`''` clears it). Build a checkbox list / range slider and call `setFilter` (or drive `columnFilters` directly) — the slot fires no event. See [Faceted filtering](/components/data-table-faceted-filtering). The React render-prop edge (documented divergence). Pair it with the [drop-in filter components](/components/data-table-faceted-filtering#drop-in-filter-components). |
| `filter-<columnId>` | `columnId, value, uniqueValues, minMax, setFilter` | **Family fill** — a static per-column override of `filter`. **Requires the column to declare `filterable`**, the same gate as `filter` itself. See [Per-column slot families](/components/data-table-columns#per-column-slot-families). |
| `editor` | `columnId, column, row, value, commit, cancel, autofocus` | Custom cell editor (a column with `editor="custom"`, or to override the built-in editor). `value` is the current draft; `commit(newValue)` validates + commits (fires `cell-edit-commit`); `cancel()` closes without saving; `column` is an opaque passthrough; `autofocus` tells the editor whether the host wants it to take initial focus (drive your input's `autofocus`/`focus()` from it). `row` is the data record — a group-header row can never enter edit, so this slot never renders on one (0.3.2). Pair it with the [drop-in editor components](/components/data-table-editing#drop-in-editor-components). The React render-prop edge (documented divergence). |
| `editor-<columnId>` | `columnId, column, row, value, commit, cancel, autofocus` | **Family fill** — a static per-column override of `editor`. **Requires the column to declare `editor="custom"`**, the same gate as `editor` itself. See [Per-column slot families](/components/data-table-columns#per-column-slot-families). |
