...
| `filter-<columnId>` | `columnId, uniqueValues, minMax, setFilter` | **Family fill** — a static per-column override of `filter`. **Requires the column to declare `filterable`**, the same gate as `filter` itself. See [Per-column slot families](/components/data-table-columns#per-column-slot-families). |
| `editor` | `columnId, column, row, value, commit, cancel` | Custom cell editor (a column with `editor="custom"`, or to override the built-in editor). `value` is the current draft; `commit(newValue)` validates + commits (fires `cell-edit-commit`); `cancel()` closes without saving; `column` is an opaque passthrough. `row` is the data record — a group-header row can never enter edit, so this slot never renders on one (0.3.2). Pair it with the [drop-in editor components](/components/data-table-editing#drop-in-editor-components). The React render-prop edge (documented divergence). |
| `editor-<columnId>` | `columnId, column, row, value, commit, cancel` | **Family fill** — a static per-column override of `editor`. **Requires the column to declare `editor="custom"`**, the same gate as `editor` itself. See [Per-column slot families](/components/data-table-columns#per-column-slot-families). |
