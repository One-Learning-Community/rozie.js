...
- **Grouped virtualization.** Native option groups shipped in `0.2.0` — ordered sections with `role="group"` headings via the `groups` prop + the `groupHeading` slot — and `0.3.0` added an optional per-group result `groupCap` with an expand-in-place "+N more" row. Combining them with the `:virtual` windowed path (sticky group headers across the window) is not built in yet.
- **`@rozie-ui/combobox` is `0.3.0`.** The surface (17 props / 2 events / a 4-verb handle / `option` · `empty` · `groupHeading` · `groupMore` slots / client + async filtering) is stable and gate-verified across all six targets, but it is younger and less battle-tested than the established per-framework libraries.
