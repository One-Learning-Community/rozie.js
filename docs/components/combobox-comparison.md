...
- **Grouped virtualization.** Native option groups shipped in `0.2.0` — ordered sections with `role="group"` headings via the `groups` prop + the `groupHeading` slot, plus an optional per-group result `groupCap` with an expand-in-place "+N more" row. Combining them with the `:virtual` windowed path (sticky group headers across the window) is not built in yet.
