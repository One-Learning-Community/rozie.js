...
| Imperative `EditorView` handle | ✅ (ref) | ✅ (ref) | ~ (directive) | ~ | ~³ | hand-roll | ✅ 12-verb `$expose` |
...
³ `solid-codemirror` surfaces the `EditorView` via its `onEditorMount` / ref primitive rather than a method handle; `svelte-codemirror-editor` exposes the view less directly. Rozie gives every target the *same* 12-verb handle (`getView` / `focus` / `getValue` / `replaceValue` / `dispatch` / `insertText` / `getSelection` / `setSelection` / `scrollToPos` / `selectAll` / `undo` / `redo`).
...
- **A uniform 12-verb imperative handle** (`$expose`) with the same shape on every target — versus "hold the `EditorView` you happened to construct," which differs per framework (a React ref, a Vue ref, a directive input, a Solid mount callback). `getView()` is always the raw-engine escape hatch.
...
- **`@rozie-ui/codemirror` is `0.1.0`.** The surface (10 props / 0 events / 12-verb `$expose` handle / five injection portal slots) is stable and gate-verified, but it is younger than the multi-year community wrappers.
