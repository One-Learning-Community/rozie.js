...
| Option | Framework(s) | Escapes ancestors via | Focus trap | Scrim source | Two-way `open` | Same API on all 6 fw |
| --- | --- | --- | :---: | --- | :---: | :---: |
...
- **First-class packages for all six frameworks**, including Lit / web components, which have *no* mainstream modal component at all, and Solid, where modals are mostly hand-rolled. Both are categories the incumbents barely serve.
- **The same component surface everywhere.** Where the ecosystem offers a different library per framework (many APIs, many scrims, many focus-trap implementations), `@rozie-ui/dialog` is one `<Dialog>` with the same `open` model, the same `close` event with its `reason`, and the same `show` / `hide` handle on all six: one component to learn, document, and migrate across your stack.
