...
- **Inline editing, cell + full-row.** Mark a `<Column editable>` and bind `r-model:data`: the component owns the edit state and writes a fresh `data` array back on commit. Five built-in editor types (`text` / `number` / `select` / `checkbox` / `date`), synchronous per-column validation announced via aria-live, full-row edit (`Shift+F2`), and a headless `#editor` slot (plus an opt-in per-column `editor-<columnId>` slot family) with opt-in drop-in editor components, where TanStack ships only the row-model state and leaves the editors to you.
