...
| Cell range selection (grid mode) | ❌ | ❌ | ❌ | ❌ | ✅ (AG) | ❌ | ✅ `Shift+Arrow` / `Shift+Click` |
...
- **`@rozie-ui/data-table` is `0.1.0`.** The surface (23 props / 12 two-way slices / 14 events / 27-verb handle / declarative `<Column>` + `columnId`-dispatched cell/header slots / selection / detail / group-bar / filter / editor slots) is stable and gate-verified across all six targets, but it is younger and less battle-tested than the established libraries.
