...
| **`@rozie-ui/data-table`** | `@rozie-ui/data-table-*` | a **component** | ✅ | ❌ **no adapter** | One source → all six, same props / nine two-way slices / events / `<Column>` API / slots / handle. Built on `@tanstack/table-core` directly, wired by hand, with no `@tanstack/<fw>-table` adapter in any leaf. |
...
The [`@rozie-ui/data-table` showcase + API reference](/components/data-table) documents the `@rozie-ui/data-table-*` packages — one pre-compiled, per-framework install (`npm i @rozie-ui/data-table-react`, etc.). The state engine is `@tanstack/table-core` (a peer dependency you control); there is **no required CSS** — a fully-tokenised skin ships inside the component, with optional one-line theme bridges for shadcn/ui, Material 3, and Bootstrap 5. The [live demo](/components/data-table-demo) runs the real Vue package in the page.

## Cross-references

- [DataTable — showcase & API](/components/data-table) — the full `@rozie-ui/data-table` surface, quick start, `<Column>` API, theming, and accessibility reference.
