...
- **Groups are data-derived, not composable.** As of `0.3.0` commands sharing a `group` field render as labeled sections automatically (headings overridable via the `groupHeading` slot, cappable per-section via `groupCap` with an expand-in-place "+N more" row) — but there is no cmdk-`Command.Group`-style composable section API, no sticky headings, and `groupCap` does not yet compose with per-row `actions` (the action-menu anchor assumes uncapped section order).
