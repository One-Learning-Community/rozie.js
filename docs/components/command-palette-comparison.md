...
- **Scoped slots.** Twelve of them, ordinary scoped slots on every target: `option` (custom row render, scoped with <span v-pre>`{ option, index, active, selected, disabled }`</span> — the listbox vocabulary shared with `@rozie-ui/listbox`, which the palette composes internally) plus its `icon` / `trailing` / `actions` row sub-slots, `groupHeading` (section headings), `actionItem` (rows of the per-row action menu), `argsField` (rows of the inline command-args form), the `empty` / `loading` / `error` state slots, `breadcrumb` (the nested-level header), and `footer` (a persistent footer bar). See the [API reference](/components/command-palette-api) for scopes.
...
- **Inline command arguments.** A command declares `args: [{ id, placeholder?, required?, default? }]` and selecting it auto-enters a panel-internal args form — no separate "prompt" component, no manual state machine. `select`'s payload additively gains `args: { [id]: value }` (trimmed), argless commands are unaffected. cmdk/kbar have no built-in equivalent; you would hand-roll the sub-flow yourself.
...
- **`@rozie-ui/command-palette` is `0.4.0`.** The surface (15 props / 4 events / a 6-verb handle / 12 slots / two-way `open` + `query`) is stable and gate-verified across all six targets, but it is far younger and less battle-tested than cmdk.
