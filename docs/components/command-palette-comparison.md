...
So a design system that ships React, Vue, Svelte, and Angular wrappers today maintains three or four different command-menu integrations (different props, different filtering models, different accessibility quality) that must somehow stay behaviorally in lockstep. For that team, `@rozie-ui/command-palette` replaces them all: one WAI-ARIA command-menu behavior to maintain, and the same `<CommandPalette>` everywhere.

## What `@rozie-ui/command-palette` gives you

- **Six packages, one API.** Install `@rozie-ui/command-palette-{react,vue,svelte,angular,solid,lit}` for your framework; there is no Rozie toolchain or build step for the consumer, and the surface is identical.
...
The case for Rozie is consistency and coverage, not feature maximalism: no other accessible command palette spans every framework with the same API, and Lit / web components and Solid in particular have essentially no headless incumbent at all.
...
- **It composes `@rozie-ui/combobox`, not the listbox family.** The results list is the vendored `Combobox` primitive in `inline` mode (scoped slots + roving nav come from there), not the published `@rozie-ui/listbox` package — the accessibility primitives are the same either way, it's just a different leaf underneath. This IS real cross-family composition of published leaves: every command-palette package declares `@rozie-ui/combobox-<target>` as a required peer and imports it at build time.
- **`@rozie-ui/command-palette` is pre-1.0.** The surface is stable and gate-verified, but it is far younger and less battle-tested than cmdk. The full prop, event, handle, and slot tables live in the [API reference](/components/command-palette-api).
