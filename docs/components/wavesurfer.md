...
### `dragToCreateRegions` needs `disableDragToSeek: true`

wavesurfer.js's core drag-to-seek and the Regions plugin's drag-to-create both listen for the SAME pointer-drag gesture on the same internal wrapper element. Drag-to-seek registers first (at construction, since it defaults on), and — because both trackers use the identical threshold-tracking pattern and each bails out once the pointer event's `defaultPrevented` flag is already set — drag-to-seek wins the race and silently swallows the gesture before the Regions plugin's own tracker ever sees it. In practice: with `dragToCreateRegions: true` alone, dragging over empty waveform space just seeks (fires `interaction`/`seeking`) and never creates a region. Pair `dragToCreateRegions` with **`disableDragToSeek: true`** so drag gestures are unambiguously routed to region creation instead. This is wavesurfer.js's own behavior, not something Rozie can route around from the author side.
