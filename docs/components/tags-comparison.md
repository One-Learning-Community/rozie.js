...
| Framework | Representative option(s) | Paste-to-bulk-add | Two-way value | Per-token validation | Same API, 6 fw |
| --- | --- | :---: | :---: | :---: | :---: |
...
| Same API on all 6 frameworks | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ | ⚠️ 3 fw via wrappers | ✅ |
...
- **`@rozie-ui/tags` is pre-1.0** and younger and less battle-tested than the established per-framework libraries. The full surface is documented in the [API reference](/components/tags-api).
