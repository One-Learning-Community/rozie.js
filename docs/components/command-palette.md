...
- **`position: fixed` overlay, with an escape hatch for clipping ancestors.** The overlay is a `position: fixed` full-viewport backdrop + a centered `role="dialog"` panel, rendered only while `open`. By default it renders in place — an ancestor with `overflow: hidden`, `transform`, `filter`, or `contain` traps a fixed overlay just like it would any other, which is precisely why the `appendTo` prop exists: set it to portal the overlay out from under a clipping ancestor. See [Escaping a clipped ancestor](/components/command-palette-api#escaping-a-clipped-ancestor-appendto) in the API reference. A click on the backdrop (not the panel) closes; Escape closes; selecting an item closes when `closeOnSelect` (the default).
