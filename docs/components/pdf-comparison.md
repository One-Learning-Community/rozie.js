...
- **`@rozie-ui/pdf` is `0.1.0`.** The surface (12 props / 8 events / 20-verb handle / two-way `page` model / selectable text layer / text find / single + continuous render modes) is stable and gate-verified, but it is younger than the multi-year incumbents.
