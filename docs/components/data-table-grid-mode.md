...
Grid mode also ships a spreadsheet-style clipboard, fill, and clear surface, scoped to the current active cell / range selection. Copy, paste, cut, and clear are no-ops — falling through to the browser's native behavior — while a **header** cell is active, so none of them ever silently mutates a body cell from an unexpected focus position. Select-all behaves differently (it always intercepts the key; see its bullet below):
...
- **Row counting.** The `<table>` root carries `aria-rowcount` — the header row count plus the full filtered pre-pagination data total (not just the current page) — and every header/body `<tr>` carries a 1-based `aria-rowindex` that already accounts for the header rows ahead of it. These are present in both `table` and `grid` interaction mode, and are unaffected by pagination or row windowing — the index always reflects the row's position in the full model.
- **Group-header rows (grouping active).** A group-header row's `<tr>` carries `aria-expanded` (`"true"` / `"false"`), reflecting that group's collapsed/expanded state. While grouping is active, every row — group header and leaf alike — additionally carries `aria-level` (1-based nesting depth), giving the multi-level grouped-row hierarchy the same treegrid semantics APG expects.
- **Plain expandable rows.** `<tr>`-level `aria-expanded` is reserved for group-header rows; a non-grouped expandable row's `aria-expanded` lives on its expander `<button>` instead, never on the row itself. The two cases never overlap — a group-header row's own expand affordance is a separate group-toggle button, not the leading detail chevron.
