...
`Chart` is **generic**: the `type` prop switches the chart kind across the whole Chart.js controller set — `line`, `bar`, `pie`, `doughnut`, `radar`, `polarArea`, `scatter`, `bubble`, and any registerable controller. Chart.js v3+ ships tree-shakable with nothing pre-registered, and the generic `Chart` follows that model — it does **not** auto-register controllers; you register what you use, import the kitchen-sink `/auto` entry, or reach for one of the eight per-type components below. See [Registration & per-type components](#registration-per-type-components).
