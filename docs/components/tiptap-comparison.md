...
| Imperative command handle | ✅ (the `Editor`) | ✅ (the `Editor`) | ✅ (you own `Editor`) | ✅ (store) | ✅ (read hooks) | hand-roll | ✅ 20-verb `$expose` |
...
- **A uniform 20-verb imperative command handle** (`$expose`) with the same shape on every target — versus "hold the `Editor` you happened to construct" (which differs per framework: hook return, ref, store, or a directive input).
...
const placeholderExtensions = $props.placeholder
  ...
  : []
// consumer $props.extensions stay LAST so they still win — and a StarterKit-bundled
// node/mark the consumer replaces (a same-named entry in extensions) is auto-disabled
// via StarterKit.configure(...) unless the consumer already decided its fate via
// the `starterKit` prop; a final name-dedup keeps the last (consumer) occurrence.
...
- **`@rozie-ui/tiptap` is `0.1.0`** — the surface (10 props / 4 events / 20-verb handle / `toolbar` + `bubbleMenu` + `floatingMenu` mount-once slots + `nodeView` reactive slot) is stable and gate-verified, but it is younger than the multi-year official wrappers.
