...
`<NodeType resizable>` opts a node TYPE into corner-handle resizing that persists an explicit `width` / `height` back through the two-way `graph` model. See [Node resizer](/components/rete-editing#node-resizer).
