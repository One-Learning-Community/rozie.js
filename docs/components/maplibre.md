...
Data and styled layers come in through the `:sources` / `:layers` config-array props, or as declarative `<Source>` / `<Layer>` children nested under `<MapLibre>`; both shapes feed the same id-keyed, style-load-gated registry. The full sub-API (both authoring shapes, hit-testing layer features via `:interactiveLayerIds`, and the style-load timing rules) lives on [Sources & layers](/components/maplibre-sources-layers).
