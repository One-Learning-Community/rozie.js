...
<<< ../../packages/ui/combobox/src/Combobox.rozie{html}[Combobox.rozie — the source]

…and Rozie compiles it to six framework-native components. Switch the tabs to see the **actual generated output** for each target (this is exactly what ships in `@rozie-ui/combobox-{react,vue,svelte,angular,solid,lit}`):
