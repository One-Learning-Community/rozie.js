...
- **`@rozie-ui/resizable` is pre-1.0.** The surface is stable and gate-verified, but it is younger and less battle-tested than the established per-framework libraries. The full prop, event, handle, and slot tables live in the [API reference](/components/resizable-api).
