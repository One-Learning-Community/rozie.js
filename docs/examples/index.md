...
- [SCSS styling](/examples/scss): `<style lang="scss">` with compile-time dart-sass, a Sass map iterated with `@each`, `@for` / `@function` with `@if` control flow, `%placeholder` + `@extend`, `#{…}` interpolation.
- [TypeScript authoring](/examples/typescript): `<script lang="ts">` with author-declared `interface` / `type`, a type-only import, typed `$computed` declarations, and the statement-position type-hoist for the class-based targets.
