...
| --- | --- | --- | --- |
| **G1 — `basicSetup`-style batteries** (autocomplete, search, bracket matching, code folding, lint gutter) | `@uiw/react-codemirror` (`basicSetup`/`minimalSetup`) and most | **Medium** | **⏳ Deferred** — Rozie bundles a thin baseline (line numbers + history + default keymap). Everything else comes through `:extensions` today; a `basicSetup`-style toggle is a candidate follow-up. |
...
| G3 — Full theme-extension prop | `@uiw` (`theme` prop + `@uiw/codemirror-themes` catalog) | Low–Medium | **⏳ Deferred** — `theme` is a string (`"light"`/`"dark"` → `oneDark`), not an arbitrary theme-extension passthrough. Custom themes work **today** via `:extensions` (composed last, so they win). |
...
- **The bundled baseline is intentionally thin.** Rozie ships one language (JavaScript), one dark theme (`oneDark`), and a fixed keymap (default + history). `@uiw/react-codemirror`'s batteries-included `basicSetup` (autocomplete, search, fold, bracket matching, lint gutter) is richer out of the box — a richer Rozie setup carries more `:extensions` wiring. That is by design (CodeMirror 6 *is* an extension array, and the `:extensions` passthrough is the curated-vs-long-tail seam), but it is a real ergonomics difference for a single-framework app that wants everything pre-wired.
...
- **`@rozie-ui/codemirror` is `0.1.0`.** The surface (7 props / 0 events / 8-verb `$expose` handle / one `panel` portal slot) is stable and gate-verified, but it is younger than the multi-year community wrappers.
