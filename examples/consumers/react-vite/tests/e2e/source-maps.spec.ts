// DX-01: stack frames from runtime errors thrown inside `.rozie` code resolve
// through the source-map chain (magic-string `.rozie → .tsx` map →
// @vitejs/plugin-react `.tsx → .js` map → Vite `.js → bundle` composition)
// back to the original `.rozie` file.
//
...
//
// NOTE: under our path-virtual scheme (D-58), the chain composition produces
// sources entries ending in `.rozie.tsx` (the synthetic id ends in `.tsx` to
// satisfy plugin-react's transform). For automated verification, we accept
// `.rozie` or `.rozie.tsx` as the source-anchor.
...
test('Runtime error stack frame resolves to .rozie source via source-map chain (DX-01)', async ({
  page,
}) => {
  const errors: Error[] = [];
  page.on('pageerror', (e) => {
  ...
  // After path-virtual the synthetic id ends in `.rozie.tsx`. We accept either.
  const rozieSources = sources.filter((s) => /\.rozie(\.tsx)?$/.test(s));
  expect(
    rozieSources.length,
    `expected sources[] to include .rozie or .rozie.tsx entries; got: ${JSON.stringify(sources.slice(0, 12))}`,
  ).toBeGreaterThan(0);
  ...
  // compiled .tsx text. Both are valid composition outcomes; both let DevTools
  // navigate to recognizable Rozie-author code.
  ...
  expect(original.source).toMatch(/\.rozie(\.tsx)?$/);
});
