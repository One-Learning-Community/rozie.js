...
| Build (per-package) | **`tsdown`** (Rolldown-backed) | tsup successor, ESM-first, dual ESM/CJS + d.ts in one shot, 10-100x faster type emission via Oxc isolated-decl. **One documented exception:** `@rozie/runtime-angular` builds with `ng-packagr` instead — see note below. |
...
| `tsdown` (^0.21) | Per-package bundle to dual ESM/CJS + d.ts | Swap in tsup if tsdown causes friction (it's young — 0.x). Migration is a one-line config change either way. **Deviation:** `@rozie/runtime-angular` does NOT use tsdown. It builds with `ng-packagr -p ng-package.json -c tsconfig.lib.json` at `angularCompilerOptions.compilationMode: "partial"` because decorated Angular code must ship partial-Ivy declarations to be AOT-consumable by downstream `ngtsc` builds — a shape tsdown cannot produce. This is the single documented exception to the per-package tsdown default (Phase 80 D-05); the build recipe is the one already proven at `packages/ui/pagination/packages/angular`. |
...
- **Spike findings for rozie** (implementation patterns, constraints, gotchas) → `Skill("spike-findings-rozie")`
<!-- GSD:skills-end -->
