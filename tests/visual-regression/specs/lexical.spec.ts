...
// Bootstrap escape hatch (matrix.spec.ts parity): the screenshot cell is
// test.fixme until `LexicalScreenshot.png` exists, but the PNG can only be
// generated by running the cell under Playwright's `-u` (which does NOT override
// test.fixme). Set ROZIE_VR_BOOTSTRAP_BASELINE=LexicalScreenshot (the Docker
// baseline-regen recipe's `vr.sh -u -b LexicalScreenshot`) to force-ungate for ONE
// generation pass; never set in CI.
...
const screenshotUngated =
  screenshotBaselineExists || BOOTSTRAP_BASELINES.has('LexicalScreenshot');
...
for (const target of TARGETS) {
  ...
  const runner =
    !built || KNOWN_FAILING.has(target) || !screenshotUngated
      ? test.fixme
      : test;
