// Phase 12 dogfood: the `.phone` custom MODEL modifier compiles via emitVue
...
function compile(source: string, filename: string) {
  ...
  return emitVue(lowered.ir, { source, filename, modifierRegistry: registry });
}
...
describe('Phase 12 — .phone compiles via emitVue (custom model-modifier dogfood)', () => {
  it('emits the phone-reformat valueTransform on r-model.phone.lazy', () => {
    const result = compile(SOURCE, 'PhoneInput.rozie');
    expect(result.diagnostics.filter((d) => d.severity === 'error')).toEqual([]);
    // A custom modifier has no native Vue equivalent — Vue hand-emits the
    // valueTransform into a :value + @input/@change handler.
