// Phase 12 dogfood: the `.phone` custom MODEL modifier compiles via emitSolid
...
function compile(source: string, filename: string) {
  ...
  return emitSolid(lowered.ir, { source, filename, modifierRegistry: registry });
}
...
describe('Phase 12 — .phone compiles via emitSolid (custom model-modifier dogfood)', () => {
  it('emits the phone-reformat valueTransform on r-model.phone.lazy', () => {
    const result = compile(SOURCE, 'PhoneInput.rozie');
    expect(result.diagnostics.filter((d) => d.severity === 'error')).toEqual([]);
    // Solid splices the valueTransform around e.currentTarget.value; .lazy
    // swaps onInput -> onChange (Solid's native change event).
