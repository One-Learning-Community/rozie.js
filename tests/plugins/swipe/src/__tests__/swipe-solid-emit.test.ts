...
// emitSolid target WITHOUT any change to @rozie/core.
//
...
// Solid output. Proves D-22b SemVer stability of the public extension surface
// across the Solid target (which consumes swipeModifier.solid() +
// SolidEmissionDescriptor — landed in Plan 01/02).
...
function compile(source: string, filename: string) {
  ...
  const result = emitSolid(lowered.ir, { source, filename, modifierRegistry: registry });
  return result;
}

describe('MOD-05 — swipe compiles via Phase 6 emitSolid (D-22b SemVer proof)', () => {
...
{ swiped: false }
    ...
    // The inlineGuard code (swipeModifier.solid() hook) is spliced into the
    // synthesized Solid event handler. The directional comment marker proves
    // the swipe modifier's solid() hook fired and the inlineGuard path ran.
