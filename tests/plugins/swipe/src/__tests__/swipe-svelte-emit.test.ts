...
// emitSvelte target WITHOUT any change to @rozie/core.
//
...
// Svelte output. Proves D-22b SemVer stability of the public extension surface
// across the Svelte target.
...
function compile(source: string, filename: string) {
  ...
  const result = emitSvelte(lowered.ir, { source, filename, modifierRegistry: registry });
  return result;
}

describe('MOD-05 — swipe compiles via Phase 5 emitSvelte (D-22b SemVer proof)', () => {
...
{ swiped: false }
    ...
    // The inlineGuard code (swipeModifier.svelte() hook) is spliced into the
    // synthesized Svelte event handler. The directional comment marker proves
    // the swipe modifier's svelte() hook fired and the inlineGuard path ran.
