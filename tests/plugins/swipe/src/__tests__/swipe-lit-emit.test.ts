// Phase 07.1 Plan 03 — MOD-05 dogfood: swipe modifier compiles via the Phase 6
// emitLit target WITHOUT any change to @rozie/core.
//
...
// Lit output. Proves D-22b SemVer stability of the public extension surface
// across the Lit target (which dispatches swipeModifier.lit() +
// LitEmissionDescriptor via the registry — landed in Plan 01/02).
...
function compile(source: string, filename: string) {
  ...
  const result = emitLit(lowered.ir, { source, filename, modifierRegistry: registry });
  return result;
}

describe('MOD-05 — swipe compiles via Phase 6 emitLit (D-22b SemVer proof)', () => {
...
{ swiped: false }
    ...
    // The inlineGuard code (swipeModifier.lit() hook) is spliced into the
    // synthesized Lit event handler. The directional comment marker proves the
    // swipe modifier's lit() hook fired via registry dispatch and the
    // inlineGuard path ran.
