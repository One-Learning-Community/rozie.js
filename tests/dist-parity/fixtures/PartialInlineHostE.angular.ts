...
@Component({
  selector: 'rozie-partial-inline-host-e',
  standalone: true,
  template: `

    <div class="partial-inline-host" #rozieSpread_0 #rozieListenersTarget_1>
      <span class="echo">{{ rozieDisplay(usedNameE(1)) }}</span>
      <span class="echo">{{ rozieDisplay(hostTailE(1)) }}</span>
    </div>

  `,
  styles: [`
    :host(rozie-partial-inline-host-e) { display: contents; }
  `],
})
export class PartialInlineHostE {
  base = input<number>(1);
