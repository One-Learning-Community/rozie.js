...
@Component({
  selector: 'rozie-clone-probe',
  standalone: true,
  template: `

    <div class="probe" #rozieSpread_0 #rozieListenersTarget_1>
      <span class="count">count: {{ rozieDisplay(state().count) }}</span>
      @if (cloned()) {
    <span class="cloned">cloned</span>
    }</div>
...
export class CloneProbe {
  state = signal({
    count: 0,
    created: new Date(0)
  });
  cloned = signal<any>(null);

  ngAfterViewInit() {
    this.cloned.set(structuredClone(this.state()));
  }
