...
export default function SearchInput(_props: SearchInputProps): JSX.Element {
  ...
  return (
    ...
    <div {...attrs} class={"search-input" + (((attrs as unknown as Record<string, unknown>).class as string | undefined) ? " " + ((attrs as unknown as Record<string, unknown>).class as string | undefined) : "")} data-rozie-s-8bbc4a60="">
      ...
      <input type="search" ref={(el) => { inputElRef = el as HTMLElement; }} placeholder={local.placeholder} value={query()} onInput={($event: InputEvent & { currentTarget: HTMLInputElement; target: Element }) => { (e => setQuery(e.currentTarget.value))($event); _rozieDebouncedOnSearch($event); }} onKeyDown={($event: KeyboardEvent & { currentTarget: HTMLInputElement; target: Element }) => { (($event: KeyboardEvent & { currentTarget: HTMLInputElement; target: Element }) => { if ($event.key !== 'Enter') return; onSearch(); })($event); (($event: KeyboardEvent & { currentTarget: HTMLInputElement; target: Element }) => { if ($event.key !== 'Escape') return; clear(); })($event); }} data-rozie-s-8bbc4a60="" />
