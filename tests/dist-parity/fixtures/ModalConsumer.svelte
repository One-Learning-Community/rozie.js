...
<div {...__rozieAttrs} class={["modal-consumer", (__rozieAttrs)?.class]} use:applyListeners={__rozieAttrs} data-rozie-s-5d081d3a>
  <Modal bind:open={open1} data-rozie-s-5d081d3a>{#snippet header({ close })}
      <h2 data-rozie-s-5d081d3a>{title}</h2>
  ...
  <Modal bind:open={open2} data-rozie-s-5d081d3a snippets={{ [slotName]: __rozieDynSlot_0 }}>
  ...
  <WrapperModal bind:open={open3} title={title} data-rozie-s-5d081d3a>{#snippet brand()}
