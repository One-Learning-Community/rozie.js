...
@Component({
  selector: 'rozie-element-plus-slot-fallthrough',
  standalone: true,
  imports: [NgTemplateOutlet, NgClass],
  template: `

    <ng-container *ngTemplateOutlet="(headerTpl ?? __rozieFillMap()['header'] ?? templates()?.['header'])" />
    <div class="epsf-root" [ngClass]="'epsf-root--' + variant()" #rozieSpread_0 #rozieListenersTarget_1>chrome</div>
  ...
  `,
  styles: [`
    :host(rozie-element-plus-slot-fallthrough) { display: contents; }
    .epsf-root {
      display: block;
      padding: 0.5rem;
...
export class ElementPlusSlotFallthrough {
  ...
  static ngTemplateContextGuard(
    _dir: ElementPlusSlotFallthrough,
    _ctx: unknown,
