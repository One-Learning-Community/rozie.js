...
@Component({
  selector: 'rozie-partial-inline-host-d',
  standalone: true,
  template: `

    <div class="partial-inline-host" #rozieSpread_0 #rozieListenersTarget_1>
      <span class="echo">{{ rozieDisplay(outer()) }}</span>
    </div>

  `,
  styles: [`
    :host(rozie-partial-inline-host-d) { display: contents; }
  `],
})
export class PartialInlineHostD {
  base = input<number>(1);

  inner = computed(() => this.base() + 10);
  outer = computed(() => clampD(this.inner() + this.base()));
