...
@Component({
  selector: 'rozie-partial-inline-host-i',
  standalone: true,
  template: `

    <div class="partial-inline-host" #rozieSpread_0 #rozieListenersTarget_1>
      <span class="echo">{{ rozieDisplay(rangeTransitionI) }}</span>
      <span class="echo">{{ rozieDisplay(afterDeclI(1)) }}</span>
      <span class="echo">{{ rozieDisplay(fillDragUpI) }}</span>
    </div>

  `,
  styles: [`
    :host(rozie-partial-inline-host-i) { display: contents; }
  `],
})
export class PartialInlineHostI {
  base = input<number>(1);
