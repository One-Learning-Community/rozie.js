...
@Component({
  selector: 'rozie-partial-inline-host-k',
  standalone: true,
  template: `

    <div class="partial-inline-host" #rozieSpread_0 #rozieListenersTarget_1>
      <span class="echo">{{ rozieDisplay(ariaSortK(1)) }}</span>
      <span class="echo">{{ rozieDisplay(sortIndicatorK(1)) }}</span>
    </div>

  `,
  styles: [`
    :host(rozie-partial-inline-host-k) { display: contents; }
  `],
})
export class PartialInlineHostK {
  base = input<number>(1);
