...
export default function InlineEquivHostJ(_props: InlineEquivHostJProps): JSX.Element {
  ...
  return (
    <>
    <div {...attrs} class={"partial-inline-host" + (((attrs as unknown as Record<string, unknown>).class as string | undefined) ? " " + ((attrs as unknown as Record<string, unknown>).class as string | undefined) : "")} data-rozie-s-6eab802a="">
      <span class={"echo"} data-rozie-s-6eab802a="">{rozieDisplay(setColumnFilterJ(1, 2))}</span>
      <span class={"echo"} data-rozie-s-6eab802a="">{rozieDisplay(refreshRowModelJ)}</span>
    </div>
    </>
  );
}
