...
export default function InlineEquivHostI(_props: InlineEquivHostIProps): JSX.Element {
  ...
  return (
    <>
    <div {...attrs} class={"partial-inline-host" + (((attrs as unknown as Record<string, unknown>).class as string | undefined) ? " " + ((attrs as unknown as Record<string, unknown>).class as string | undefined) : "")} data-rozie-s-ce4a0b5a="">
      <span class={"echo"} data-rozie-s-ce4a0b5a="">{rozieDisplay(rangeTransitionI)}</span>
      <span class={"echo"} data-rozie-s-ce4a0b5a="">{rozieDisplay(afterDeclI(1))}</span>
      <span class={"echo"} data-rozie-s-ce4a0b5a="">{rozieDisplay(fillDragUpI)}</span>
    </div>
    </>
  );
}
