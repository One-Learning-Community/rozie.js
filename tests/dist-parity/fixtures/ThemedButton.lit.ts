...
@customElement('rozie-themed-button')
export default class ThemedButton extends SignalWatcher(LitElement) {
  static styles = css`
:host{display:contents}
.btn[data-rozie-s-7914ecaa] {
  display: inline-flex;
  align-items: center;
  gap: 0.5rem;
  padding: 0.5rem 1rem;
...
`;
  ...
  render() {
    return html`
<button class="btn ${(this.variant)}" style=${styleMap({ '--btn-bg': '#3b82f6', '--btn-fg': '#ffffff' })} ${rozieSpread(this.$attrs)} ${rozieListeners(this.$listeners)} data-rozie-s-7914ecaa>
  ${this.label}
</button>
`;
  }
  ...
  private get $attrs(): Record<string, string> {
    const __skip = new Set<string>(['data-rozie-ref', 'label', 'variant']);
