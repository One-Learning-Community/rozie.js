...
@Component({
  selector: 'rozie-partial-inline-host-c',
  standalone: true,
  template: `

    <div class="partial-inline-host" #rozieSpread_0 #rozieListenersTarget_1>
      <span class="echo">{{ rozieDisplay(usedName()) }}</span>
    </div>

  `,
  styles: [`
    :host(rozie-partial-inline-host-c) { display: contents; }
  `],
})
export class PartialInlineHostC {
  base = input<number>(1);

  usedName = computed(() => clamp(this.double(this.base())));
