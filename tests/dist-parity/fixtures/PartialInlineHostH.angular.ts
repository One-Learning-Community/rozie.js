...
@Component({
  selector: 'rozie-partial-inline-host-h',
  standalone: true,
  template: `

    <div class="partial-inline-host" #rozieSpread_0 #rozieListenersTarget_1>
      <span class="echo">{{ rozieDisplay(editTransitionH) }}</span>
      <span class="echo">{{ rozieDisplay(editorBindingsH(1)) }}</span>
    </div>

  `,
  styles: [`
    :host(rozie-partial-inline-host-h) { display: contents; }
  `],
})
export class PartialInlineHostH {
  base = input<number>(1);
