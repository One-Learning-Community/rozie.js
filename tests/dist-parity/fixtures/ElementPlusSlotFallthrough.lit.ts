...
@customElement('rozie-element-plus-slot-fallthrough')
export default class ElementPlusSlotFallthrough extends SignalWatcher(LitElement) {
  static styles = css`
:host{display:contents}
.epsf-root[data-rozie-s-986e3472] {
  display: block;
  padding: 0.5rem;
...
<div class="epsf-root ${(rozieClass('epsf-root--' + this.variant))}" ${rozieSpread(this.$attrs)} ${rozieListeners(this.$listeners)} data-rozie-s-986e3472>chrome</div>
<slot></slot>
...
`;
  ...
  private get $attrs(): Record<string, string> {
    const __skip = new Set<string>(['data-rozie-ref', 'variant']);
