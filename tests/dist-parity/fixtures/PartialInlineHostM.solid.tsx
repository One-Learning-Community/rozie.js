...
export default function PartialInlineHostM(_props: PartialInlineHostMProps): JSX.Element {
  ...
  return (
    <>
    <div {...attrs} class={"partial-inline-host" + (((attrs as unknown as Record<string, unknown>).class as string | undefined) ? " " + ((attrs as unknown as Record<string, unknown>).class as string | undefined) : "")} data-rozie-s-5d3a593a="">
      <span class={"echo"} data-rozie-s-5d3a593a="">{rozieDisplay(gridKeydownHandlersM(1, 2))}</span>
      <span class={"echo"} data-rozie-s-5d3a593a="">{rozieDisplay(refreshRowModelM)}</span>
    </div>
    </>
  );
}
