...
@customElement('rozie-loop-mustache-keyed-slot-rfor')
export default class LoopMustacheKeyedSlotRfor extends SignalWatcher(LitElement) {
  static styles = css`
...
<div class="r" ${rozieSpread(this.$attrs)} ${rozieListeners(this.$listeners)} data-rozie-s-10bfe9b6>${repeat<any>(this.rows, (row, _idx) => row.id, (row, _idx) => html`${this.__rozieDefaultSlot__ !== undefined ? this.__rozieDefaultSlot__({name: row}) : html`<slot data-rozie-params=${(() => { try { return JSON.stringify({name: row}); } catch { return '{}'; } })()}></slot>`}`)}</div>
`;
  }
  ...
  private get $attrs(): Record<string, string> {
    const __skip = new Set<string>(['data-rozie-ref', 'rows']);
