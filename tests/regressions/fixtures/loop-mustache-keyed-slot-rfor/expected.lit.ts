...
<div class="r" ${rozieSpread(this.$attrs)} ${rozieListeners(this.$listeners)} data-rozie-s-10bfe9b6>${repeat<any>(this.rows, (row, _idx) => row.id, (row, _idx) => html`${this.rozieSlots?.[row] !== undefined ? this.rozieSlots?.[row]!({}) : html`<slot name="${row}"></slot>`}`)}</div>
`;
  }
