...
@customElement('rozie-on-mount-mount-local-cleanup')
export default class OnMountMountLocalCleanup extends SignalWatcher(LitElement) {
  static styles = css`
...
`;
  ...
  firstUpdated(): void {
    this._disconnectCleanups.push((() => clearInterval(timer)));
  ...
  render() {
    return html`
<div class="ticks" ${rozieSpread(this.$attrs)} ${rozieListeners(this.$listeners)} data-rozie-s-c1a25008>${this._ticks.value}</div>
`;
  }
  ...
  private get $attrs(): Record<string, string> {
    const __skip = new Set<string>(['data-rozie-ref', 'label']);
