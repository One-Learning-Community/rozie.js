...
`;
  ...
  render() {
    return html`
<div class="fpg" ${rozieSpread(this.$attrs)} ${rozieListeners(this.$listeners)} data-rozie-s-a5944868>
  <button class="save" @click=${this.onSave} data-rozie-s-a5944868>save</button>
  <button class="cancel" @click=${($event: MouseEvent & { currentTarget: HTMLButtonElement; target: HTMLButtonElement }) => { this.onCancel && this.onCancel(); }} data-rozie-s-a5944868>cancel</button>
  <span class="n" data-rozie-s-a5944868>${this._n.value}</span>
</div>
`;
  }
  ...
  private get $attrs(): Record<string, string> {
    const __skip = new Set<string>(['data-rozie-ref', 'on-save', 'onsave', 'on-cancel', 'oncancel']);
