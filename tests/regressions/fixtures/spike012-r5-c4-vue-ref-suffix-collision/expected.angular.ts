...
@Component({
  selector: 'rozie-ref-suffix-collision',
  standalone: true,
  template: `
...
export class RefSuffixCollision {
  n = signal(0);
  box = viewChild<ElementRef<HTMLDivElement>>('box');

  ngAfterViewInit() {
    this.n.set((this.box()?.nativeElement as HTMLElement).childElementCount + this.boxRef);
  }
...
export default RefSuffixCollision;
