...
@customElement('rozie-label-expose-shadow')
export default class LabelExposeShadow extends SignalWatcher(LitElement) {
  static styles = css`
...
`;
  ...
  render() {
    return html`
<div class="n" ${rozieSpread(this.$attrs)} ${rozieListeners(this.$listeners)} data-rozie-s-24395436>${this._n.value}</div>
`;
  }
  ...
  noise(): void {
    reset: for (let i = 0; i < 3; i++) {
      if (i === 1) continue reset;
      if (i === 2) break reset;
    }
  }
