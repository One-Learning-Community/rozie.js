...
private get theme() { return (this.__rozieCtxConsumer_theme.value) as {
  ...
  render() {
    return html`
<div class="r" ${rozieSpread(this.$attrs)} ${rozieListeners(this.$listeners)} data-rozie-s-f072667f><button @click=${($event: MouseEvent & { currentTarget: HTMLButtonElement; target: HTMLButtonElement }) => { this.theme && this.theme.cycle(); }} data-rozie-s-f072667f>${rozieDisplay(this.theme && this.theme.color)}</button></div>
`;
  }
