...
@customElement('rozie-ref-reserved-member-const')
export default class RefReservedMemberConst extends SignalWatcher(LitElement) {
  static styles = css`
:host{display:contents}
`;

  private _n = signal(0);
  @query('[data-rozie-ref="title"]') private _refTitle!: HTMLElement;
  ...
  firstUpdated(): void {
    this._n.value = (this._refTitle as HTMLElement).childElementCount + this.title$local.length;
  }
  ...
  render() {
    return html`
<div class="r" ${rozieSpread(this.$attrs)} ${rozieListeners(this.$listeners)} data-rozie-s-2d369a20>
  <div data-rozie-ref="title" data-rozie-s-2d369a20>x</div>
</div>
`;
  }
