...
export default function SolidEventParamType(_props: SolidEventParamTypeProps): JSX.Element {
  ...
  return (
    ...
    <div {...attrs} class={"root" + (((attrs as unknown as Record<string, unknown>).class as string | undefined) ? " " + ((attrs as unknown as Record<string, unknown>).class as string | undefined) : "")} data-rozie-s-618cbb65="">
      <button class={"stop"} onClick={($event: MouseEvent & { currentTarget: HTMLButtonElement; target: Element }) => { $event.stopPropagation(); bump(); }} data-rozie-s-618cbb65="">{n()}</button>
      <button class={"plain"} onClick={($event: MouseEvent & { currentTarget: HTMLButtonElement; target: Element }) => { bump(); }} data-rozie-s-618cbb65="">plain</button>
    </div>
    </>
  );
}
