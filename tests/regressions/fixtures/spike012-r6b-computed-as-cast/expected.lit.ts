...
`;
  ...
  @property({ type: String, reflect: true }) raw: string = '';
  ...
  render() {
    return html`
<div class="r" ${rozieSpread(this.$attrs)} ${rozieListeners(this.$listeners)} data-rozie-s-6ad03933>${rozieDisplay(this.label)}</div>
`;
  }
  ...
  private get $attrs(): Record<string, string> {
    const __skip = new Set<string>(['data-rozie-ref', 'raw']);
