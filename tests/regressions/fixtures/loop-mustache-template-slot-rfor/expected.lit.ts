...
<div class="r" ${rozieSpread(this.$attrs)} ${rozieListeners(this.$listeners)} data-rozie-s-42e72e5a>${repeat<any>(this.items, (x, _idx) => x, (x, _idx) => html`${this.__rozieDefaultSlot__ !== undefined ? this.__rozieDefaultSlot__({name: x}) : html`<slot data-rozie-params=${(() => { try { return JSON.stringify({name: x}); } catch { return '{}'; } })()}></slot>`}`)}</div>
`;
  }
