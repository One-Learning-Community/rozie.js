...
<div class="r" ${rozieSpread(this.$attrs)} ${rozieListeners(this.$listeners)} data-rozie-s-42e72e5a>${repeat<any>(this.items, (x, _idx) => x, (x, _idx) => html`${this.rozieSlots?.[x] !== undefined ? this.rozieSlots?.[x]!({}) : html`<slot name="${x}"></slot>`}`)}</div>
`;
  }
