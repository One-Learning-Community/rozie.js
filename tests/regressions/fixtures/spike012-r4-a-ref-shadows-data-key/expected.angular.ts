...
@Component({
  selector: 'rozie-ref-shadows-data-key',
  standalone: true,
  template: `
...
export class RefShadowsDataKey {
  box = signal(0);
  n = signal(0);
  boxRef = viewChild<ElementRef<HTMLDivElement>>('boxRef');

  ngAfterViewInit() {
    this.n.set((this.boxRef()?.nativeElement as HTMLElement).childElementCount + this.box());
  }
...
export default RefShadowsDataKey;
