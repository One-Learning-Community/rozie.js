...
@customElement('rozie-lit-aria-mixin')
export default class LitAriaMixin extends SignalWatcher(LitElement) {
  static styles = css`
...
`;
  ...
  render() {
    return html`
<div class="lit-ariamixin" ${rozieSpread(this.$attrs)} ${rozieListeners(this.$listeners)} data-rozie-s-1cf1c129>
  ${this.ariaLabel ? html`<p data-rozie-s-1cf1c129>${this.ariaLabel}</p>` : nothing}${this.label ? html`<p data-rozie-s-1cf1c129>${this.label}</p>` : nothing}</div>
`;
  }
  ...
  private get $attrs(): Record<string, string> {
    const __skip = new Set<string>(['data-rozie-ref', 'aria-label', 'arialabel', 'label']);
