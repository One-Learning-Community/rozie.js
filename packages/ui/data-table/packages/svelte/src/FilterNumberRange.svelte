...
<span style="display:flex; align-items: center" data-rozie-s-97b2c090><input class="rdt-col-filter" part="col-filter" type="number" aria-label={rozieAttr(columnId + ' min')} placeholder={rozieAttr(minPlaceholder())} value={minDraft} oninput={($event) => { onMinInput($event); }} onchange={($event) => { applyRange(); }} data-rozie-s-97b2c090 /><span data-rozie-s-97b2c090> - </span><input class="rdt-col-filter" part="col-filter" type="number" aria-label={rozieAttr(columnId + ' max')} placeholder={rozieAttr(maxPlaceholder())} value={maxDraft} oninput={($event) => { onMaxInput($event); }} onchange={($event) => { applyRange(); }} data-rozie-s-97b2c090 /></span>
