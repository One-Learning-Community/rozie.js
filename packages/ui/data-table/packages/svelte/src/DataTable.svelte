...
interface Props {
  ...
  filter?: Snippet<[{ columnId: any; value: any; uniqueValues: any; minMax: any; setFilter: any }]>;
  cell?: Snippet<[{ columnId: any; column: any; row: any; value: any }]>;
  editor?: Snippet<[{ columnId: any; column: any; row: any; value: any; commit: any; cancel: any; autofocus: any }]>;
  snippets?: { [key: `colHeader-${string}`]: Snippet<[{ columnId: any; column: any; label: any }]>; [key: `filter-${string}`]: Snippet<[{ columnId: any; value: any; uniqueValues: any; minMax: any; setFilter: any }]>; [key: `cell-${string}`]: Snippet<[{ columnId: any; column: any; row: any; value: any }]>; [key: `editor-${string}`]: Snippet<[{ columnId: any; column: any; row: any; value: any; commit: any; cancel: any; autofocus: any }]>; } & Record<string, any>;
...
const cell$$slot = $derived(__cellProp ?? snippets?.cell);
...
let editVer = $state(0);
let editFocusColId: any = $state(null);
...
let colVirtualizerCleanup: any = null;
...
const effectiveColumnPinning = (): any => {
  ...
  if (expandable === true) rail.push(EXPANDER_COL_ID);
...
const seedColumnPinning = (): void => {
  ...
  columnPinningDefault = seeded;
  columnPinning = seeded;
};
...
const columnDefs = () => {
  const cfg = columns || [];
  const reg = colReg || {};
  if (columnDefsCache && columns === columnDefsCacheColumnsRef && colReg === columnDefsCacheColRegRef) {
  ...
  columnDefsCache = out;
  columnDefsCacheColumnsRef = columns;
...
const writeData = (next: any) => {
  ...
  programmatic++;
  dataDefault = fresh; // fresh raw array only (never in-place, never a proxy)
  data = fresh; // two-way emit if bound (no-op-diff if not)
  programmatic--;
};
...
const recordSnapshot = (current: any) => {
  undoStack.push(current);
  const limit = undoLimit != null ? undoLimit : 100;
...
const emitHistoryChangeIfEdged = (prevU: any, prevR: any) => {
  const nextU = canUndo();
  const nextR = canRedo();
...
const autoMeasureOn = () => autoMeasure === true;
...
const forcedColumns = (): number[] => {
  ...
  if (editingRow >= 0 && editingCol >= 0 && out.indexOf(editingCol) === -1) {
    out.push(editingCol);
  }
  return out;
};
...
const scheduleRemeasure = () => {
  ...
  if (typeof requestAnimationFrame === 'function') remeasureRaf = requestAnimationFrame(rafPass);else if (ranMicro) remeasurePending = false;else remeasureRaf = setTimeout(rafPass, 0);
};
...
const teardownRemeasure = () => {
  ...
  if (remeasureRaf != null) {
    ...
    if (typeof requestAnimationFrame === 'function') cancelAnimationFrame(remeasureRaf);else clearTimeout(remeasureRaf);
    remeasureRaf = null;
  }
  remeasurePending = false;
};
...
const refineRowEstimate = (): void => {
  ...
  // Explicit windowVer bump (87-07, Rule 1): virtual-core's own setOptions()+_willUpdate()
  // re-feed above does NOT itself invoke notify()/onChange (confirmed from the installed
  // source: _willUpdate() is a no-op once `scrollElement` is already set, dist/esm/
  // index.js:389-393), so a re-feed needs its own explicit trigger to make the spacer
  // re-derivation happen at all. Routed through bumpWindowVer() (87-10, see its own comment
  // above) rather than a raw `$data.windowVer = $data.windowVer + 1` — coalescing this bump
  // with any onChange-triggered ones already pending from the SAME synchronous sweep is what
...
const colIsOutsideWindow = (c: any) => {
...
let lastPropsData: unknown = null;
const maybeClearHistoryOnExternalSwap = () => {
  const pd = data;
  if (pd === lastPropsData) return; // $props.data did not change → not an external swap
  lastPropsData = pd;
  if (!undoable) return;
...
const pinStyle = (colId: any, zIndex = 1) => {
  if (tick() < 0 || !table) return '';
  const col = table.getColumn(colId);
...
const rowShowsDetail = (row: any) => getSubRows == null && !rowIsGrouped(row) && rowIsExpanded(row);
...
const rowIndexIsGrouped = (rowIndex: any) => rowIsGrouped((rows || [])[rowIndex]);
...
const groupRowDescriptor = (row: any) => {
...
const cellSlotRow = (row: any) => rowIsGrouped(row) ? groupRowDescriptor(row) : row ? row.original : null;
...
const onHideColumn = (colId: any, evt: any) => {
  if (evt && evt.stopPropagation) evt.stopPropagation();
  if (!table) return;
  const col = table.getColumn(colId);
...
const onGridKeyDown = (e: any) => {
  ...
  if (nextRow !== prevRow || nextCol !== prevCol || nextIsHeader !== prevIsHeader || nextLevel !== prevLevel) {
    ...
    onactivecellchange?.(nextIsHeader ? {
      rowIndex: null,
      colIndex: nextCol,
      isHeader: true
    } : {
...
// ══ Phase 87 87-04/87-05 — column-axis template windowing (D-06/D-11; declared HERE (after
// the columnChrome/gridFocusNav imports above) so a forward cross-partial reference to
// colsWindowed()/windowedColIndices() in the merged per-target output never TDZs on React —
// the 87-02 useCallback-deps-array ordering lesson, the SAME reason remeasureColumnWindow()
// below stays out of virtualization.rzts). ══
// windowedHeadersFor(hg, hgLevel) (87-05): generalizes 87-04's windowedHeaderRow() (leaf-level
// only) to EVERY header level with a CLAMPED colspan (D-11). A header spans the CONTIGUOUS run
// of leaf-column indices [cursor, cursor + header.colSpan) — table-core lays out every header
// level's `headers` array left-to-right over the SAME full leaf-column order, so walking a
// running cursor by each header's OWN colSpan reconstructs its absolute leaf range with no
// extra table-core calls. Intersect that range with windowedColIndices() (which already unions
// the D-10 forced set): a header whose intersection is EMPTY is dropped entirely (a group
// wholly outside the window renders no <th> at all — D-11); otherwise it is emitted as
// { header, span } with span = the intersection SIZE (the clamp). Degrades to hg.headers
...
const windowedHeadersFor = (hg: any, hgLevel: any) => {
...
// (byte-identical).
const windowedColSpan = () => colsWindowed() ? windowedColIndices().length + 2 : visibleColCount();
...
const edgeDelta = (clientX: any, clientY: any) => {
...
const resolveCellAt = (clientX: any, clientY: any) => {
...
const resolveEditFocusCellEl = (rowIndex: any, colIndex: any) => {
...
const focusEditorWhenReady = (rowIndex: any, colIndex: any, selectAll = true) => {
  if (!gridRoot) return;
  let attempts = 0;
  const tryFocus = () => {
...
const editorAutofocusFor = (colId: any, rowIndex: any) => {
  if (editVer < 0) return false;
...
const beginRowEdit = (row: any) => {
  ...
  editFocusColId = editable[0].colId;
...
const rowEditTab = (target: any, backward: any) => {
  ...
  editFocusColId = editable[nextPos].colId;
  editVer = editVer + 1;
...
$effect(() => { (() => [sorting, globalFilter, columnFilters, pagination,
...
rowCount, pageCount, rowSelection, expanded, expandable, grouping, groupable, columnVisibility, columnSizing, columnOrder, columnPinning, selectionMode, (data || []).length,
...
columns, colReg])(); untrack(() => { if (__rozieWatchInitial_0) { __rozieWatchInitial_0 = false; return; } (() => {
...
$effect(() => { (() => [sorting, columnFilters, globalFilter, sortingDefault, columnFiltersDefault, globalFilterDefault])(); untrack(() => { if (__rozieWatchInitial_1) { __rozieWatchInitial_1 = false; return; } (() => {
  const msg = buildSortFilterAnnounce();
  if (msg) liveAnnounce = msg;
})(); }); });
</script>

<div class="rozie-data-table-wrap" bind:this={__rozieRoot} data-rozie-s-d5dcab4c><div class="rdt-column-defs" style="display:none" aria-hidden="true" data-rozie-s-d5dcab4c>{@render children?.()}</div>{#if !!invalidMsg}<div class="rdt-sr-live" role="status" aria-live="polite" aria-atomic="true" data-rozie-s-d5dcab4c>{invalidMsg}</div>{/if}{#if !!pasteAnnounce}<div class="rdt-sr-live rdt-sr-paste" data-testid="paste-announce" role="status" aria-live="polite" aria-atomic="true" data-rozie-s-d5dcab4c>{pasteAnnounce}</div>{/if}{#if !!liveAnnounce}<div class="rdt-sr-live rdt-sr-sortfilter" data-testid="sortfilter-announce" role="status" aria-live="polite" aria-atomic="true" data-rozie-s-d5dcab4c>{liveAnnounce}</div>{/if}<div class="rdt-toolbar" data-rozie-s-d5dcab4c><input class="rdt-global-filter" type="text" role="searchbox" aria-label="Search table" value={globalFilterValue()} oninput={($event) => { onGlobalFilterInput($event); }} data-rozie-s-d5dcab4c />{#if allLeafColumns().length}<details class="rdt-colvis" data-rozie-s-d5dcab4c><summary class="rdt-colvis-summary" data-rozie-s-d5dcab4c>Columns</summary><div class="rdt-colvis-menu" role="group" aria-label="Toggle columns" data-rozie-s-d5dcab4c>{#each allLeafColumns() as lc (lc.id)}<label class="rdt-colvis-item" data-rozie-s-d5dcab4c><input type="checkbox" class="rdt-colvis-checkbox" checked={lc.visible} onchange={($event) => { onToggleVisibility(lc.id); }} data-rozie-s-d5dcab4c /><span class="rdt-colvis-label" data-rozie-s-d5dcab4c>{rozieDisplay(lc.label)}</span></label>{/each}</div></details>{/if}</div>{#if groupable}<div class="rdt-group-bar-host" data-rozie-s-d5dcab4c>{#if groupBar}{@render groupBar({ grouping: groupingKeys(), groupableColumns: groupableColumns(), applyGrouping, clearGrouping })}{:else}{#each groupingKeys() as gk (gk)}<span class="rdt-group-token" data-group-token="" data-rozie-s-d5dcab4c>{rozieDisplay(gk)}</span>{/each}{/if}</div>{/if}{#if isWindowed()}<div class="rdt-scroll" style={rozieStyle(rowsWindowed() && maxHeight ? 'max-height:' + maxHeight + ';overflow:auto;--rozie-data-table-max-height:' + maxHeight : 'overflow:auto')} data-rozie-s-d5dcab4c><table class={["rozie-data-table", { 'rdt-sticky': stickyHeader, 'rdt-col-windowed': colsWindowed() }]} role={rozieAttr(tableRole())} aria-rowcount={rozieAttr(gridAriaRowCount())} onkeydown={($event) => { onGridKeyDown($event); }} onfocusin={($event) => { syncActiveFromEvent($event); }} onfocusout={($event) => { onGridFocusOut($event); }} onmousedown={($event) => { onGridMouseDown($event); }} ondblclick={($event) => { onGridDblClick($event); }} onclick={($event) => { onGridClick($event); }} data-rozie-s-d5dcab4c><thead class="rdt-thead" role="rowgroup" data-rozie-s-d5dcab4c>{#each headerGroups as hg, hgLevel (hg.id)}<tr class="rdt-tr" role="row" aria-rowindex={rozieAttr(hgLevel + 1)} data-rozie-s-d5dcab4c>{#if colsWindowed()}<th class="rdt-col-spacer" aria-hidden="true" style={rozieStyle('width:' + colPadLeft() + 'px;padding:0;border:0')} data-rozie-s-d5dcab4c></th>{/if}{#each windowedHeadersFor(hg, hgLevel) as wh (wh.header.id)}<th class={["rdt-th", { 'rdt-select-th': isSelectColumn(wh.header.column.id), 'rdt-expander-th': isExpanderColumn(wh.header.column.id), 'rdt-th-resizing': columnIsResizing(wh.header.column.id), 'rdt-cell-active': isActiveCell('__header', headerColIndexOf(hg, wh.header), hgLevel) }]} role="columnheader" data-col={rozieAttr(wh.header.column.id)} data-grid-cell="" data-row="__header" data-header-level={rozieAttr(hgLevel)} colspan={rozieAttr(wh.span > 1 ? wh.span : null)} data-col-index={rozieAttr(headerColIndexOf(hg, wh.header))} tabindex={rozieAttr(cellTabindex('__header', headerColIndexOf(hg, wh.header), hgLevel))} aria-sort={rozieAttr(ariaSortFor(wh.header.column.id))} style={rozieStyle(thStyle(wh.header))} data-rozie-s-d5dcab4c>{#if isSelectColumn(wh.header.column.id)}<span style="display:contents" data-rozie-s-d5dcab4c>{#if selectAll}{@render selectAll({ checked: isAllRowsSelected(), indeterminate: isSomeRowsSelected(), toggle: onToggleAllRows })}{:else}{#if selectionMode === 'multiple'}<input class="rdt-select-all" type="checkbox" aria-label="Select all rows" checked={isAllRowsSelected()} onchange={($event) => { onToggleAllRows($event); }} data-rozie-s-d5dcab4c />{/if}{/if}</span>{:else if isExpanderColumn(wh.header.column.id)}<span style="display:contents" data-rozie-s-d5dcab4c></span>{:else}<span style="display:contents" data-rozie-s-d5dcab4c>{#if wh.header.column.getCanSort && wh.header.column.getCanSort()}<button type="button" class="rdt-sort-btn" onclick={($event) => { onHeaderSort(wh.header.column.id, $event); }} data-rozie-s-d5dcab4c><span class="rdt-header-label" data-rozie-s-d5dcab4c>{#if (snippets?.[`colHeader-${wh.header.column.id}`])}{@render (snippets?.[`colHeader-${wh.header.column.id}`])({ columnId: wh.header.column.id, column: wh.header.column, label: headerLabel(wh.header.column.id) })}{:else}{#if colHeader}{@render colHeader({ columnId: wh.header.column.id, column: wh.header.column, label: headerLabel(wh.header.column.id) })}{:else}{rozieDisplay(headerLabel(wh.header.column.id))}{/if}{/if}</span><span class="rdt-sort-ind" aria-hidden="true" data-rozie-s-d5dcab4c>{rozieDisplay(sortIndicator(wh.header.column.id))}</span></button>{:else}<span style="display:contents" data-rozie-s-d5dcab4c><span class="rdt-header-label" data-rozie-s-d5dcab4c>{#if (snippets?.[`colHeader-${wh.header.column.id}`])}{@render (snippets?.[`colHeader-${wh.header.column.id}`])({ columnId: wh.header.column.id, column: wh.header.column, label: headerLabel(wh.header.column.id) })}{:else}{#if colHeader}{@render colHeader({ columnId: wh.header.column.id, column: wh.header.column, label: headerLabel(wh.header.column.id) })}{:else}{rozieDisplay(headerLabel(wh.header.column.id))}{/if}{/if}</span></span>{/if}<Popover trigger="click" placement="bottom-end" strategy="fixed" offset={4} data-rozie-s-d5dcab4c>{#snippet anchor()}<button type="button" class="rdt-col-menu-trigger" aria-label={rozieAttr('Column options for ' + headerLabel(wh.header.column.id))} data-rozie-s-d5dcab4c>⋯</button>{/snippet}<div class="rdt-col-menu" role="menu" data-rozie-s-d5dcab4c><button type="button" role="menuitem" class="rdt-col-menu-item" aria-pressed={columnPinSide(wh.header.column.id) === 'left'} onclick={($event) => { onPinColumn(wh.header.column.id, 'left', $event); }} data-rozie-s-d5dcab4c>Pin left</button><button type="button" role="menuitem" class="rdt-col-menu-item" aria-pressed={columnPinSide(wh.header.column.id) === 'right'} onclick={($event) => { onPinColumn(wh.header.column.id, 'right', $event); }} data-rozie-s-d5dcab4c>Pin right</button><button type="button" role="menuitem" class="rdt-col-menu-item" aria-pressed={!columnPinSide(wh.header.column.id)} onclick={($event) => { onPinColumn(wh.header.column.id, false, $event); }} data-rozie-s-d5dcab4c>Unpin</button><hr class="rdt-col-menu-sep" data-rozie-s-d5dcab4c /><button type="button" role="menuitem" class="rdt-col-menu-item" onclick={($event) => { onHideColumn(wh.header.column.id, $event); }} data-rozie-s-d5dcab4c>Hide column</button></div></Popover><button type="button" class="rdt-resize-handle" aria-label={rozieAttr('Resize ' + headerLabel(wh.header.column.id))} onpointerdown={($event) => { onResizeStart(wh.header.column.id, $event); }} ontouchstart={($event) => { onResizeStart(wh.header.column.id, $event); }} data-rozie-s-d5dcab4c><span class="rdt-resize-grip" aria-hidden="true" data-rozie-s-d5dcab4c></span></button></span>{/if}</th>{/each}{#if colsWindowed()}<th class="rdt-col-spacer" aria-hidden="true" style={rozieStyle('width:' + colPadRight() + 'px;padding:0;border:0')} data-rozie-s-d5dcab4c></th>{/if}</tr>{/each}{#if hasAnyFilterableColumn()}<tr class="rdt-filter-row" data-rozie-s-d5dcab4c>{#if colsWindowed()}<th class="rdt-col-spacer" aria-hidden="true" style={rozieStyle('width:' + colPadLeft() + 'px;padding:0;border:0')} data-rozie-s-d5dcab4c></th>{/if}{#each windowedHeadersFor(headerGroups[headerGroups.length - 1], headerGroups.length - 1) as wh (wh.header.id)}<th class="rdt-filter-cell" role="presentation" data-col={rozieAttr(wh.header.column.id)} style={rozieStyle(pinStyle(wh.header.column.id))} data-rozie-s-d5dcab4c>{#if isSelectColumn(wh.header.column.id)}<span style="display:contents" data-rozie-s-d5dcab4c></span>{:else if isExpanderColumn(wh.header.column.id)}<span style="display:contents" data-rozie-s-d5dcab4c></span>{:else}<span style="display:contents" data-rozie-s-d5dcab4c>{#if columnIsFilterable(wh.header.column.id)}<span style="display:contents" data-rozie-s-d5dcab4c>{#if (snippets?.[`filter-${wh.header.column.id}`])}{@render (snippets?.[`filter-${wh.header.column.id}`])({ columnId: wh.header.column.id, value: columnFilterValue(wh.header.column.id), uniqueValues: getFacetedUniqueValues(wh.header.column.id), minMax: getFacetedMinMaxValues(wh.header.column.id), setFilter: setColumnFilter })}{:else}{#if filter}{@render filter({ columnId: wh.header.column.id, value: columnFilterValue(wh.header.column.id), uniqueValues: getFacetedUniqueValues(wh.header.column.id), minMax: getFacetedMinMaxValues(wh.header.column.id), setFilter: setColumnFilter })}{:else}<input class="rdt-col-filter" type="text" aria-label={rozieAttr('Filter ' + headerLabel(wh.header.column.id))} value={columnFilterValue(wh.header.column.id)} oninput={($event) => { onColumnFilterInput(wh.header.column.id, $event); }} onclick={($event) => { stopEvent($event); }} data-rozie-s-d5dcab4c />{/if}{/if}</span>{/if}</span>{/if}</th>{/each}{#if colsWindowed()}<th class="rdt-col-spacer" aria-hidden="true" style={rozieStyle('width:' + colPadRight() + 'px;padding:0;border:0')} data-rozie-s-d5dcab4c></th>{/if}</tr>{/if}</thead><tbody class="rdt-tbody" role="rowgroup" data-rozie-s-d5dcab4c><tr class="rdt-spacer" aria-hidden="true" data-rozie-s-d5dcab4c><td colspan={rozieAttr(windowedColSpan())} style={rozieStyle('height:' + padTop() + 'px;padding:0;border:0')} data-rozie-s-d5dcab4c></td></tr>{#each windowedRows() as wr (wr.row.id)}<tr class={["rdt-tr", { 'rdt-group-header': rowIsGrouped(wr.row), 'rdt-row-pinned': wr.pinned }]} role="row" data-row={rozieAttr(wr.vi.index)} aria-rowindex={rozieAttr(headerRowCount() + wr.vi.index + 1)} data-index={rozieAttr(wr.vi.index)} data-pinned={rozieAttr(wr.pinned ? 'true' : null)} data-depth={rozieAttr(wr.row.depth)} data-group-header={rozieAttr(rowIsGrouped(wr.row) ? wr.row.id : null)} data-group-leaf={rozieAttr(groupingActive() && !rowIsGrouped(wr.row) ? wr.row.id : null)} aria-expanded={rozieAttr(rowIsGrouped(wr.row) ? !!rowIsExpanded(wr.row) : null)} aria-selected={rozieAttr(selectionMode !== 'none' ? !!rowIsSelected(wr.row) : null)} aria-level={rozieAttr(groupingActive() ? wr.row.depth + 1 : null)} data-rozie-s-d5dcab4c>{#if colsWindowed()}<td class="rdt-col-spacer" aria-hidden="true" style={rozieStyle('width:' + colPadLeft() + 'px;padding:0;border:0')} data-rozie-s-d5dcab4c></td>{/if}{#each windowedCells(wr.row) as cell (cell.id)}<td class={["rdt-td", { 'rdt-select-td': isSelectColumn(cell.column.id), 'rdt-expander-td': isExpanderColumn(cell.column.id), 'rdt-in-range': inRange(wr.vi.index, colIndexOf(wr.row, cell)), 'rdt-cell-active': isActiveCell(String(wr.vi.index), colIndexOf(wr.row, cell)) }]} role={rozieAttr(cellRole())} data-col={rozieAttr(cell.column.id)} data-grid-cell="" data-row={rozieAttr(wr.vi.index)} data-col-index={rozieAttr(colIndexOf(wr.row, cell))} tabindex={rozieAttr(cellTabindex(String(wr.vi.index), colIndexOf(wr.row, cell)))} style={rozieStyle(bodyCellStyle(wr.row, cell.column.id))} aria-invalid={rozieAttr(cellAriaInvalid(wr.vi.index, colIndexOf(wr.row, cell)))} data-in-range={rozieAttr(inRange(wr.vi.index, colIndexOf(wr.row, cell)) ? 'true' : null)} data-agg-cell={rozieAttr(cellIsAggregated(cell) ? cell.column.id : null)} data-rozie-s-d5dcab4c>{#if isExpanderColumn(cell.column.id)}<span style="display:contents" data-rozie-s-d5dcab4c>{#if rowCanExpand(wr.row)}<button type="button" class="rdt-expander" data-expander="" aria-expanded={!!rowIsExpanded(wr.row)} aria-label={rozieAttr(rowIsExpanded(wr.row) ? 'Collapse row' : 'Expand row')} onclick={($event) => { onToggleExpand(wr.row, $event); }} data-rozie-s-d5dcab4c>{rozieDisplay(rowIsExpanded(wr.row) ? '▾' : '▸')}</button>{/if}</span>{:else if isSelectColumn(cell.column.id)}<span style="display:contents" data-rozie-s-d5dcab4c>{#if selectCell}{@render selectCell({ row: cellSlotRow(wr.row), checked: rowIsSelected(wr.row), toggle: e => onToggleRow(wr.row, e) })}{:else}<input class="rdt-select-row" type="checkbox" aria-label="Select row" checked={rowIsSelected(wr.row)} onchange={($event) => { onToggleRow(wr.row, $event); }} data-rozie-s-d5dcab4c />{/if}</span>{:else if cellIsGrouped(cell)}<span style="display:contents" data-rozie-s-d5dcab4c><button type="button" class="rdt-expander rdt-group-toggle" data-expander="" aria-expanded={!!rowIsExpanded(wr.row)} aria-label={rozieAttr(rowIsExpanded(wr.row) ? 'Collapse group' : 'Expand group')} onclick={($event) => { onToggleExpand(wr.row, $event); }} data-rozie-s-d5dcab4c>{rozieDisplay(rowIsExpanded(wr.row) ? '▾' : '▸')}</button><span class="rdt-group-value" data-rozie-s-d5dcab4c>{#if (snippets?.[`cell-${cell.column.id}`])}{@render (snippets?.[`cell-${cell.column.id}`])({ columnId: cell.column.id, column: cell.column, row: cellSlotRow(wr.row), value: cell.getValue() })}{:else}{#if cell$$slot}{@render cell$$slot({ columnId: cell.column.id, column: cell.column, row: cellSlotRow(wr.row), value: cell.getValue() })}{:else}{rozieDisplay(cell.getValue())}{/if}{/if}</span><span class="rdt-group-count" data-rozie-s-d5dcab4c>{rozieDisplay('(' + groupSubRowCount(wr.row) + ')')}</span></span>{:else if isEditing(wr.vi.index, colIndexOf(wr.row, cell))}<span style="display:contents" data-rozie-s-d5dcab4c>{#if editorTypeOf(cell.column.id) === 'number'}<input class="rdt-cell-editor" type="number" data-editing-cell="" data-builtin-editor="" value={editorValueFor(cell.column.id)} oninput={($event) => { onCellEditorInput(cell.column.id, $event); }} onkeydown={($event) => { onEditorKeyDown($event); }} onblur={($event) => { onEditorBlur($event); }} data-rozie-s-d5dcab4c />{:else if editorTypeOf(cell.column.id) === 'select'}<select class="rdt-cell-editor" data-editing-cell="" data-builtin-editor="" value={rozieAttr(editorValueFor(cell.column.id))} onchange={($event) => { onCellEditorInput(cell.column.id, $event); }} onkeydown={($event) => { onEditorKeyDown($event); }} onblur={($event) => { onEditorBlur($event); }} data-rozie-s-d5dcab4c>{#each editorOptionsOf(cell.column.id) as opt (opt.value)}<option value={rozieAttr(opt.value)} data-rozie-s-d5dcab4c>{rozieDisplay(opt.label)}</option>{/each}</select>{:else if editorTypeOf(cell.column.id) === 'checkbox'}<input class="rdt-cell-editor" type="checkbox" data-editing-cell="" data-builtin-editor="" checked={editorCheckedFor(cell.column.id)} onchange={($event) => { onCellEditorCheckbox(cell.column.id, $event); }} onkeydown={($event) => { onEditorKeyDown($event); }} onblur={($event) => { onEditorBlur($event); }} data-rozie-s-d5dcab4c />{:else if editorTypeOf(cell.column.id) === 'custom'}<span style="display:contents" data-rozie-s-d5dcab4c>{#if (snippets?.[`editor-${cell.column.id}`])}{@render (snippets?.[`editor-${cell.column.id}`])({ columnId: cell.column.id, column: cell.column, row: cellSlotRow(wr.row), value: editorValueFor(cell.column.id), commit: editorCommitFor(cell.column.id), cancel: editorCancelFor(), autofocus: editorAutofocusFor(cell.column.id, wr.vi.index) })}{:else}{#if editor}{@render editor({ columnId: cell.column.id, column: cell.column, row: cellSlotRow(wr.row), value: editorValueFor(cell.column.id), commit: editorCommitFor(cell.column.id), cancel: editorCancelFor(), autofocus: editorAutofocusFor(cell.column.id, wr.vi.index) })}{:else}<input class="rdt-cell-editor" type="text" data-editing-cell="" data-builtin-editor="" value={editorValueFor(cell.column.id)} oninput={($event) => { onCellEditorInput(cell.column.id, $event); }} onkeydown={($event) => { onEditorKeyDown($event); }} onblur={($event) => { onEditorBlur($event); }} data-rozie-s-d5dcab4c />{/if}{/if}</span>{:else}<input class="rdt-cell-editor" type="text" data-editing-cell="" data-builtin-editor="" value={editorValueFor(cell.column.id)} oninput={($event) => { onCellEditorInput(cell.column.id, $event); }} onkeydown={($event) => { onEditorKeyDown($event); }} onblur={($event) => { onEditorBlur($event); }} data-rozie-s-d5dcab4c />{/if}</span>{:else if cellIsPlaceholder(cell)}<span style="display:contents" data-rozie-s-d5dcab4c></span>{:else}<span class="rdt-cell-value" data-rozie-s-d5dcab4c>{#if (snippets?.[`cell-${cell.column.id}`])}{@render (snippets?.[`cell-${cell.column.id}`])({ columnId: cell.column.id, column: cell.column, row: cellSlotRow(wr.row), value: cell.getValue() })}{:else}{#if cell$$slot}{@render cell$$slot({ columnId: cell.column.id, column: cell.column, row: cellSlotRow(wr.row), value: cell.getValue() })}{:else}{rozieDisplay(cell.getValue())}{/if}{/if}</span>{/if}{#if isFillHandleCell(wr.vi.index, colIndexOf(wr.row, cell))}<span class="rdt-fill-handle" data-fill-handle="" data-testid="fill-handle" aria-hidden="true" onpointerdown={($event) => { onFillHandlePointerDown($event); }} data-rozie-s-d5dcab4c></span>{/if}</td>{/each}{#if colsWindowed()}<td class="rdt-col-spacer" aria-hidden="true" style={rozieStyle('width:' + colPadRight() + 'px;padding:0;border:0')} data-rozie-s-d5dcab4c></td>{/if}</tr>{#if rowShowsDetail(wr.row)}<tr class="rdt-detail-row" role="row" data-detail-row={rozieAttr(wr.row.id)} data-rozie-s-d5dcab4c><td class="rdt-detail-cell" colspan={rozieAttr(windowedColSpan())} data-rozie-s-d5dcab4c>{@render detail?.({ row: wr.row.original })}</td></tr>{/if}{/each}<tr class="rdt-spacer" aria-hidden="true" data-rozie-s-d5dcab4c><td colspan={rozieAttr(windowedColSpan())} style={rozieStyle('height:' + padBottom() + 'px;padding:0;border:0')} data-rozie-s-d5dcab4c></td></tr></tbody></table></div>{:else}<table class={["rozie-data-table", { 'rdt-sticky': stickyHeader }]} role={rozieAttr(tableRole())} aria-rowcount={rozieAttr(gridAriaRowCount())} onkeydown={($event) => { onGridKeyDown($event); }} onfocusin={($event) => { syncActiveFromEvent($event); }} onfocusout={($event) => { onGridFocusOut($event); }} onmousedown={($event) => { onGridMouseDown($event); }} ondblclick={($event) => { onGridDblClick($event); }} onclick={($event) => { onGridClick($event); }} data-rozie-s-d5dcab4c><thead class="rdt-thead" role="rowgroup" data-rozie-s-d5dcab4c>{#each headerGroups as hg, hgLevel (hg.id)}<tr class="rdt-tr" role="row" aria-rowindex={rozieAttr(hgLevel + 1)} data-rozie-s-d5dcab4c>{#each hg.headers as header (header.id)}<th class={["rdt-th", { 'rdt-select-th': isSelectColumn(header.column.id), 'rdt-expander-th': isExpanderColumn(header.column.id), 'rdt-th-resizing': columnIsResizing(header.column.id), 'rdt-cell-active': isActiveCell('__header', headerColIndexOf(hg, header), hgLevel) }]} role="columnheader" data-col={rozieAttr(header.column.id)} data-grid-cell="" data-row="__header" data-header-level={rozieAttr(hgLevel)} colspan={rozieAttr(header.colSpan > 1 ? header.colSpan : null)} data-col-index={rozieAttr(headerColIndexOf(hg, header))} tabindex={rozieAttr(cellTabindex('__header', headerColIndexOf(hg, header), hgLevel))} aria-sort={rozieAttr(ariaSortFor(header.column.id))} style={rozieStyle(thStyle(header))} data-rozie-s-d5dcab4c>{#if isSelectColumn(header.column.id)}<span style="display:contents" data-rozie-s-d5dcab4c>{#if selectAll}{@render selectAll({ checked: isAllRowsSelected(), indeterminate: isSomeRowsSelected(), toggle: onToggleAllRows })}{:else}{#if selectionMode === 'multiple'}<input class="rdt-select-all" type="checkbox" aria-label="Select all rows" checked={isAllRowsSelected()} onchange={($event) => { onToggleAllRows($event); }} data-rozie-s-d5dcab4c />{/if}{/if}</span>{:else if isExpanderColumn(header.column.id)}<span style="display:contents" data-rozie-s-d5dcab4c></span>{:else}<span style="display:contents" data-rozie-s-d5dcab4c>{#if header.column.getCanSort && header.column.getCanSort()}<button type="button" class="rdt-sort-btn" onclick={($event) => { onHeaderSort(header.column.id, $event); }} data-rozie-s-d5dcab4c><span class="rdt-header-label" data-rozie-s-d5dcab4c>{#if (snippets?.[`colHeader-${header.column.id}`])}{@render (snippets?.[`colHeader-${header.column.id}`])({ columnId: header.column.id, column: header.column, label: headerLabel(header.column.id) })}{:else}{#if colHeader}{@render colHeader({ columnId: header.column.id, column: header.column, label: headerLabel(header.column.id) })}{:else}{rozieDisplay(headerLabel(header.column.id))}{/if}{/if}</span><span class="rdt-sort-ind" aria-hidden="true" data-rozie-s-d5dcab4c>{rozieDisplay(sortIndicator(header.column.id))}</span></button>{:else}<span style="display:contents" data-rozie-s-d5dcab4c><span class="rdt-header-label" data-rozie-s-d5dcab4c>{#if (snippets?.[`colHeader-${header.column.id}`])}{@render (snippets?.[`colHeader-${header.column.id}`])({ columnId: header.column.id, column: header.column, label: headerLabel(header.column.id) })}{:else}{#if colHeader}{@render colHeader({ columnId: header.column.id, column: header.column, label: headerLabel(header.column.id) })}{:else}{rozieDisplay(headerLabel(header.column.id))}{/if}{/if}</span></span>{/if}<Popover trigger="click" placement="bottom-end" strategy="fixed" offset={4} data-rozie-s-d5dcab4c>{#snippet anchor()}<button type="button" class="rdt-col-menu-trigger" aria-label={rozieAttr('Column options for ' + headerLabel(header.column.id))} data-rozie-s-d5dcab4c>⋯</button>{/snippet}<div class="rdt-col-menu" role="menu" data-rozie-s-d5dcab4c><button type="button" role="menuitem" class="rdt-col-menu-item" aria-pressed={columnPinSide(header.column.id) === 'left'} onclick={($event) => { onPinColumn(header.column.id, 'left', $event); }} data-rozie-s-d5dcab4c>Pin left</button><button type="button" role="menuitem" class="rdt-col-menu-item" aria-pressed={columnPinSide(header.column.id) === 'right'} onclick={($event) => { onPinColumn(header.column.id, 'right', $event); }} data-rozie-s-d5dcab4c>Pin right</button><button type="button" role="menuitem" class="rdt-col-menu-item" aria-pressed={!columnPinSide(header.column.id)} onclick={($event) => { onPinColumn(header.column.id, false, $event); }} data-rozie-s-d5dcab4c>Unpin</button><hr class="rdt-col-menu-sep" data-rozie-s-d5dcab4c /><button type="button" role="menuitem" class="rdt-col-menu-item" onclick={($event) => { onHideColumn(header.column.id, $event); }} data-rozie-s-d5dcab4c>Hide column</button></div></Popover><button type="button" class="rdt-resize-handle" aria-label={rozieAttr('Resize ' + headerLabel(header.column.id))} onpointerdown={($event) => { onResizeStart(header.column.id, $event); }} ontouchstart={($event) => { onResizeStart(header.column.id, $event); }} data-rozie-s-d5dcab4c><span class="rdt-resize-grip" aria-hidden="true" data-rozie-s-d5dcab4c></span></button></span>{/if}</th>{/each}</tr>{/each}{#if hasAnyFilterableColumn()}<tr class="rdt-filter-row" data-rozie-s-d5dcab4c>{#each headerGroups[headerGroups.length - 1].headers as header (header.id)}<th class="rdt-filter-cell" role="presentation" style={rozieStyle(pinStyle(header.column.id))} data-rozie-s-d5dcab4c>{#if isSelectColumn(header.column.id)}<span style="display:contents" data-rozie-s-d5dcab4c></span>{:else if isExpanderColumn(header.column.id)}<span style="display:contents" data-rozie-s-d5dcab4c></span>{:else}<span style="display:contents" data-rozie-s-d5dcab4c>{#if columnIsFilterable(header.column.id)}<span style="display:contents" data-rozie-s-d5dcab4c>{#if (snippets?.[`filter-${header.column.id}`])}{@render (snippets?.[`filter-${header.column.id}`])({ columnId: header.column.id, value: columnFilterValue(header.column.id), uniqueValues: getFacetedUniqueValues(header.column.id), minMax: getFacetedMinMaxValues(header.column.id), setFilter: setColumnFilter })}{:else}{#if filter}{@render filter({ columnId: header.column.id, value: columnFilterValue(header.column.id), uniqueValues: getFacetedUniqueValues(header.column.id), minMax: getFacetedMinMaxValues(header.column.id), setFilter: setColumnFilter })}{:else}<input class="rdt-col-filter" type="text" aria-label={rozieAttr('Filter ' + headerLabel(header.column.id))} value={columnFilterValue(header.column.id)} oninput={($event) => { onColumnFilterInput(header.column.id, $event); }} onclick={($event) => { stopEvent($event); }} data-rozie-s-d5dcab4c />{/if}{/if}</span>{/if}</span>{/if}</th>{/each}</tr>{/if}</thead><tbody class="rdt-tbody" role="rowgroup" data-rozie-s-d5dcab4c>{#each rows as row (row.id)}<tr class={["rdt-tr", { 'rdt-group-header': rowIsGrouped(row) }]} role="row" data-depth={rozieAttr(row.depth)} aria-rowindex={rozieAttr(bodyAriaRowIndex(row))} data-group-header={rozieAttr(rowIsGrouped(row) ? row.id : null)} data-group-leaf={rozieAttr(groupingActive() && !rowIsGrouped(row) ? row.id : null)} aria-expanded={rozieAttr(rowIsGrouped(row) ? !!rowIsExpanded(row) : null)} aria-selected={rozieAttr(selectionMode !== 'none' ? !!rowIsSelected(row) : null)} aria-level={rozieAttr(groupingActive() ? row.depth + 1 : null)} data-rozie-s-d5dcab4c>{#each visibleCellsFor(row) as cell (cell.id)}<td class={["rdt-td", { 'rdt-select-td': isSelectColumn(cell.column.id), 'rdt-expander-td': isExpanderColumn(cell.column.id), 'rdt-in-range': inRange(rowIndexOf(row), colIndexOf(row, cell)), 'rdt-cell-active': isActiveCell(String(rowIndexOf(row)), colIndexOf(row, cell)) }]} role={rozieAttr(cellRole())} data-col={rozieAttr(cell.column.id)} data-grid-cell="" data-row={rozieAttr(rowIndexOf(row))} data-col-index={rozieAttr(colIndexOf(row, cell))} tabindex={rozieAttr(cellTabindex(String(rowIndexOf(row)), colIndexOf(row, cell)))} style={rozieStyle(bodyCellStyle(row, cell.column.id))} aria-invalid={rozieAttr(cellAriaInvalid(rowIndexOf(row), colIndexOf(row, cell)))} data-in-range={rozieAttr(inRange(rowIndexOf(row), colIndexOf(row, cell)) ? 'true' : null)} data-agg-cell={rozieAttr(cellIsAggregated(cell) ? cell.column.id : null)} data-rozie-s-d5dcab4c>{#if isExpanderColumn(cell.column.id)}<span style="display:contents" data-rozie-s-d5dcab4c>{#if rowCanExpand(row)}<button type="button" class="rdt-expander" data-expander="" aria-expanded={!!rowIsExpanded(row)} aria-label={rozieAttr(rowIsExpanded(row) ? 'Collapse row' : 'Expand row')} onclick={($event) => { onToggleExpand(row, $event); }} data-rozie-s-d5dcab4c>{rozieDisplay(rowIsExpanded(row) ? '▾' : '▸')}</button>{/if}</span>{:else if isSelectColumn(cell.column.id)}<span style="display:contents" data-rozie-s-d5dcab4c>{#if selectCell}{@render selectCell({ row: cellSlotRow(row), checked: rowIsSelected(row), toggle: e => onToggleRow(row, e) })}{:else}<input class="rdt-select-row" type="checkbox" aria-label="Select row" checked={rowIsSelected(row)} onchange={($event) => { onToggleRow(row, $event); }} data-rozie-s-d5dcab4c />{/if}</span>{:else if cellIsGrouped(cell)}<span style="display:contents" data-rozie-s-d5dcab4c><button type="button" class="rdt-expander rdt-group-toggle" data-expander="" aria-expanded={!!rowIsExpanded(row)} aria-label={rozieAttr(rowIsExpanded(row) ? 'Collapse group' : 'Expand group')} onclick={($event) => { onToggleExpand(row, $event); }} data-rozie-s-d5dcab4c>{rozieDisplay(rowIsExpanded(row) ? '▾' : '▸')}</button><span class="rdt-group-value" data-rozie-s-d5dcab4c>{#if (snippets?.[`cell-${cell.column.id}`])}{@render (snippets?.[`cell-${cell.column.id}`])({ columnId: cell.column.id, column: cell.column, row: cellSlotRow(row), value: cell.getValue() })}{:else}{#if cell$$slot}{@render cell$$slot({ columnId: cell.column.id, column: cell.column, row: cellSlotRow(row), value: cell.getValue() })}{:else}{rozieDisplay(cell.getValue())}{/if}{/if}</span><span class="rdt-group-count" data-rozie-s-d5dcab4c>{rozieDisplay('(' + groupSubRowCount(row) + ')')}</span></span>{:else if isEditing(rowIndexOf(row), colIndexOf(row, cell))}<span style="display:contents" data-rozie-s-d5dcab4c>{#if editorTypeOf(cell.column.id) === 'number'}<input class="rdt-cell-editor" type="number" data-editing-cell="" data-builtin-editor="" value={editorValueFor(cell.column.id)} oninput={($event) => { onCellEditorInput(cell.column.id, $event); }} onkeydown={($event) => { onEditorKeyDown($event); }} onblur={($event) => { onEditorBlur($event); }} data-rozie-s-d5dcab4c />{:else if editorTypeOf(cell.column.id) === 'select'}<select class="rdt-cell-editor" data-editing-cell="" data-builtin-editor="" value={rozieAttr(editorValueFor(cell.column.id))} onchange={($event) => { onCellEditorInput(cell.column.id, $event); }} onkeydown={($event) => { onEditorKeyDown($event); }} onblur={($event) => { onEditorBlur($event); }} data-rozie-s-d5dcab4c>{#each editorOptionsOf(cell.column.id) as opt (opt.value)}<option value={rozieAttr(opt.value)} data-rozie-s-d5dcab4c>{rozieDisplay(opt.label)}</option>{/each}</select>{:else if editorTypeOf(cell.column.id) === 'checkbox'}<input class="rdt-cell-editor" type="checkbox" data-editing-cell="" data-builtin-editor="" checked={editorCheckedFor(cell.column.id)} onchange={($event) => { onCellEditorCheckbox(cell.column.id, $event); }} onkeydown={($event) => { onEditorKeyDown($event); }} onblur={($event) => { onEditorBlur($event); }} data-rozie-s-d5dcab4c />{:else if editorTypeOf(cell.column.id) === 'custom'}<span style="display:contents" data-rozie-s-d5dcab4c>{#if (snippets?.[`editor-${cell.column.id}`])}{@render (snippets?.[`editor-${cell.column.id}`])({ columnId: cell.column.id, column: cell.column, row: cellSlotRow(row), value: editorValueFor(cell.column.id), commit: editorCommitFor(cell.column.id), cancel: editorCancelFor(), autofocus: editorAutofocusFor(cell.column.id, rowIndexOf(row)) })}{:else}{#if editor}{@render editor({ columnId: cell.column.id, column: cell.column, row: cellSlotRow(row), value: editorValueFor(cell.column.id), commit: editorCommitFor(cell.column.id), cancel: editorCancelFor(), autofocus: editorAutofocusFor(cell.column.id, rowIndexOf(row)) })}{:else}<input class="rdt-cell-editor" type="text" data-editing-cell="" data-builtin-editor="" value={editorValueFor(cell.column.id)} oninput={($event) => { onCellEditorInput(cell.column.id, $event); }} onkeydown={($event) => { onEditorKeyDown($event); }} onblur={($event) => { onEditorBlur($event); }} data-rozie-s-d5dcab4c />{/if}{/if}</span>{:else}<input class="rdt-cell-editor" type="text" data-editing-cell="" data-builtin-editor="" value={editorValueFor(cell.column.id)} oninput={($event) => { onCellEditorInput(cell.column.id, $event); }} onkeydown={($event) => { onEditorKeyDown($event); }} onblur={($event) => { onEditorBlur($event); }} data-rozie-s-d5dcab4c />{/if}</span>{:else if cellIsPlaceholder(cell)}<span style="display:contents" data-rozie-s-d5dcab4c></span>{:else}<span class="rdt-cell-value" data-rozie-s-d5dcab4c>{#if (snippets?.[`cell-${cell.column.id}`])}{@render (snippets?.[`cell-${cell.column.id}`])({ columnId: cell.column.id, column: cell.column, row: cellSlotRow(row), value: cell.getValue() })}{:else}{#if cell$$slot}{@render cell$$slot({ columnId: cell.column.id, column: cell.column, row: cellSlotRow(row), value: cell.getValue() })}{:else}{rozieDisplay(cell.getValue())}{/if}{/if}</span>{/if}{#if isFillHandleCell(rowIndexOf(row), colIndexOf(row, cell))}<span class="rdt-fill-handle" data-fill-handle="" data-testid="fill-handle" aria-hidden="true" onpointerdown={($event) => { onFillHandlePointerDown($event); }} data-rozie-s-d5dcab4c></span>{/if}</td>{/each}</tr>{#if rowShowsDetail(row)}<tr class="rdt-detail-row" role="row" data-detail-row={rozieAttr(row.id)} data-rozie-s-d5dcab4c><td class="rdt-detail-cell" colspan={rozieAttr(visibleColCount())} data-rozie-s-d5dcab4c>{@render detail?.({ row: row.original })}</td></tr>{/if}{/each}</tbody></table>{/if}{#if !rowsWindowed()}<div class="rdt-pagination" role="group" aria-label="Pagination" data-rozie-s-d5dcab4c><button type="button" class="rdt-page-btn rdt-page-prev" disabled={!canPrevPage()} onclick={($event) => { onPrevPage(); }} data-rozie-s-d5dcab4c>Prev</button><span class="rdt-page-status" aria-live="polite" data-rozie-s-d5dcab4c>{rozieDisplay('Page ' + (pageIndex() + 1) + ' of ' + displayPageCount())}</span><button type="button" class="rdt-page-btn rdt-page-next" disabled={!canNextPage()} onclick={($event) => { onNextPage(); }} data-rozie-s-d5dcab4c>Next</button><select class="rdt-page-size" aria-label="Rows per page" value={rozieAttr(pageSize())} onchange={($event) => { onPageSizeChange($event); }} data-rozie-s-d5dcab4c><option value={10} data-rozie-s-d5dcab4c>10</option><option value={25} data-rozie-s-d5dcab4c>25</option><option value={50} data-rozie-s-d5dcab4c>50</option><option value={100} data-rozie-s-d5dcab4c>100</option></select></div>{/if}</div>
