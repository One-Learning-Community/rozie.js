import { Component, ContentChild, DestroyRef, ElementRef, TemplateRef, ViewEncapsulation, computed, contentChildren, effect, forwardRef, inject, input, model, output, signal, untracked, viewChild } from '@angular/core';
...
@Component({
  ...
  template: `
    ...
    {{ rozieDisplay(headerLabel(wh.header.column.id)) }}
                  ...
                  <button type="button" class="rdt-col-menu-trigger" [attr.aria-label]="rozieAttr('Column options for ' + headerLabel(wh.header.column.id))">⋯</button>
                </ng-template><ng-template #defaultSlot><div class="rdt-col-menu" role="menu">
                  <button type="button" role="menuitem" class="rdt-col-menu-item" [attr.aria-pressed]="columnPinSide(wh.header.column.id) === 'left'" (click)="onPinColumn(wh.header.column.id, 'left', $event)">Pin left</button>
                  <button type="button" role="menuitem" class="rdt-col-menu-item" [attr.aria-pressed]="columnPinSide(wh.header.column.id) === 'right'" (click)="onPinColumn(wh.header.column.id, 'right', $event)">Pin right</button>
                  <button type="button" role="menuitem" class="rdt-col-menu-item" [attr.aria-pressed]="!columnPinSide(wh.header.column.id)" (click)="onPinColumn(wh.header.column.id, false, $event)">Unpin</button>
              ...
              <button type="button" class="rdt-resize-handle" [attr.aria-label]="rozieAttr('Resize ' + headerLabel(wh.header.column.id))" (pointerdown)="onResizeStart(wh.header.column.id, $event)" (touchstart)="onResizeStart(wh.header.column.id, $event)"><span class="rdt-resize-grip" aria-hidden="true"></span></button>
            </span>
    }</th>
    }
          
          @if (colsWindowed()) {
    <th class="rdt-col-spacer" aria-hidden="true" [attr.style]="'width:' + colPadRight() + 'px;padding:0;border:0'"></th>
    }</tr>
    }
    ...
    <th class="rdt-col-spacer" aria-hidden="true" [attr.style]="'width:' + colPadRight() + 'px;padding:0;border:0'"></th>
      ...
      <tbody class="rdt-tbody" role="rowgroup">
        ...
        <tr class="rdt-spacer" aria-hidden="true">
          <td [attr.colspan]="rozieAttr(windowedColSpan())" [attr.style]="'height:' + padTop() + 'px;padding:0;border:0'"></td>
        </tr>
    ...
    <span class="rdt-fill-handle" data-fill-handle="" data-testid="fill-handle" aria-hidden="true" (pointerdown)="onFillHandlePointerDown($event)"></span>
          ...
          @if (colsWindowed()) {
    <td class="rdt-col-spacer" aria-hidden="true" [attr.style]="'width:' + colPadRight() + 'px;padding:0;border:0'"></td>
    }</tr>
        
        @if (rowShowsDetail(wr.row)) {
    <tr class="rdt-detail-row" role="row" [attr.data-detail-row]="rozieAttr(wr.row.id)">
          <td class="rdt-detail-cell" [attr.colspan]="rozieAttr(windowedColSpan())">
        ...
        <tr class="rdt-spacer" aria-hidden="true">
          <td [attr.colspan]="rozieAttr(windowedColSpan())" [attr.style]="'height:' + padBottom() + 'px;padding:0;border:0'"></td>
        </tr>
      </tbody>
    </table>
    </div>
  ...
  styles: [`
    ...
    .rozie-data-table-wrap ::ng-deep .rdt-group-token {
      display: inline-flex;
      align-items: center;
    ...
    .rozie-data-table-wrap .rdt-filter-row ::ng-deep .rdt-col-filter {
...
export class DataTable {
   ...
   */
  getSubRows = input<((...args: any[]) => any) | null>(null);
  /**
   ...
   */
  virtual = input<boolean | string>(false);
  /**
   ...
   */
  autoMeasure = input<boolean>(false);
  /**
  ...
  seedColumnPinning = (): void => {
    ...
    const live = this.columnPinning() != null ? this.columnPinning() : this.columnPinningDefault();
    const isRealPin = (id: string) => id !== this.SELECT_COL_ID && id !== this.EXPANDER_COL_ID;
    ...
    this.columnPinningDefault.set(seeded);
    this.columnPinning.set(seeded);
  };
  ...
  writeData = (next: any) => {
    ...
    this.programmatic++;
    this.dataDefault.set(fresh); // fresh raw array only (never in-place, never a proxy)
    this.data.set(fresh); // two-way emit if bound (no-op-diff if not)
    this.programmatic--;
  };
  ...
  recordSnapshot = (current: any) => {
    const __undoLimit = this.undoLimit();
    this.undoStack.push(current);
    const limit = __undoLimit != null ? __undoLimit : 100;
  ...
  columnSizingInfoSync: any = null;
  onColumnSizingInfoChangeCb = (updater: any) => {
    const base = this.columnSizingInfoSync != null ? this.columnSizingInfoSync : this.columnSizingInfo();
    const next = applyUpdater(updater, base);
    if (next == null) return;
    this.columnSizingInfoSync = next;
    this.columnSizingInfo.set(next);
  };
  ...
  forcedColumns = (): number[] => {
    ...
    if (!this.activeIsHeader() && __activeColIndex >= 0 && out.indexOf(__activeColIndex) === -1) {
      out.push(__activeColIndex);
    }
    ...
    if (this.editingRow() >= 0 && __editingCol >= 0 && out.indexOf(__editingCol) === -1) {
      out.push(__editingCol);
    }
    return out;
  };
  ...
  rowIndexIsGrouped = (rowIndex: any) => this.rowIsGrouped((this.rows() || [])[rowIndex]);
  ...
  syncActiveFromEvent = (e: any) => {
    ...
    const prevRow = this.activeRow();
    const prevCol = this.activeColIndex();
    const prevLevel = this.activeHeaderLevel();
    ...
    if (isHeader) {
      ...
      this.activeHeaderLevel.set(movedLevel);
    } else {
  ...
  recoverGridFocus = (rowKey: any, col: any, level: any, guardMoved: any = false) => {
  ...
  onEditorBlur = (e: any) => {
    ...
    if (this.inRowEdit()) {
      ...
      if (rowNextRow != null && rowNextRow === String(this.editingRowIndex())) return;
      this.commitRow();
      return;
    }
  ...
  protected get __style() {
      ...
      return this.rowsWindowed() && __maxHeight ? 'max-height:' + __maxHeight + ';overflow:auto;--rozie-data-table-max-height:' + __maxHeight : 'overflow:auto';
    }

  private _selectCell_ctx = (wr: any, cell: any) => ({ $implicit: { row: this.cellSlotRow(wr.row), checked: this.rowIsSelected(wr.row), toggle: e => this.onToggleRow(wr.row, e) }, row: this.cellSlotRow(wr.row), checked: this.rowIsSelected(wr.row), toggle: e => this.onToggleRow(wr.row, e) });

  private _selectCell_ctx_1 = (row: any, cell: any) => ({ $implicit: { row: this.cellSlotRow(row), checked: this.rowIsSelected(row), toggle: e => this.onToggleRow(row, e) }, row: this.cellSlotRow(row), checked: this.rowIsSelected(row), toggle: e => this.onToggleRow(row, e) });
