...
const DataTable = forwardRef<DataTableHandle, DataTableProps>(function DataTable(_props: DataTableProps, ref): JSX.Element {
  ...
  const pinSeedApplied = useRef(false);
  ...
  const seedColumnPinning = useCallback((): void => {
  ...
  }, [columnDefs, columnPinning, columnPinningDefault, setColumnPinning]);
  ...
  const onColumnSizingInfoChangeCb = useCallback((updater: any) => {
    const base = columnSizingInfoSync.current != null ? columnSizingInfoSync.current : columnSizingInfo;
  ...
  const bumpWindowVer = useCallback((): void => {
