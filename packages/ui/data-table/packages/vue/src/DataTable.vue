...
<table :class="['rozie-data-table', { 'rdt-sticky': props.stickyHeader, 'rdt-col-windowed': colsWindowed() }]" :role="tableRole()" :aria-rowcount="(gridAriaRowCount()) ?? undefined" @keydown="onGridKeyDown($event)" @focusin="syncActiveFromEvent($event)" @focusout="onGridFocusOut($event)" @mousedown="onGridMouseDown($event)" @dblclick="onGridDblClick($event)" @click="onGridClick($event)">
        ...
        <span v-if="isSelectColumn(wh.header.column.id)" style="display:contents">
          ...
          <button v-if="wh.header.column.getCanSort && wh.header.column.getCanSort()" type="button" class="rdt-sort-btn" @click="onHeaderSort(wh.header.column.id, $event)">
            ...
            <span class="rdt-header-label">
              ...
              <button type="button" role="menuitem" class="rdt-col-menu-item" :aria-pressed="columnPinSide(wh.header.column.id) === 'left'" @click="onPinColumn(wh.header.column.id, 'left', $event)">Pin left</button>
              <button type="button" role="menuitem" class="rdt-col-menu-item" :aria-pressed="columnPinSide(wh.header.column.id) === 'right'" @click="onPinColumn(wh.header.column.id, 'right', $event)">Pin right</button>
              <button type="button" role="menuitem" class="rdt-col-menu-item" :aria-pressed="!columnPinSide(wh.header.column.id)" @click="onPinColumn(wh.header.column.id, false, $event)">Unpin</button>
              <hr class="rdt-col-menu-sep" />
              <button type="button" role="menuitem" class="rdt-col-menu-item" @click="onHideColumn(wh.header.column.id, $event)">Hide column</button>
            </div></Popover>
          <button type="button" class="rdt-resize-handle" :aria-label="'Resize ' + headerLabel(wh.header.column.id)" @pointerdown="onResizeStart(wh.header.column.id, $event)" @touchstart="onResizeStart(wh.header.column.id, $event)"><span class="rdt-resize-grip" aria-hidden="true"></span></button>
        </span></th>
      
      <th v-if="colsWindowed()" class="rdt-col-spacer" aria-hidden="true" :style="'width:' + colPadRight() + 'px;padding:0;border:0'"></th></tr>
  ...
  <tbody class="rdt-tbody" role="rowgroup">
    ...
    <tr class="rdt-spacer" aria-hidden="true">
      <td :colspan="(windowedColSpan()) ?? undefined" :style="'height:' + padTop() + 'px;padding:0;border:0'"></td>
    </tr>
    ...
    <tr :class="['rdt-tr', { 'rdt-group-header': rowIsGrouped(wr.row), 'rdt-row-pinned': wr.pinned }]" role="row" :data-row="wr.vi.index" :aria-rowindex="headerRowCount() + wr.vi.index + 1" :data-index="wr.vi.index" :data-pinned="wr.pinned ? 'true' : undefined" :data-depth="wr.row.depth" :data-group-header="rowIsGrouped(wr.row) ? wr.row.id : undefined" :data-group-leaf="groupingActive() && !rowIsGrouped(wr.row) ? wr.row.id : undefined" :aria-expanded="(rowIsGrouped(wr.row) ? !!rowIsExpanded(wr.row) : undefined) ?? undefined" :aria-selected="(props.selectionMode !== 'none' ? !!rowIsSelected(wr.row) : undefined) ?? undefined" :aria-level="(groupingActive() ? wr.row.depth + 1 : undefined) ?? undefined">
      ...
      <td v-if="colsWindowed()" class="rdt-col-spacer" aria-hidden="true" :style="'width:' + colPadLeft() + 'px;padding:0;border:0'"></td><td v-for="cell in windowedCells(wr.row)" :key="cell.id" :class="['rdt-td', { 'rdt-select-td': isSelectColumn(cell.column.id), 'rdt-expander-td': isExpanderColumn(cell.column.id), 'rdt-in-range': inRange(wr.vi.index, colIndexOf(wr.row, cell)), 'rdt-cell-active': isActiveCell(String(wr.vi.index), colIndexOf(wr.row, cell)) }]" :role="cellRole()" :data-col="cell.column.id" data-grid-cell="" :data-row="wr.vi.index" :data-col-index="colIndexOf(wr.row, cell)" :tabindex="(cellTabindex(String(wr.vi.index), colIndexOf(wr.row, cell))) ?? undefined" :style="bodyCellStyle(wr.row, cell.column.id)" :aria-invalid="(cellAriaInvalid(wr.vi.index, colIndexOf(wr.row, cell))) ?? undefined" :data-in-range="inRange(wr.vi.index, colIndexOf(wr.row, cell)) ? 'true' : undefined" :data-agg-cell="cellIsAggregated(cell) ? cell.column.id : undefined">
        ...
        <span v-if="isExpanderColumn(cell.column.id)" style="display:contents">
          ...
          </span><input v-else-if="editorTypeOf(cell.column.id) === 'number'" class="rdt-cell-editor" type="number" data-editing-cell="" data-builtin-editor="" :value="editorValueFor(cell.column.id)" @input="onCellEditorInput(cell.column.id, $event)" @keydown="onEditorKeyDown($event)" @blur="onEditorBlur($event)" /><select v-else-if="editorTypeOf(cell.column.id) === 'select'" class="rdt-cell-editor" data-editing-cell="" data-builtin-editor="" :value="editorValueFor(cell.column.id)" @change="onCellEditorInput(cell.column.id, $event)" @keydown="onEditorKeyDown($event)" @blur="onEditorBlur($event)">
    ...
    <tr v-if="rowShowsDetail(wr.row)" class="rdt-detail-row" role="row" :data-detail-row="wr.row.id">
      <td class="rdt-detail-cell" :colspan="(windowedColSpan()) ?? undefined">
        <slot name="detail" :row="wr.row.original"></slot>
      </td>
    </tr></template>
    
    <tr class="rdt-spacer" aria-hidden="true">
      <td :colspan="(windowedColSpan()) ?? undefined" :style="'height:' + padBottom() + 'px;padding:0;border:0'"></td>
    </tr>
  </tbody>
</table>
...
</table><div v-if="!rowsWindowed()" class="rdt-pagination" role="group" aria-label="Pagination">
...
const props = withDefaults(
  ...
  { columns: () => [], selectionMode: 'none', manual: false, rowCount: null, pageCount: null, expandable: false, getSubRows: null, groupable: false, stickyHeader: false, interactionMode: 'table', singleClickEdit: false, undoable: false, undoLimit: 100, virtual: false, estimateRowHeight: 40, autoMeasure: false, maxHeight: '' }
);
...
const editVer = ref(0);
const editFocusColId = ref<any>(null);
...
const columnDefs = () => {
  ...
  if (columnDefsCache && props.columns === columnDefsCacheColumnsRef && colReg.value === columnDefsCacheColRegRef) {
  ...
  columnDefsCacheColRegRef = colReg.value;
...
const writeData = (next: any) => {
  ...
  programmatic++;
  dataDefault.value = fresh; // fresh raw array only (never in-place, never a proxy)
  data.value = fresh; // two-way emit if bound (no-op-diff if not)
  programmatic--;
};
...
const forcedColumns = (): number[] => {
  ...
  if (!activeIsHeader.value && activeColIndex.value >= 0 && out.indexOf(activeColIndex.value) === -1) {
    out.push(activeColIndex.value);
  }
  ...
  if (editingRow.value >= 0 && editingCol.value >= 0 && out.indexOf(editingCol.value) === -1) {
    out.push(editingCol.value);
  }
  return out;
};
...
const effectiveSorting = () => sorting.value != null ? sorting.value : sortingDefault.value;
const effectiveColumnFilters = () => columnFilters.value != null ? columnFilters.value : columnFiltersDefault.value;
const effectiveGlobalFilter = () => globalFilter.value != null ? globalFilter.value : globalFilterDefault.value;
...
const moveCol = (delta: any) => {
  ...
  const count = activeIsHeader.value ? headerCountAtLevel(activeHeaderLevel.value) : visibleColCount();
  const max = count - 1;
...
const onGridKeyDown = (e: any) => {
  ...
  if (nextRow !== prevRow || nextCol !== prevCol || nextIsHeader !== prevIsHeader || nextLevel !== prevLevel) {
    ...
    emit('activecell-change', nextIsHeader ? {
      rowIndex: null,
      colIndex: nextCol,
      isHeader: true
    } : {
...
const syncActiveFromEvent = (e: any) => {
  ...
  if (isHeader) {
    ...
    activeHeaderLevel.value = movedLevel;
  } else {
...
const focusEditorWhenReady = (selectAll = true) => {
  ...
  if (editFocusColId.value != null && hasEditorSlot(editFocusColId.value)) return;
  let attempts = 0;
  const tryFocus = () => {
...
const editorAutofocusFor = (colId: any, rowIndex: any) => {
  ...
  if (editingRowIndex.value != null) {
    ...
    if (editingRow.value !== rowIndex) return false;
  }
  return editFocusColId.value != null && editFocusColId.value === colId;
};
...
const beginRowEdit = (row: any) => {
  ...
  editFocusColId.value = editable[0].colId;
  focusEditorWhenReady();
};
...
const rowEditTab = (target: any, backward: any) => {
  ...
  editFocusColId.value = editable[nextPos].colId;
  editVer.value = editVer.value + 1;
...
const onEditorBlur = (e: any) => {
  ...
  if (inRowEdit()) {
    ...
    if (rowNextRow != null && rowNextRow === String(editingRowIndex.value)) return;
    commitRow();
    return;
  }
...
.rozie-data-table.rdt-col-windowed {
  table-layout: fixed;
}
.rozie-data-table.rdt-col-windowed .rdt-th,
.rozie-data-table.rdt-col-windowed .rdt-td {
