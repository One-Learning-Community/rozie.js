...
export default function FilterNumberRange(_props: FilterNumberRangeProps): JSX.Element {
  ...
  return (
    ...
    <span style={{ display: "flex", "align-items": "center" }} data-rozie-s-97b2c090="">
      <input part="col-filter" type="number" aria-label={rozieAttr(local.columnId + ' min')} class={"rdt-col-filter"} placeholder={rozieAttr(minPlaceholder())} value={minDraft()} onInput={($event: InputEvent & { currentTarget: HTMLInputElement; target: Element }) => { onMinInput($event); }} onChange={($event: Event & { currentTarget: HTMLInputElement; target: Element }) => { applyRange(); }} data-rozie-s-97b2c090="" />
      <span data-rozie-s-97b2c090=""> - </span>
      <input part="col-filter" type="number" aria-label={rozieAttr(local.columnId + ' max')} class={"rdt-col-filter"} placeholder={rozieAttr(maxPlaceholder())} value={maxDraft()} onInput={($event: InputEvent & { currentTarget: HTMLInputElement; target: Element }) => { onMaxInput($event); }} onChange={($event: Event & { currentTarget: HTMLInputElement; target: Element }) => { applyRange(); }} data-rozie-s-97b2c090="" />
    </span>
    </>
  );
}
