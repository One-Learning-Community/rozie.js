...
// 14 sites — the 2 virtual-body + 2 non-virtual-body `#selectCell` call sites (cvo C3, 1
// occurrence apiece, unchanged) PLUS the 4 `#cell` call sites (afh) EACH DOUBLED to 2
// occurrences apiece (phase 88 plan 04, D-04b) PLUS the 2 `#editor` call sites EACH DOUBLED
// to 2 occurrences apiece (phase 88 plan 07, D-01/D-04): each seam's `<seam>-<columnId>`
// family slot nests directly around the pre-existing generic `#<seam>` slot as its inline
// fallback, and both tiers carry the identical scoped params — including
// `:row="cellSlotRow(...)"`. 4 cell sites × 2 tiers = 8, 2 editor sites × 2 tiers = 4, plus
// the unchanged 2 selectCell sites = 14. A future edit that drops or bypasses either tier at
// any site is caught immediately. `#detail` is DELIBERATELY EXCLUDED: it is already guarded
// by `rowShowsDetail`'s `!rowIsGrouped(row)` check, so a group-header row can never reach
...
describe('B2/C3 invariant: every #cell/#selectCell/#editor slot site binds :row through cellSlotRow(...)', () => {
  it('DataTable.rozie carries exactly 14 `:row="cellSlotRow(` occurrences', () => {
