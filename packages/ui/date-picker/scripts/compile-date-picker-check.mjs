/**
 * compile-date-picker-check.mjs — the live compile() collision/surface gate for
 * the DatePicker family (run BEFORE any leaf work, the otp/pagination discipline).
 *
...
if (process.exitCode) {
  console.error('\n✗ compile-date-picker-check FAILED');
} else {
  console.log(`✓ DatePicker surface OK: ${EXPECT.props.length} props (${EXPECT.models.length} model) / ${EXPECT.emits.length} emits / ${EXPECT.slots.length} slots / ${EXPECT.expose.length} expose; compile()×6 zero-error.`);
}
