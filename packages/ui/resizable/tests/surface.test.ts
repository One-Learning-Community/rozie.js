/**
 * surface.test.ts — the Resizable.rozie surface gate as a vitest test (so it
 * runs under `turbo run test`, not just scripts/compile-resizable-check.mjs).
 *
 ...
 *      Lit reserved-lifecycle all surface here as compile() errors). The expose
 *      verb is `applySize` (NOT `setSize`) precisely to dodge the `size` model's
 *      React state-setter collision (ROZ524).
 *
...
const SRC = resolve(HERE, '..', 'src', 'Resizable.rozie');
...
describe('Resizable.rozie surface gate', () => {
  ...
  it('model:true props match (size)', () => {
  ...
  it('no expose-verb collides with an emit (ROZ121) or the React size-model setter (ROZ524)', () => {
    const exposeNames = ir.expose.map((e: { name: string }) => e.name);
