/**
 * compile-resizable-check.mjs — the live compile() collision/surface gate for
 * the Resizable family (run BEFORE any leaf work, the otp/slider discipline).
 *
...
if (process.exitCode) {
  console.error('\n✗ compile-resizable-check FAILED');
} else {
  console.log(`✓ Resizable surface OK: ${EXPECT.props.length} props (${EXPECT.models.length} model) / ${EXPECT.emits.length} emits / ${EXPECT.slots.length} slots / ${EXPECT.expose.length} expose; compile()×6 zero-error.`);
}
