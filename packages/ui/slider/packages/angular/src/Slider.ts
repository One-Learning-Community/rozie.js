...
@Component({
  ...
  template: `
    ...
    <input #inputEl class="rozie-slider-input" type="range" [min]="min()" [max]="max()" [step]="step()" [value]="singleValue()" [disabled]="!!(disabled() || this.__rozieCvaDisabled())" [attr.aria-label]="rozieAttr(ariaLabel())" [attr.aria-orientation]="rozieAttr(orientation() === 'vertical' ? 'vertical' : 'horizontal')" [attr.aria-valuetext]="rozieAttr(formatValue() !== null ? display(singleValue()) : null)" (input)="onInputSingle($event)" (keydown)="onKeyDownSingle($event)" />
    }@if (range()) {
    <input #inputEl class="rozie-slider-input rozie-slider-input--lo" type="range" [min]="min()" [max]="max()" [step]="step()" [value]="rangePair()[0]" [disabled]="!!(disabled() || this.__rozieCvaDisabled())" [attr.aria-label]="rozieAttr(ariaLabel())" [attr.aria-orientation]="rozieAttr(orientation() === 'vertical' ? 'vertical' : 'horizontal')" [attr.aria-valuetext]="rozieAttr(formatValue() !== null ? display(rangePair()[0]) : null)" (input)="onInputLo($event)" (keydown)="onKeyDownRange('lo', $event)" />
    }@if (range()) {
    <input class="rozie-slider-input rozie-slider-input--hi" type="range" [min]="min()" [max]="max()" [step]="step()" [value]="rangePair()[1]" [disabled]="!!(disabled() || this.__rozieCvaDisabled())" [attr.aria-label]="rozieAttr(ariaLabel())" [attr.aria-orientation]="rozieAttr(orientation() === 'vertical' ? 'vertical' : 'horizontal')" [attr.aria-valuetext]="rozieAttr(formatValue() !== null ? display(rangePair()[1]) : null)" (input)="onInputHi($event)" (keydown)="onKeyDownRange('hi', $event)" />
    }</div>
