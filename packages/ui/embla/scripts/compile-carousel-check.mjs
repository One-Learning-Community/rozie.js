/**
 * compile-carousel-check.mjs — the live compile() collision/surface gate for the
 * Embla Carousel port (run BEFORE any leaf work, the Cropper/TipTap/Chart.js/
 * MapLibre discipline).
 *
...
if (!setEq(slotNames, EXPECT.slots)) fail(`slots mismatch: got [${slotNames.map((s) => JSON.stringify(s)).sort()}], want [${[...EXPECT.slots].map((s) => JSON.stringify(s)).sort()}]`);
...
// no exposed verb may equal an emit name, and no exposed verb may be the React
// model-setter for a model prop (`set` + Capitalized model name).
...
if (process.exitCode) {
  console.error('\n✗ compile-carousel-check FAILED');
} else {
  console.log(`✓ Carousel surface OK: ${EXPECT.props.length} props (${EXPECT.models.length} model) / ${EXPECT.emits.length} emits / ${EXPECT.slots.length} slots / ${EXPECT.expose.length} expose; compile()×6 zero-error.`);
}
