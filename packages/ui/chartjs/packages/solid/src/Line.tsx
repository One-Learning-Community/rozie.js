...
__rozieInjectStyle('Line-4270cd12', `.rozie-chart[data-rozie-s-4270cd12] {
  position: relative;
  width: 100%;
}
.rozie-chart[data-rozie-s-4270cd12] canvas[data-rozie-s-4270cd12] {
  display: block;
  width: 100% !important;
...
export default function Line(_props: LineProps): JSX.Element {
  ...
  return (
    <>
    <div class={"rozie-chart"} style={{ height: local.height + 'px', width: local.width ? local.width + 'px' : undefined }} data-rozie-s-4270cd12="">
      
      <canvas ref={(el) => { canvasElRef = el as HTMLElement; }} role="img" aria-label={local.ariaLabel} data-rozie-s-4270cd12="">{(_props.fallbackSlot ?? _props.slots?.['fallback']?.({}))}</canvas>
    </div>
