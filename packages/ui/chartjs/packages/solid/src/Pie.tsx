...
__rozieInjectStyle('Pie-d3ac8fdc', `.rozie-chart[data-rozie-s-d3ac8fdc] {
  position: relative;
  width: 100%;
}
.rozie-chart[data-rozie-s-d3ac8fdc] canvas[data-rozie-s-d3ac8fdc] {
  display: block;
  width: 100% !important;
  height: 100% !important;
}
.rozie-chart[data-rozie-s-d3ac8fdc] [data-rozie-s-d3ac8fdc]:global(.rozie-chart-tooltip) {
...
export default function Pie(_props: PieProps): JSX.Element {
  ...
  return (
    <>
    <div class={"rozie-chart"} style={{ height: local.height + 'px', width: local.width ? local.width + 'px' : undefined }} data-rozie-s-d3ac8fdc="">
      
      <canvas ref={(el) => { canvasElRef = el as HTMLElement; }} role="img" aria-label={local.ariaLabel} data-rozie-s-d3ac8fdc="">{(_props.fallbackSlot ?? _props.slots?.['fallback']?.({}))}</canvas>
    </div>
