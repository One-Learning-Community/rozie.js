...
__rozieInjectStyle('Bar-ed04b5ca', `.rozie-chart[data-rozie-s-ed04b5ca] {
  position: relative;
  width: 100%;
}
...
export default function Bar(_props: BarProps): JSX.Element {
  ...
  return (
    <>
    <div class={"rozie-chart"} style={{ height: local.height + 'px', width: local.width ? local.width + 'px' : undefined }} data-rozie-s-ed04b5ca="">
      
      <canvas ref={(el) => { canvasElRef = el as HTMLElement; }} role="img" aria-label={local.ariaLabel} data-rozie-s-ed04b5ca="">{(_props.fallbackSlot ?? _props.slots?.['fallback']?.({}))}</canvas>
    </div>
