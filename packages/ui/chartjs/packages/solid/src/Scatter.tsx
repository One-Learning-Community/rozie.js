...
__rozieInjectStyle('Scatter-e7b1d9e8', `.rozie-chart[data-rozie-s-e7b1d9e8] {
  position: relative;
  width: 100%;
}
...
export default function Scatter(_props: ScatterProps): JSX.Element {
  ...
  return (
    <>
    <div class={"rozie-chart"} style={{ height: local.height + 'px', width: local.width ? local.width + 'px' : undefined }} data-rozie-s-e7b1d9e8="">
      
      <canvas ref={(el) => { canvasElRef = el as HTMLElement; }} role="img" aria-label={local.ariaLabel} data-rozie-s-e7b1d9e8="">{(_props.fallbackSlot ?? _props.slots?.['fallback']?.({}))}</canvas>
    </div>
