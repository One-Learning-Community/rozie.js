...
import { Chart as ChartJS, PieController, ArcElement, Legend, Tooltip, Colors } from 'chart.js';
// Pie registers only its own Chart.js controller/element/scale set
...
const Pie = forwardRef<PieHandle, PieProps>(function Pie(_props: PieProps, ref): JSX.Element {
  ...
  const props: Omit<PieProps, 'data' | 'options' | 'height' | 'width' | 'plugins' | 'updateMode' | 'redraw' | 'ariaLabel' | 'datasetIdKey' | 'destroyDelay'> & { data: Record<string, any>; options: Record<string, any>; height: number; width: number; plugins: any[]; updateMode: string; redraw: boolean; ariaLabel: string; datasetIdKey: string; destroyDelay: number } = {
    ..._props,
  ...
  const portals = {
    tooltip: (container: HTMLElement, scope: { model: unknown }): (() => void) => {
      ...
      container.setAttribute('data-rozie-portal-tooltip', 'd3ac8fdc');
  ...
  // Pie registers only its own Chart.js controller/element/scale set
  // (tree-shakable — importing this component does not pull every controller).
  ChartJS.register(PieController, ArcElement, Legend, Tooltip, Colors);
  ...
  return (
    <>
    <div className={"rozie-chart"} style={{ height: props.height + 'px', width: props.width ? props.width + 'px' : undefined }} data-rozie-s-d3ac8fdc="">
      
      <canvas ref={canvasEl} role="img" aria-label={props.ariaLabel} data-rozie-s-d3ac8fdc="">{(props.renderFallback ?? props.slots?.['fallback'])?.()}</canvas>
    </div>
