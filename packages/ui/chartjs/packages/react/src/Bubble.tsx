...
import { Chart as ChartJS, BubbleController, PointElement, LinearScale, Legend, Tooltip, Colors } from 'chart.js';
// Bubble registers only its own Chart.js controller/element/scale set
...
const Bubble = forwardRef<BubbleHandle, BubbleProps>(function Bubble(_props: BubbleProps, ref): JSX.Element {
  ...
  const props: Omit<BubbleProps, 'data' | 'options' | 'height' | 'width' | 'plugins' | 'updateMode' | 'redraw' | 'ariaLabel' | 'datasetIdKey' | 'destroyDelay'> & { data: Record<string, any>; options: Record<string, any>; height: number; width: number; plugins: any[]; updateMode: string; redraw: boolean; ariaLabel: string; datasetIdKey: string; destroyDelay: number } = {
    ..._props,
  ...
  const portals = {
    tooltip: (container: HTMLElement, scope: { model: unknown }): (() => void) => {
      ...
      container.setAttribute('data-rozie-portal-tooltip', 'da10bb9e');
  ...
  // Bubble registers only its own Chart.js controller/element/scale set
  // (tree-shakable — importing this component does not pull every controller).
  ChartJS.register(BubbleController, PointElement, LinearScale, Legend, Tooltip, Colors);
  ...
  function tooltipExternal(context: any) {
    ...
    if (key !== tooltipKey.current) {
      tooltipKey.current = key;
  ...
  }, [composedOnClick, composedOnHover, props.data, props.options, props.plugins, props.renderTooltip, tooltipExternal]);
  function recreate() {
  ...
  useEffect(() => {
    ...
    instance.current = new ChartJS(canvasEl$local.current, _buildConfigRef.current());
    return () => {
  ...
  useEffect(() => {
    ...
    if (!instance.current) return;
    instance.current.options = buildConfig().options;
  ...
  return (
    <>
    <div className={"rozie-chart"} style={{ height: props.height + 'px', width: props.width ? props.width + 'px' : undefined }} data-rozie-s-da10bb9e="">
      
      <canvas ref={canvasEl} role="img" aria-label={props.ariaLabel} data-rozie-s-da10bb9e="">{(props.renderFallback ?? props.slots?.['fallback'])?.()}</canvas>
    </div>
