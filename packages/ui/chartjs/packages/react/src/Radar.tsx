...
const Radar = forwardRef<RadarHandle, RadarProps>(function Radar(_props: RadarProps, ref): JSX.Element {
  ...
  const props: Omit<RadarProps, 'data' | 'options' | 'height' | 'width' | 'plugins' | 'updateMode' | 'redraw' | 'ariaLabel' | 'datasetIdKey' | 'destroyDelay'> & { data: Record<string, any>; options: Record<string, any>; height: number; width: number; plugins: any[]; updateMode: string; redraw: boolean; ariaLabel: string; datasetIdKey: string; destroyDelay: number } = {
    ..._props,
  ...
  return (
    <>
    <div className={"rozie-chart"} style={{ height: props.height + 'px', width: props.width ? props.width + 'px' : undefined }} data-rozie-s-6680f000="">
      
      <canvas ref={canvasEl} role="img" aria-label={props.ariaLabel} data-rozie-s-6680f000="">{(props.renderFallback ?? props.slots?.['fallback'])?.()}</canvas>
    </div>
