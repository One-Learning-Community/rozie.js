/**
 * web-types.mjs — build a JetBrains `web-types.json` for the Vue leaf from a
 ...
 * chartjs has no `model: true` props, so no `update:<prop>` event is ever
 * synthesized here — the events array is exactly `ir.emits`.
 *
 * MULTI-COMPONENT (unlike codemirror, single-component): the vue leaf ships
 * NINE components — the generic `Chart` plus the 8 per-type variants (`Line`
 * `Bar` `Pie` `Doughnut` `PolarArea` `Radar` `Scatter` `Bubble`). This builder
 * stays single-component by construction (`vue-components: [ one ]`) — the
 * caller (codegen.mjs) invokes it once per component, over the SAME per-type
 * variant IR list codegen already lowers for the barrels and the deep-import
 * subpaths, and splices the nine `vue-components[0]` entries into ONE document
 * (the rete FlowCanvas/NodeType/Port merge shape — see codegen.mjs).
 *
 * Ported from `packages/ui/rete/scripts/web-types.mjs` (D-05).
 *
...
/** Build the scoped-slot `type` string from a slot's params (`{ model: any }`). */
function slotScopeType(slot) {
  const params = slot.params || [];
  if (!params.length) return undefined;
  const body = params.map((p) => `${p.name}: any`).join('; ');
  return `{ ${body} }`;
}

/**
 * Build the web-types document object for a SINGLE component (Chart or one
 * per-type variant). Called once per component by codegen.mjs; the caller
 * splices the resulting `vue-components[0]` entries into one document.
 *
 * @param {object}  opts
 * @param {object}  opts.ir            once-parsed/lowered IR for THIS component (props/emits/slots)
 * @param {string}  opts.pkgName       leaf package name (e.g. @rozie-ui/chartjs-vue)
 * @param {string}  opts.version       leaf version (from its package.json)
 * @param {string}  opts.componentName imported symbol / tag (e.g. Chart, Bar, PolarArea)
...
export function buildWebTypes({
  ...
  // Events: the source emits (chartjs has no model:true props, so no
  // synthesized update:<prop> event — every variant's events are exactly
  // click/hover/datasetClick).
