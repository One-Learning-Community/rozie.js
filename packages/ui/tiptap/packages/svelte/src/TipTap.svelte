...
interface Props {
   ...
   * @example
   * <TipTap bind:html={content} placeholder="Start writing…" />
   */
  html?: string;
  /**
   ...
   * @example
   * <TipTap nodeSpecs={[{ name: 'mention', tag: 'span[data-mention]', group: 'inline', inline: true, atom: true, attrs: { id: { default: null } } }]} />
   */
  nodeSpecs?: any[];
  /**
...
const makeNodeView = (spec: any) => (props: any) => {
  const {
    node,
    getPos,
    editor: ed
...
// here.
const makeNodeViewExtensions = (specs: any) => specs.map((spec: any) => {
...
onMount(() => {
  ...
  const nodeViewExtensions = nodeView && nodeSpecs.length ? makeNodeViewExtensions(nodeSpecs) : [];
