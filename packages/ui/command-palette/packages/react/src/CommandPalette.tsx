...
interface ArgsFieldCtx { item: any; arg: any; value: any; setValue: any; }
...
interface CommandPaletteProps {
  ...
  renderArgsField?: (ctx: ArgsFieldCtx) => ReactNode;
...
const CommandPalette = forwardRef<CommandPaletteHandle, CommandPaletteProps>(function CommandPalette(_props: CommandPaletteProps, ref): JSX.Element {
  ...
  const props: Omit<CommandPaletteProps, 'score' | 'items' | 'defaultItems' | 'placeholder' | 'emptyText' | 'closeOnSelect' | 'ariaLabel' | 'idBase' | 'searchDebounce' | 'actionKey' | 'closeOnAction' | 'groupCap' | 'appendTo' | 'virtual' | 'virtualMaxHeight' | 'virtualEstimateRowHeight'> & { score: ((...args: any[]) => any) | null; items: any[]; defaultItems: any[]; placeholder: string; emptyText: string; closeOnSelect: boolean; ariaLabel: string; idBase: string; searchDebounce: number; actionKey: string; closeOnAction: boolean; groupCap: number; appendTo: boolean | string; virtual: boolean; virtualMaxHeight: (string) | null; virtualEstimateRowHeight: (number) | null } = {
    ..._props,
    ...
    virtualMaxHeight: _props.virtualMaxHeight ?? null,
  ...
  function breadcrumbStack() {
    return breadcrumb(levelStack, props.ariaLabel);
  }
  ...
  const openActionMenu = useCallback((item: any) => {
    ...
    const anchorTop = activeRow && frame$local ? activeRow.getBoundingClientRect().top - frame$local.getBoundingClientRect().top + frame$local.scrollTop : 0;
    setActionMenuTop(anchorTop);
    ...
    if (typeof requestAnimationFrame !== 'undefined') {
      requestAnimationFrame(() => {
        ...
        if (menuEl && frame$local) {
          ...
          if (anchorTop > maxTop) setActionMenuTop(maxTop);
        }
      });
    } else {
      focusFirstMenuItem();
    }
  }, [deepQuerySelector, focusFirstMenuItem]);
  ...
  function focusFirstArgField() {
    const frame$local = frame.current;
    if (!frame$local) return;
    const el: any = frame$local.querySelector('[data-command-palette-args] input');
    if (el && typeof el.focus === 'function') el.focus();
  ...
  function focusArgFieldAt(idx: any) {
    const frame$local = frame.current;
    if (!frame$local) return;
    const els: any = frame$local.querySelectorAll('[data-command-palette-args] input');
  ...
  function openArgsSurface(item: any) {
    ...
    // strictly later task.
    argsJustOpened.current = true;
    Promise.resolve().then(() => {
      argsJustOpened.current = false;
    });
    combobox.current?.pinOpen(true);
    if (typeof requestAnimationFrame !== 'undefined') {
      requestAnimationFrame(() => {
  ...
  const onPanelKeydown = useCallback((e: any) => {
    ...
    if (activeSurface === 'args') {
      if (e.key === 'Enter') {
        ...
        if (argsJustOpened.current) return;
        submitArgs();
        return;
      }
      if (e.key === 'Backspace') {
        ...
        const frame$local = frame.current;
        const firstInput: any = frame$local ? frame$local.querySelector('[data-command-palette-args] input') : null;
  ...
  return (
    ...
    {!!(open) && ((() => { const __rozieContainer = typeof document === 'undefined' ? null : (resolveAppendTo(props.appendTo)); return __rozieContainer ? createPortal(<div className={"rozie-command-palette"} onClick={($event) => { onBackdropClick($event); }} data-rozie-s-768cad96="">
      ...
      <div ref={panel} className={"rozie-command-palette-panel"} role="dialog" aria-modal="true" aria-label={props.ariaLabel} data-rozie-s-768cad96="">
        ...
        <Combobox ref={combobox} inline={true} disableFilter={true} closeOnSelect={false} options={orderedItems()} groups={commandGroups()} groupCap={props.groupCap} virtual={currentVirtual()} maxHeight={currentVirtualMaxHeight()} estimateRowHeight={currentVirtualEstimateRowHeight()} optionValue={commandValue} optionDisabled={commandDisabled} placeholder={currentPlaceholder()} ariaLabel={props.ariaLabel} idBase={props.idBase} value={activeValue} onValueChange={setActiveValue} onChange={($event) => { onComboboxChange($event); }} onSearch={($event) => { onComboboxSearch($event); }} data-rozie-s-768cad96="" renderOption={({ option, index, active, selected, disabled }) => (<>
        ...
        {!!(activeSurface === 'args') && <div data-command-palette-args="" data-testid="command-palette-args" className={"rozie-command-palette-args"} role="group" aria-label={rozieAttr('Arguments for ' + (argsState ? labelText(argsState.item) : ''))} data-rozie-s-768cad96="">
          <span className={"rozie-command-palette-args-chip rozie-command-palette-breadcrumb-segment--current"} data-testid="command-palette-args-chip" aria-hidden="true" data-rozie-s-768cad96="">{rozieDisplay(argsState ? labelText(argsState.item) : '')}</span>
          {(argsState ? argsState.argList : []).map((arg, argIdx) => <span key={arg.id} className={"rozie-command-palette-args-field"} data-rozie-s-768cad96="">
            {(props.renderArgsField ?? props.slots?.['argsField']) ? ((props.renderArgsField ?? props.slots?.['argsField']) as Function)({ item: argsState ? argsState.item : null, arg, value: argsState ? argsState.values[arg.id] : '', setValue: setArgValueFor(arg.id) }) : <input type="text" className={"rozie-command-palette-args-input"} data-testid="command-palette-args-input" value={argsState ? argsState.values[arg.id] : ''} placeholder={rozieAttr(arg.placeholder || arg.id)} aria-label={rozieAttr(arg.placeholder || arg.id)} onInput={($event) => { onArgFieldInput(arg.id, $event); }} data-rozie-s-768cad96="" />}
          </span>)}
      ...
      {!!(atActions()) && <div data-command-palette-menu="" data-testid="command-palette-actions-menu" className={"rozie-command-palette-actions-menu"} role="menu" aria-label={rozieAttr(actionAnchor ? labelText(actionAnchor.item) : undefined)} style={parseInlineStyle('top:' + actionMenuTop + 'px')} onKeyDown={($event) => { onActionMenuKeydown($event); }} data-rozie-s-768cad96="">
