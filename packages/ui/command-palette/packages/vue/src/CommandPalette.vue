...
<Teleport :to="__roziePortalTo0" :disabled="!__roziePortalTo0"><div v-if="open" class="rozie-command-palette" @click="onBackdropClick($event)">
  ...
  <div ref="panelRef" class="rozie-command-palette-panel" role="dialog" aria-modal="true" :aria-label="props.ariaLabel">
    ...
    </div><div :class="['rozie-command-palette-list-region', { 'rozie-command-palette-list-region--inert': activeSurface === 'args' }]" :aria-hidden="!!(activeSurface === 'args')">
    
    <Combobox ref="comboboxRef" :inline="true" :disable-filter="true" :close-on-select="false" :options="orderedItems()" :groups="commandGroups()" :group-cap="props.groupCap" :virtual="currentVirtual()" :max-height="currentVirtualMaxHeight()" :estimate-row-height="currentVirtualEstimateRowHeight()" :option-value="commandValue" :option-disabled="commandDisabled" :placeholder="currentPlaceholder()" :aria-label="props.ariaLabel" :id-base="props.idBase" v-model:value="activeValue" @change="onComboboxChange($event)" @search="onComboboxSearch($event)"><template #option="{ option, index, active, selected, disabled }">
...
defineSlots<{
  ...
  argsField(props: { item: any; arg: any; value: any; setValue: any }): any;
...
const onComboboxChange = (e: any) => {
  ...
  if (activeSurface.value === 'args') return;
...
const closeAnySurface = () => {
  if (activeSurface.value === 'args') closeArgsSurface();else if (activeSurface.value !== 'list') closeActionMenu();
};
...
const onPanelKeydown = (e: any) => {
  ...
  if (e.key === 'Escape') {
    ...
    if (route === 'close-surface') {
      if (activeSurface.value === 'args') closeArgsSurface();else closeActionMenu();
    } else if (route === 'pop-level') goBack();else closePalette();
    return;
  }
  if (activeSurface.value === 'args') {
    if (e.key === 'Enter') {
      e.preventDefault();
