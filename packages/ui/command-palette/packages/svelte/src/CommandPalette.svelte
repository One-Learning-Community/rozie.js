...
import { rozieAttr, rozieDisplay, roziePortal, rozieStyle } from '@rozie/runtime-svelte';
...
interface Props {
  ...
  argsField?: Snippet<[{ item: any; arg: any; value: any; setValue: any }]>;
...
let {
  ...
  appendTo = false,
  virtual = false,
  virtualMaxHeight = null,
  virtualEstimateRowHeight = null,
  breadcrumb: __breadcrumbProp,
  option: __optionProp,
  groupHeading: __groupHeadingProp,
  empty: __emptyProp,
  argsField: __argsFieldProp,
...
const argsField = $derived(__argsFieldProp ?? snippets?.argsField);
...
const cpAnchorIndex = (option: any) => {
  ...
  if (typeof requestAnimationFrame !== 'undefined') {
    requestAnimationFrame(() => {
      ...
      if (menuEl && frame$local) {
        ...
        if (anchorTop > maxTop) actionMenuTop = maxTop;
      }
    });
  } else {
    focusFirstMenuItem();
...
const setArgValue = (id: any, v: any) => {
...
// attribute.
const setArgValueFor = (id: any) => (v: any) => setArgValue(id, v);
...
const onArgFieldInput = (id: any, e: any) => {
