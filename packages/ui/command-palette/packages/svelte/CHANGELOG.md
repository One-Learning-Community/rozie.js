...
- Stale-publish reconciliation. The published `0.4.0` tarball predates several regenerations that landed on `main` without a version bump, so the registry kept serving stale bytes. This release republishes the current generated output:
  ...
  - Apple-platform detection for hotkey badge rendering (⌘ vs Ctrl) is now sniffed once at mount into state instead of being recomputed on every render — an internal perf change with no observable difference.
...
- Updated dependencies [f3e1bdf]
  - @rozie-ui/combobox-svelte@0.3.0
...
- Updated dependencies [458db46]
  - @rozie-ui/combobox-svelte@0.2.0
