 ...
 */
export function commandMatches(item, loweredQuery) {
    if (!loweredQuery)
        return true;
    if (!item)
        return false;
    const label = String(item.label == null ? '' : item.label).toLowerCase();
    if (label.indexOf(loweredQuery) !== -1)
        return true;
    const kws = Array.isArray(item.keywords) ? item.keywords : [];
    for (let i = 0; i < kws.length; i++) {
        if (String(kws[i] == null ? '' : kws[i]).toLowerCase().indexOf(loweredQuery) !== -1)
            return true;
    }
    return false;
}
 ...
 */
export function filterCommands(items, rawQuery) {
