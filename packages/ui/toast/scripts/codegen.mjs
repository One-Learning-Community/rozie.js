 ...
 *
 * NAME/SLUG split: the component is `Toaster` (file Toaster.rozie → emits
 * Toaster.{tsx,vue,svelte,ts}, barrel `Toaster` + `ToasterHandle`, Lit tag
 * `rozie-toaster`), but the family SLUG is `toast` (root pkg @rozie-ui/toast,
 * leaves @rozie-ui/toast-*, docs docs/components/toast.md) — mirrors DataTable in
 * the data-table slug.
 *
 * Toaster is collision-free: it emits NOTHING (the imperative show/dismiss/clear
 * handle replaces events), has no `model: true` prop, and its expose verbs are
 * not inherited host-element members — so compile()×6 is fully clean (zero error
 * AND zero warning). The README renderer skips the Events section when ir.emits
 * is empty.
 *
 ...
 * consumers can `import '@rozie-ui/toast-<fw>/themes/X.css'`.
 *
 * Steps:
 *   1. read src/Toaster.rozie
 ...
 * its src/Toaster.vue + themes + README (it never cleans the leaf src, so the
...
const ROOT = resolve(import.meta.dirname, '..'); // packages/ui/toast
...
const TARGETS = {
  react: { dir: 'react', file: 'Toaster.tsx', build: 'tsdown' },
  vue: { dir: 'vue', file: 'Toaster.vue', build: 'source' },
  svelte: { dir: 'svelte', file: 'Toaster.svelte', build: 'source' },
  angular: { dir: 'angular', file: 'Toaster.ts', build: 'source' },
  solid: { dir: 'solid', file: 'Toaster.tsx', build: 'tsdown' },
  lit: { dir: 'lit', file: 'Toaster.ts', build: 'tsdown' },
};
...
function main() {
  ...
  // Keep the hand-kept manifests in lockstep with the IR. (Toaster emits NOTHING,
  // so the emits loop is a no-op — eventManifest is intentionally empty.)
  ...
  for (const [target, cfg] of Object.entries(TARGETS)) {
    ...
    // `Toaster` the READMEs/consumers import (`export *` would NOT forward a
    // default). React/Solid also emit a named `ToasterHandle` interface (the
    ...
    // React-only sidecars.
    if (target === 'react') {
      if (r.css) writeFileSync(resolve(leafSrc, 'Toaster.css'), r.css);
      if (r.types) writeFileSync(resolve(leafSrc, 'Toaster.d.ts'), r.types);
    }
  ...
  const docsPath = resolve(REPO_ROOT, 'docs/components/toast.md');
  ...
  validateDocsSurfaceNames(ir, 'toast', REPO_ROOT);
