/**
 * README rendering + docs-table validation for @rozie-ui/combobox.
 *
 * Everything structural is derived from a SINGLE parse of Combobox.rozie
...
const frameworks = ${OPTIONS_SNIPPET};
...
export function Demo() {
  ...
  lit: {
    lang: 'ts',
    code: `import '@rozie-ui/combobox-lit';
...
// `value` model prop IS the control value — a combobox IS a form control.
...
export class DemoComponent {
  ...
  solid: {
    lang: 'tsx',
    code: `import { Combobox, type ComboboxHandle } from '@rozie-ui/combobox-solid';
...
export function renderReadme(target, ir, eventManifest, pkgName, handleManifest = {}) {
  ...
  lines.push('');
  lines.push(
    `Idiomatic **${target}** \`Combobox\` — a headless, fully-accessible (WAI-ARIA) ` +
      `combobox / autocomplete: a text input plus a popup listbox with ` +
      `\`aria-activedescendant\` keyboard navigation (Arrow/Home/End/Enter/Escape), ` +
      `built-in client-side filtering (or async/server-side via the \`search\` event + ` +
      `\`disableFilter\`), a custom-option scoped slot, and a two-way \`value\` binding — ` +
      `compiled from one ` +
      `[Rozie](https://github.com/One-Learning-Community/rozie.js) source. ` +
      `There is NO third-party engine; the behaviour is authored once on native DOM. ` +
  ...
  lines.push('');
  lines.push(
    'Every visual value is a `--rozie-combobox-*` CSS custom property — override any of them at ' +
  ...
  if (target === 'angular' && modelProps.length === 1) {
    ...
    lines.push(
      `The generated class implements \`ControlValueAccessor\` — the \`${modelProp.name}\` model ` +
        'prop is the control value, so a combobox **is** a form control. It binds to template-driven ' +
  ...
  if (ir.expose && ir.expose.length > 0) {
    ...
    lines.push(
      'Beyond props, the component exposes imperative methods (declared once in the Rozie source ' +
        ...
        'text input) — on the Lit custom element this is an accepted ROZ137 warn-only override, ' +
        'the public `focus()` handle is intended:',
    );
    lines.push('');
