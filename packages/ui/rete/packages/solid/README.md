...
```tsx
import '@rozie-ui/rete-solid/themes/shadcn.css';    // or material.css, bootstrap.css, base.css
```

The full token vocabulary — plus the `.dark` / `[data-theme="dark"]` class strategy for apps that toggle theme by a root class — lives in `@rozie-ui/rete-solid/themes/base.css`.
