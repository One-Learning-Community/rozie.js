...
export default function FlowCanvas(_props: FlowCanvasProps): JSX.Element {
  ...
  onMount(() => {
    ...
    AreaExtensions.restrictor(area, {
      scaling: () => ({
        min: typeof local.minZoom === 'number' && local.minZoom > 0 ? local.minZoom : 0.01,
        max: typeof local.maxZoom === 'number' && local.maxZoom > 0 ? local.maxZoom : 100
      })
    });
    ...
    if (container && typeof container.addEventListener === 'function') {
      onCanvasKeydown = (e: any) => {
        ...
        if (local.selectable === false || local.readonly === true) return;
    ...
    const renderNode = (element: any, node: any) => {
      ...
      if (existing && existing.element === element) {
        ...
        resolveNodeBox(existing.box, meta, meta.type != null ? typeReg()[meta.type] : null);
      ...
      resolveNodeBox(box, meta, meta.type != null ? typeReg()[meta.type] : null);
      ...
      if (context.type === 'connectioncreate') {
        ...
        if (local.validateTypes !== false) {
          ...
          if (srcType != null && tgtType != null && srcType !== tgtType) {
            if (!programmatic || imperativeAdd) _props.onConnectionRejected?.({
              ...conn,
    ...
    area.addPipe((context: any) => {
      ...
      if (context.type === 'nodepicked') {
        ...
        // core.pick + select.
        if (local.selectable === false || local.readonly === true) return undefined;
        ...
        if (local.readonly === true) return undefined;
        ...
        if (local.pannable === false) return undefined;
        ...
        if (local.zoomable === false) return undefined;
    ...
    if (container && typeof container.addEventListener === 'function') {
      resizeHandleNw = resizeHandleNwElRef || null;
      resizeHandleNe = resizeHandleNeElRef || null;
      resizeHandleSw = resizeHandleSwElRef || null;
      resizeHandleSe = resizeHandleSeElRef || null;
  ...
  createEffect(on(() => (() => local.selectable)(), (v) => untrack(() => ((v: any) => {
  ...
  let resizeHandleNwElRef: HTMLElement | null = null;
  let resizeHandleNeElRef: HTMLElement | null = null;
  let resizeHandleSwElRef: HTMLElement | null = null;
  let resizeHandleSeElRef: HTMLElement | null = null;
  ...
  function queueConnWarn(id: any, reason: any) {
  ...
  function clampResizeSize(typeSpec: any, w: any, h: any) {
  ...
  function resetNodeSize(id: any) {
    if (programmatic) return;
    if (!local.selectable || local.readonly) return;
    pushHistory();
    const g = baseGraph();
  ...
  bodyHostFor: (nodeId: any) => {
    ...
    <div ref={(el) => { canvasElRef = el as HTMLElement; }} tabIndex={0} {...attrs} class={"rozie-flow-canvas" + " " + rozieClass({ 'rozie-flow-canvas--lines': local.background === 'lines', 'rozie-flow-canvas--cross': local.background === 'cross', 'rozie-flow-canvas--none': local.background === 'none' }) + (((attrs as unknown as Record<string, unknown>).class as string | undefined) ? " " + ((attrs as unknown as Record<string, unknown>).class as string | undefined) : "")} data-rozie-s-cd396d6a="">
      ...
      <div class={"rozie-flow-resize-handle rozie-flow-resize-handle--nw"} ref={(el) => { resizeHandleNwElRef = el as HTMLElement; }} data-testid="flow-resize-handle-nw" data-rozie-s-cd396d6a="" />
      <div class={"rozie-flow-resize-handle rozie-flow-resize-handle--ne"} ref={(el) => { resizeHandleNeElRef = el as HTMLElement; }} data-testid="flow-resize-handle-ne" data-rozie-s-cd396d6a="" />
      <div class={"rozie-flow-resize-handle rozie-flow-resize-handle--sw"} ref={(el) => { resizeHandleSwElRef = el as HTMLElement; }} data-testid="flow-resize-handle-sw" data-rozie-s-cd396d6a="" />
      <div class={"rozie-flow-resize-handle rozie-flow-resize-handle--se"} ref={(el) => { resizeHandleSeElRef = el as HTMLElement; }} data-testid="flow-resize-handle-se" data-rozie-s-cd396d6a="" />
