...
const flushPendingPorts = () => {
  const pend = pendingPorts.value;
  let pendingChanged = false;
  let nodeChanged = false;
  const nextPend = {
    ...pend
  };
  let nextReg = nodeReg.value;
  ...
  if (nodeChanged) nodeReg.value = nextReg;
  if (pendingChanged) pendingPorts.value = nextPend;
};
...
provide('rete:canvas', {
  ...
  addPort: (id: any, side: any, key: any, label: any, multiple: any) => {
    ...
    if (!cur) {
      ...
      const bucket = Array.isArray(pendingPorts.value[id]) ? pendingPorts.value[id].slice() : [];
