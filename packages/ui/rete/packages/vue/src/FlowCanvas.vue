...
<div :class="['rozie-flow-canvas', { 'rozie-flow-canvas--lines': props.background === 'lines', 'rozie-flow-canvas--cross': props.background === 'cross', 'rozie-flow-canvas--none': props.background === 'none' }]" ref="canvasElRef" tabindex="0" v-bind="$attrs">
  ...
  <div class="rozie-flow-resize-handle rozie-flow-resize-handle--nw" ref="resizeHandleNwElRef" data-testid="flow-resize-handle-nw"></div>
  <div class="rozie-flow-resize-handle rozie-flow-resize-handle--ne" ref="resizeHandleNeElRef" data-testid="flow-resize-handle-ne"></div>
  <div class="rozie-flow-resize-handle rozie-flow-resize-handle--sw" ref="resizeHandleSwElRef" data-testid="flow-resize-handle-sw"></div>
  <div class="rozie-flow-resize-handle rozie-flow-resize-handle--se" ref="resizeHandleSeElRef" data-testid="flow-resize-handle-se"></div>
...
const resizeHandleNwElRef = ref<HTMLElement>();
const resizeHandleNeElRef = ref<HTMLElement>();
const resizeHandleSwElRef = ref<HTMLElement>();
...
// disconnect it.
let nodeResizeObserver: any = null;
...
const pendingResizeSizes = new Map();
...
const clampInversionWarnedTypes = new Set();
...
const incompatibleMarked = new Set();
...
const warnConn = (msg: any) => {
  if (typeof console !== 'undefined' && console.warn) {
    console.warn(`[@rozie-ui/rete] ${msg}`);
  }
};
const pendingConnWarns = new Map();
const warnedConnIds = new Set();
...
let connWarnTimer: any = null;
const flushConnWarns = () => {
  connWarnTimer = null;
...
const queueConnWarn = (id: any, reason: any) => {
  if (id == null || warnedConnIds.has(id)) return;
  pendingConnWarns.set(id, reason);
  if (connWarnTimer && typeof clearTimeout === 'function') {
    try {
      clearTimeout(connWarnTimer);
    } catch (e: any) {}
  }
  if (typeof setTimeout === 'function') connWarnTimer = setTimeout(flushConnWarns, CONN_WARN_SETTLE_MS);else Promise.resolve().then(flushConnWarns);
};
...
const duplicateInto = (g: any, id: any) => {
...
onMounted(() => {
  ...
  if (typeof ResizeObserver === 'function') {
    nodeResizeObserver = new ResizeObserver((entries: any) => {
  ...
  connectionPlugin.addPipe((context: any) => {
    ...
    if (context.type === 'connectionpick') {
      ...
      if (!programmatic && props.readonly === true) return undefined;
  ...
  AreaExtensions.restrictor(area, {
    scaling: () => ({
      min: typeof props.minZoom === 'number' && props.minZoom > 0 ? props.minZoom : 0.01,
      max: typeof props.maxZoom === 'number' && props.maxZoom > 0 ? props.maxZoom : 100
    })
  });
  ...
  if (container && typeof container.addEventListener === 'function') {
    onCanvasKeydown = (e: any) => {
      ...
      if (props.selectable === false || props.readonly === true) return;
  ...
  const renderNode = (element: any, node: any) => {
    ...
    if (existing && existing.element === element) {
      ...
      resolveNodeBox(existing.box, meta, meta.type != null ? typeReg.value[meta.type] : null);
    ...
    resolveNodeBox(box, meta, meta.type != null ? typeReg.value[meta.type] : null);
    ...
    if (nodeResizeObserver) {
      ...
      nodeResizeObserver.observe(box);
      socketDisposers.push(() => {
        try {
          nodeResizeObserver.unobserve(box);
        } catch (e: any) {}
      });
    }
    ...
    if (context.type === 'connectioncreate') {
      ...
      if (props.validateTypes !== false) {
        ...
        if (srcType != null && tgtType != null && srcType !== tgtType) {
          if (!programmatic || imperativeAdd) emit('connection-rejected', {
            ...conn,
  ...
  area.addPipe((context: any) => {
    ...
    if (context.type === 'nodepicked') {
      ...
      // core.pick + select.
      if (props.selectable === false || props.readonly === true) return undefined;
      ...
      if (props.readonly === true) return undefined;
      ...
      if (props.pannable === false) return undefined;
    ...
    try {
      for (const spec of merged as any) {
        ...
        if (!sourceNode || !targetNode) {
          queueConnWarn(spec.id, `connection "${spec.id}": endpoint node "${!sourceNode ? spec.source : spec.target}" not found.`);
          continue;
        }
        ...
        if (!sourceNode.outputs || !sourceNode.outputs[spec.sourceOutput]) {
          queueConnWarn(spec.id, `connection "${spec.id}": source node "${spec.source}" has no output "${spec.sourceOutput}".`);
          continue;
        }
        if (!targetNode.inputs || !targetNode.inputs[spec.targetInput]) {
          queueConnWarn(spec.id, `connection "${spec.id}": target node "${spec.target}" has no input "${spec.targetInput}".`);
          continue;
        }
        ...
        connMeta.set(spec.id, spec);
        const added = await editor.addConnection(conn);
        if (added === false) {
          ...
          queueConnWarn(spec.id, `connection "${spec.id}": rejected by connection validation.`);
        }
      }
  ...
  const singleResizableSelectedId = () => {
    ...
    if (id == null) return null;
    const meta = nodeMeta.get(id);
    const typeSpec = meta && meta.type != null ? typeReg.value[meta.type] : null;
  ...
  if (container && typeof container.addEventListener === 'function') {
    resizeHandleNw = resizeHandleNwElRef.value || null;
    resizeHandleNe = resizeHandleNeElRef.value || null;
    resizeHandleSw = resizeHandleSwElRef.value || null;
    resizeHandleSe = resizeHandleSeElRef.value || null;
