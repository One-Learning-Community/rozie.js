...
@Component({
  ...
  template: `
    ...
    <div class="rozie-flow-canvas" [ngClass]="{ 'rozie-flow-canvas--lines': background() === 'lines', 'rozie-flow-canvas--cross': background() === 'cross', 'rozie-flow-canvas--none': background() === 'none' }" #canvasEl tabindex="0" #rozieSpread_0 #rozieListenersTarget_1>
    ...
    <div class="rozie-flow-minimap" #minimapEl data-testid="flow-minimap"></div>
      ...
      <div class="rozie-flow-resize-handle rozie-flow-resize-handle--nw" #resizeHandleNwEl data-testid="flow-resize-handle-nw"></div>
      <div class="rozie-flow-resize-handle rozie-flow-resize-handle--ne" #resizeHandleNeEl data-testid="flow-resize-handle-ne"></div>
      <div class="rozie-flow-resize-handle rozie-flow-resize-handle--sw" #resizeHandleSwEl data-testid="flow-resize-handle-sw"></div>
      <div class="rozie-flow-resize-handle rozie-flow-resize-handle--se" #resizeHandleSeEl data-testid="flow-resize-handle-se"></div>
  ...
  styles: [`
    ...
    ::ng-deep .rozie-flow-canvas .rozie-flow-node:active { cursor: grabbing; }
    ...
    ::ng-deep .rozie-flow-canvas .rozie-flow-node__body {
        min-width: 0;
    ...
    ::ng-deep .rozie-flow-canvas .rozie-flow-socket--input { margin-left: calc(var(--rozie-flow-socket-size, 16px) / -2); }
    ::ng-deep .rozie-flow-canvas .rozie-flow-socket--output { margin-right: calc(var(--rozie-flow-socket-size, 16px) / -2); }
    ::ng-deep .rozie-flow-canvas .rozie-flow-socket:hover { background: var(--rozie-flow-socket-hover-bg, var(--rozie-flow-accent, #3b82f6)); }
    ::ng-deep .rozie-flow-canvas .rozie-flow-socket--incompatible {
    ...
    ::ng-deep .rozie-flow-canvas .rozie-flow-socket--top { margin-top: calc(var(--rozie-flow-socket-size, 16px) / -2); }
    ::ng-deep .rozie-flow-canvas .rozie-flow-socket--bottom { margin-bottom: calc(var(--rozie-flow-socket-size, 16px) / -2); }
...
export class FlowCanvas {
   ...
   * @example
   * <rozie-flow-canvas [(graph)]="graph" [validateTypes]="true" />
   */
   ...
   */
  canConnect = input<((...args: any[]) => any) | null>(null);
  /**
  ...
  resizeHandleNwEl = viewChild<ElementRef<HTMLDivElement>>('resizeHandleNwEl');
  resizeHandleNeEl = viewChild<ElementRef<HTMLDivElement>>('resizeHandleNeEl');
  resizeHandleSwEl = viewChild<ElementRef<HTMLDivElement>>('resizeHandleSwEl');
  resizeHandleSeEl = viewChild<ElementRef<HTMLDivElement>>('resizeHandleSeEl');
  ...
  constructor() {
    ...
    effect(() => { const __watchVal = (() => this.selectable())(); untracked(() => { if (this.__rozieWatchInitial_4) { this.__rozieWatchInitial_4 = false; return; } ((v: any) => {
      if (v === false) this.clearSelection();
    })(__watchVal); }); });
    effect(() => { const __watchVal = (() => this.readonly())(); untracked(() => { if (this.__rozieWatchInitial_5) { this.__rozieWatchInitial_5 = false; return; } ((v: any) => {
      if (v === true) this.clearSelection();
    })(__watchVal); }); });
  }
  ...
  ngAfterViewInit() {
    ...
    if (typeof ResizeObserver === 'function') {
      this.nodeResizeObserver = new ResizeObserver((entries: any) => {
        for (const roEntry of entries as any) {
          const nodeEntry = this.boxEntries.get(roEntry.target);
    ...
    this.connectionPlugin.addPipe((context: any) => {
      ...
      if (context.type === 'connectionpick') {
        ...
        if (!this.programmatic && this.readonly() === true) return undefined;
        ...
        if (!this.programmatic && this.markIncompatibleSockets && context.data && context.data.socket) {
          this.markIncompatibleSockets(context.data.socket.nodeId, context.data.socket.side, context.data.socket.key);
        }
    ...
    AreaExtensions.restrictor(this.area, {
      scaling: () => ({
        min: typeof this.minZoom() === 'number' && this.minZoom() > 0 ? this.minZoom() : 0.01,
        max: typeof this.maxZoom() === 'number' && this.maxZoom() > 0 ? this.maxZoom() : 100
      })
    });
    ...
    if (container && typeof container.addEventListener === 'function') {
      this.onCanvasKeydown = (e: any) => {
        ...
        if (this.selectable() === false || this.readonly() === true) return;
    ...
    // 260825-mip — resolve and apply a node's box size. THE single place width/height are
    // decided, shared by renderNode's fresh-build and in-place branches so the two can never
    // disagree.
    //
    //     effective = clamp( instance node.width ?? type width ?? auto )
    //
    // Instance beats type: an instance `width` is a deliberate act (a corner-drag write-back,
    // or explicit authoring in the bound graph), while the type value is a template default.
    // The clamp applies LAST, to whichever won — that is what makes minWidth/maxWidth genuine
    // invariants of the TYPE rather than mere gesture bounds: nothing renders outside them,
    // including auto-sized body content, which is exactly what a consumer setting
    // `maxWidth: 400` means. (`clampResizeSize` is untouched and still clamps the value
    // written back into the MODEL, so the graph never stores a number CSS silently corrects.)
    //
    // The clamp is expressed as CSS min-width/max-width rather than arithmetic because CSS
    // clamps the `auto` case too, which JS cannot do without measuring.
    //
    // An explicit width ALSO writes minWidth: base.css gives .rozie-flow-node a 140px floor,
    // which is a default for AUTO-sized nodes — a node authored `:width="120"` must render
    // 120, not be silently clamped up to 140. Same for height, defensively.
    // ── node renderer ──
    // Fills the engine-created nodeView element with: input sockets, the body
    // (consumer `node` portal fragment OR default chrome), and output sockets.
    // Re-render (area.update('node', id)) reuses the same element → update in place.
    ...
    // which is a default for AUTO-sized nodes — a node authored `:width="120"` must render
    // 120, not be silently clamped up to 140. Same for height, defensively.
    const resolveNodeBox = (box: any, meta: any, typeSpec: any) => {
      const explicitW = meta.width != null ? meta.width : typeSpec && typeSpec.width != null ? typeSpec.width : null;
      const explicitH = meta.height != null ? meta.height : typeSpec && typeSpec.height != null ? typeSpec.height : null;
      const minW = typeSpec && typeSpec.minWidth != null ? typeSpec.minWidth : null;
      const minH = typeSpec && typeSpec.minHeight != null ? typeSpec.minHeight : null;
      const maxW = typeSpec && typeSpec.maxWidth != null ? typeSpec.maxWidth : null;
      const maxH = typeSpec && typeSpec.maxHeight != null ? typeSpec.maxHeight : null;
      box.style.width = explicitW != null ? explicitW + 'px' : '';
      box.style.height = explicitH != null ? explicitH + 'px' : '';
      box.style.minWidth = explicitW != null ? explicitW + 'px' : minW != null ? minW + 'px' : '';
      box.style.minHeight = explicitH != null ? explicitH + 'px' : minH != null ? minH + 'px' : '';
      box.style.maxWidth = maxW != null ? maxW + 'px' : '';
      box.style.maxHeight = maxH != null ? maxH + 'px' : '';
    };
    const renderNode = (element: any, node: any) => {
      const __typeReg = this.typeReg();
      ...
      if (existing && existing.element === element) {
        ...
        // NodeResizer (D-06/D-07) + 260825-mip: re-apply the resolved box on every
        // re-render. A double-click-reset (74-03) clears the INSTANCE width/height, so this
        // falls back to the TYPE's authored width/height if it has one — "reset" means back
        // to the type default, which is auto only when the type declares no size — and back
        // to fully auto otherwise, reverting a previously sized box on the very next render.
        resolveNodeBox(existing.box, meta, meta.type != null ? __typeReg[meta.type] : null);
      ...
      // NodeResizer (D-06/D-07) + 260825-mip: resolve the box from the instance size, then
      // the TYPE's authored width/height, then auto — clamped by the type's min/max. With no
      // instance size and no type size and no bounds every assignment is '', so CSS width/
      // height stay `auto`: today's behavior, unchanged.
      resolveNodeBox(box, meta, meta.type != null ? __typeReg[meta.type] : null);
      ...
      if (this.nodeResizeObserver) {
        this.boxEntries.set(box, entry);
        this.nodeResizeObserver.observe(box);
        socketDisposers.push(() => {
          try {
            this.nodeResizeObserver.unobserve(box);
          } catch (e: any) {}
        });
      }
      ...
      const typeSpec = meta.type != null ? __typeReg[meta.type] : null;
    ...
    const renderConnection = (element: any, connection: any, startPointer: any, endPointer: any) => {
      ...
      const redraw = () => {
        ...
        const d = route ? waypointPathD(start, route, end) : edgeType === 'step' ? stepPath(start, end) : edgeType === 'smoothstep' ? smoothstepPath(start, end) : edgeType === 'straight' ? straightPath(start, end) : classicConnectionPath([start, end], curvature);
    ...
    this.markIncompatibleSockets = (pickedNodeId: any, pickedSide: any, pickedKey: any) => {
      ...
      for (const [regKey, el] of this.socketReg as any) {
        ...
        if (candidateType !== pickedType) {
          ...
          this.incompatibleMarked.add(el);
        }
      }
    ...
    this.area.addPipe((context: any) => {
      ...
      if (this.programmatic) return context;
      if (context.type === 'nodepicked') {
        ...
        // core.pick + select.
        if (this.selectable() === false || __readonly === true) return undefined;
        ...
        if (__readonly === true) return undefined;
        ...
        if (this.pannable() === false) return undefined;
        ...
        if (this.zoomable() === false) return undefined;
    ...
    const singleResizableSelectedId = () => {
      ...
      const meta = this.nodeMeta.get(id);
      const typeSpec = meta && meta.type != null ? this.typeReg()[meta.type] : null;
    ...
    const trackResizer = () => {
      ...
      if (id == null || !this.area || !container || handles.some((h: any) => !h)) {
      ...
      const ny = rect.top - cbox.top;
      const HALF = 4;
      positionHandle(this.resizeHandleNw, [nx, ny], HALF);
      positionHandle(this.resizeHandleNe, [nx + rect.width, ny], HALF);
      positionHandle(this.resizeHandleSw, [nx, ny + rect.height], HALF);
      positionHandle(this.resizeHandleSe, [nx + rect.width, ny + rect.height], HALF);
    };
    ...
    // trackResizer, leaving the 4 handles stuck visible at their last position. Only the
    // in-flight-rAF guard remains; trackResizer's own id==null branch does the hiding.
    this.scheduleResizerTrack = () => {
      if (this.resizerTrackRaf) return;
      if (typeof requestAnimationFrame === 'function') {
        this.resizerTrackRaf = requestAnimationFrame(trackResizer);
      } else {
        this.resizerTrackRaf = 1;
        Promise.resolve().then(trackResizer);
      }
    };
    // Recompute the tracked node from the live selection (called from the selection emit).
    // Recompute the tracked node from the live selection (called from the selection emit).
    this.syncResizerSelection = () => {
      const id = singleResizableSelectedId();
      if (id === this.resizerTrackedId) {
        this.scheduleResizerTrack();
        return;
      }
      this.resizerTrackedId = id;
      this.scheduleResizerTrack();
    };
    ...
    // the belt to that braces.
    if (container && typeof container.addEventListener === 'function') {
      this.resizeHandleNw = this.resizeHandleNwEl()?.nativeElement || null;
      this.resizeHandleNe = this.resizeHandleNeEl()?.nativeElement || null;
      this.resizeHandleSw = this.resizeHandleSwEl()?.nativeElement || null;
      this.resizeHandleSe = this.resizeHandleSeEl()?.nativeElement || null;
      ...
      const beginResize = (corner: any, handleEl: any) => (e: any) => {
        ...
        if (!this.programmatic && this.history() !== false) {
          this.pendingResizeSnapshot = this.snapshotCurrent();
        }
        this.resizeGestureActive = false;
        this.resizeActiveHandleEl = handleEl;
        this.onResizeHandleMove = (me: any) => {
          const k = this.area && this.area.area && this.area.area.transform && this.area.area.transform.k || 1;
          ...
          const clamped = this.clampResizeSize(typeSpec, w, h);
          const pending = this.pendingResizeSizes.get(id);
          const changed = !pending || pending.width !== clamped.width || pending.height !== clamped.height;
          if (changed && !this.resizeGestureActive) {
            this.resizeGestureActive = true;
            if (this.pendingResizeSnapshot) {
              this.pushHistorySnapshot(this.pendingResizeSnapshot);
          ...
          this.pendingResizeSizes.set(id, next);
          this.scheduleResizeFlush();
        };
        this.onResizeHandleUp = (ue: any) => {
          try {
            if (handleEl.releasePointerCapture && ue && ue.pointerId != null) handleEl.releasePointerCapture(ue.pointerId);
          } catch (err: any) {}
          if (this.resizeActiveHandleEl && this.onResizeHandleMove) {
            try {
              this.resizeActiveHandleEl.removeEventListener('pointermove', this.onResizeHandleMove);
            } catch (err: any) {}
          }
          if (this.resizeActiveHandleEl && this.onResizeHandleUp) {
            try {
              this.resizeActiveHandleEl.removeEventListener('pointerup', this.onResizeHandleUp);
            } catch (err: any) {}
          }
        ...
        handleEl.addEventListener('pointerup', this.onResizeHandleUp);
      };
      this.onResizeNwDown = beginResize('nw', this.resizeHandleNw);
      this.onResizeNeDown = beginResize('ne', this.resizeHandleNe);
      this.onResizeSwDown = beginResize('sw', this.resizeHandleSw);
      this.onResizeSeDown = beginResize('se', this.resizeHandleSe);
      if (this.resizeHandleNw) this.resizeHandleNw.addEventListener('pointerdown', this.onResizeNwDown);
      if (this.resizeHandleNe) this.resizeHandleNe.addEventListener('pointerdown', this.onResizeNeDown);
      if (this.resizeHandleSw) this.resizeHandleSw.addEventListener('pointerdown', this.onResizeSwDown);
      if (this.resizeHandleSe) this.resizeHandleSe.addEventListener('pointerdown', this.onResizeSeDown);
    }
    ...
    this.__rozieDestroyRef.onDestroy(() => {
      ...
      if (this.resizeHandleNw && this.onResizeNwDown) {
        try {
          this.resizeHandleNw.removeEventListener('pointerdown', this.onResizeNwDown);
        } catch (e: any) {}
      }
      if (this.resizeHandleNe && this.onResizeNeDown) {
        try {
          this.resizeHandleNe.removeEventListener('pointerdown', this.onResizeNeDown);
        } catch (e: any) {}
      }
      if (this.resizeHandleSw && this.onResizeSwDown) {
        try {
          this.resizeHandleSw.removeEventListener('pointerdown', this.onResizeSwDown);
        } catch (e: any) {}
      }
      if (this.resizeHandleSe && this.onResizeSeDown) {
        try {
          this.resizeHandleSe.removeEventListener('pointerdown', this.onResizeSeDown);
        } catch (e: any) {}
      }
      if (this.resizeActiveHandleEl && this.onResizeHandleMove) {
        try {
      ...
      this.onResizeHandleUp = null;
      this.resizeActiveHandleEl = null;
      if (this.resizerTrackRaf && typeof cancelAnimationFrame === 'function') {
        try {
          cancelAnimationFrame(this.resizerTrackRaf);
        } catch (e: any) {}
      }
      this.resizerTrackRaf = 0;
      if (this.resizeFlushRaf && typeof cancelAnimationFrame === 'function') {
        try {
  ...
  resizeHandleNw: any = null;
  resizeHandleNe: any = null;
  resizeHandleSw: any = null;
  ...
  scheduleResizeFlush = () => {
    if (this.resizeFlushRaf) return;
    if (typeof requestAnimationFrame === 'function') {
      this.resizeFlushRaf = requestAnimationFrame(this.flushResizeWriteBack);
    } else {
      this.resizeFlushRaf = 1;
  ...
  autoArrange = async (opts: any) => {
    ...
    if (!this.arrangeReady) {
      this.arrangeReady = import('rete-auto-arrange-plugin').then((m: any) => {
        ...
        this.arrange.addPreset(() => ({
          port: (data: any) => this.arrangePort(data)
        }));
    ...
    let result: any = null;
    try {
      const layoutOutcome = await this.arrange.layout({
