...
const FlowCanvas = forwardRef<FlowCanvasHandle, FlowCanvasProps>(function FlowCanvas(_props: FlowCanvasProps, ref): JSX.Element {
  ...
  const clearIncompatibleSockets = useRef<any>(null);
  ...
  function arrangePort(data: any) {
    const nodeView = area.current && area.current.nodeViews ? area.current.nodeViews.get(data.nodeId) : null;
    const nodeEl = nodeView && nodeView.element ? nodeView.element : null;
    const socketEl = socketReg.get(data.nodeId + '::' + data.side + '::' + data.key);
    const t = area.current && area.current.area ? area.current.area.transform : null;
    const k = t && t.k ? t.k : 1;
  ...
  async function autoArrange(opts: any) {
    ...
    if (!arrangeReady.current) {
      arrangeReady.current = import('rete-auto-arrange-plugin').then((m: any) => {
        arrange.current = new m.AutoArrangePlugin();
        ...
        // on all six.
        arrange.current.addPreset(() => ({
          port: (data: any) => arrangePort(data)
        }));
        area.current.use(arrange.current);
      });
      arrangeReady.current.catch(() => {
        arrangeReady.current = null;
      });
    }
    await arrangeReady.current;
    ...
    let result: any = null;
    try {
      const layoutOutcome = await arrange.current.layout({
  ...
  const portals = {
    ...
    connectionPlugin.current.addPipe((context: any) => {
      ...
      if (context.type === 'connectionpick') {
        ...
        if (!programmatic.current && _readonlyRef.current === true) return undefined;
        ...
        if (!programmatic.current && markIncompatibleSockets.current && context.data && context.data.socket) {
          markIncompatibleSockets.current(context.data.socket.nodeId, context.data.socket.side, context.data.socket.key);
        }
    ...
    AreaExtensions.restrictor(area.current, {
      scaling: () => ({
        min: typeof _minZoomRef.current === 'number' && _minZoomRef.current > 0 ? _minZoomRef.current : 0.01,
        max: typeof _maxZoomRef.current === 'number' && _maxZoomRef.current > 0 ? _maxZoomRef.current : 100
      })
    });
    ...
    const renderNode = (element: any, node: any) => {
      ...
      if (existing && existing.element === element) {
        ...
        resolveNodeBox(existing.box, meta, meta.type != null ? _typeRegRef.current[meta.type] : null);
      ...
      resolveNodeBox(box, meta, meta.type != null ? _typeRegRef.current[meta.type] : null);
    ...
    area.current.addPipe((context: any) => {
      ...
      if (programmatic.current) return context;
      if (context.type === 'nodepicked') {
        ...
        // core.pick + select.
        if (_selectableRef.current === false || _readonlyRef.current === true) return undefined;
        ...
        if (_readonlyRef.current === true) return undefined;
        ...
        const size = _snapGridRef.current;
        ...
        if (_pannableRef.current === false) return undefined;
        ...
        if (_zoomableRef.current === false) return undefined;
    ...
    if (container && typeof container.addEventListener === 'function') {
      ...
      const beginResize = (corner: any, handleEl: any) => (e: any) => {
        ...
        onResizeHandleUp.current = (ue: any) => {
          ...
          const hadMovement = resizeGestureActive.current;
          resizeGestureActive.current = false;
          pendingResizeSnapshot.current = null;
  ...
  }, [props.selectable]); // eslint-disable-line react-hooks/exhaustive-deps
  useEffect(() => {
