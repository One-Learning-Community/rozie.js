...
`;
   ...
   * @example
   * <rozie-number-field .modelValue=${qty} @model-value-change=${…} .min=${0} .max=${10}></rozie-number-field>
   */
...
<div class="${Object.entries({ "rozie-number-field": true, 'rozie-number-field--disabled': this.disabled }).filter(([, v]) => v).map(([k]) => k).join(' ')}" ${rozieSpread(this.$attrs)} ${rozieListeners(this.$listeners)} data-rozie-s-ceb089aa>
  ...
  <input class="rozie-number-field-input" type="text" inputmode="decimal" autocomplete="off" role="spinbutton" .value=${this.displayText()} ?disabled=${!!this.disabled} ?readonly=${!!this.readonly} aria-label=${rozieAttr(this.ariaLabel)} aria-valuemin=${rozieAttr(this.min)} aria-valuemax=${rozieAttr(this.max)} aria-valuenow=${rozieAttr(this.modelValue)} aria-valuetext=${rozieAttr(this.ariaText())} aria-disabled=${!!this.disabled} @input=${($event: InputEvent & { currentTarget: HTMLInputElement; target: HTMLInputElement }) => { this.onInput($event); }} @focus=${($event: FocusEvent & { currentTarget: HTMLInputElement; target: HTMLInputElement }) => { this.onFocus($event); }} @blur=${($event: FocusEvent & { currentTarget: HTMLInputElement; target: HTMLInputElement }) => { this.onBlur(); }} @keydown=${($event: KeyboardEvent & { currentTarget: HTMLInputElement; target: HTMLInputElement }) => { this.onKeydown($event); }} @pointerdown=${($event: PointerEvent & { currentTarget: HTMLInputElement; target: HTMLInputElement }) => { this.onScrubDown($event); }} @pointermove=${($event: PointerEvent & { currentTarget: HTMLInputElement; target: HTMLInputElement }) => { this.onScrubMove($event); }} @pointerup=${($event: PointerEvent & { currentTarget: HTMLInputElement; target: HTMLInputElement }) => { this.onScrubUp(); }} data-rozie-ref="input" data-rozie-s-ceb089aa />
