/**
 * compile-number-field-check.mjs — the live compile() collision/surface gate for
 * the NumberField family (run BEFORE any leaf work, the otp/slider discipline).
 *
...
if (process.exitCode) {
  console.error('\n✗ compile-number-field-check FAILED');
} else {
  console.log(`✓ NumberField surface OK: ${EXPECT.props.length} props (${EXPECT.models.length} model) / ${EXPECT.emits.length} emit / ${EXPECT.slots.length} slots / ${EXPECT.expose.length} expose; compile()×6 zero-error (focus = accepted warn-only ROZ137).`);
}
