 ...
 * engine for @rozie-ui/number-field.
 *
 ...
 *
 * NOTE: the number-field's `focus` $expose verb DELIBERATELY overrides the
 ...
 * consumers can `import '@rozie-ui/number-field-<fw>/themes/X.css'`.
 *
 * Steps:
 *   1. read src/NumberField.rozie
 ...
 *        (React only: also write NumberField.css + NumberField.d.ts)
 *   4. copy src/themes/ → each leaf src/themes/
 *   5. render each leaf README from the IR + the hand-kept event/handle manifests
 *   6. ENFORCE validateDocsPropsTable against docs/components/number-field.md
 ...
 * its src/NumberField.vue + themes + README (it never cleans the leaf src, so the
...
const ROOT = resolve(import.meta.dirname, '..'); // packages/ui/number-field
const REPO_ROOT = resolve(ROOT, '..', '..', '..'); // monorepo root
const SRC = resolve(ROOT, 'src/NumberField.rozie');
const FILENAME = 'NumberField.rozie';

/** Per-target leaf dir + emitted filename (build mode is informational). */
const TARGETS = {
  react: { dir: 'react', file: 'NumberField.tsx', build: 'tsdown' },
  vue: { dir: 'vue', file: 'NumberField.vue', build: 'source' },
  svelte: { dir: 'svelte', file: 'NumberField.svelte', build: 'source' },
  angular: { dir: 'angular', file: 'NumberField.ts', build: 'source' },
  solid: { dir: 'solid', file: 'NumberField.tsx', build: 'tsdown' },
  lit: { dir: 'lit', file: 'NumberField.ts', build: 'tsdown' },
};
...
function main() {
  ...
  for (const [target, cfg] of Object.entries(TARGETS)) {
    ...
    // `NumberField` the READMEs/consumers import (`export *` would NOT forward a
    // default). React/Solid also emit a named `NumberFieldHandle` interface (the
        ...
        (target === 'react' || target === 'solid') && ir.expose.length > 0
          ? `export { default as NumberField } from './NumberField';\n` +
            ...
            `/** The \`$expose\` imperative handle received via \`ref\` — { ${ir.expose
              .map((m) => m.name)
              .join(', ')} }. */\n` +
            `export type { NumberFieldHandle } from './NumberField';\n`
          : `export { default as NumberField } from './NumberField';\nexport { default } from './NumberField';\n`;
    ...
    // React-only sidecars.
    if (target === 'react') {
      if (r.css) writeFileSync(resolve(leafSrc, 'NumberField.css'), r.css);
      if (r.types) writeFileSync(resolve(leafSrc, 'NumberField.d.ts'), r.types);
    }
  ...
  const docsPath = resolve(REPO_ROOT, 'docs/components/number-field.md');
  ...
  validateDocsSurfaceNames(ir, 'number-field', REPO_ROOT);
