/**
 * surface.test.ts — the NumberField.rozie surface gate as a vitest test (so it
 * runs under `turbo run test`, not just scripts/compile-number-field-check.mjs).
 *
...
const SRC = resolve(HERE, '..', 'src', 'NumberField.rozie');
...
describe('NumberField.rozie surface gate', () => {
  ...
  it('expose surface matches (focus/increment/decrement/clear)', () => {
