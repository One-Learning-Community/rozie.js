/**
 * surface.test.ts — the FullCalendar.rozie surface gate as a vitest test (so it
 * runs under `turbo run test`, mirroring the cropper/pdf house pattern).
 *
 * Re-asserts:
 ...
 *
 * FullCalendar is the slot-heavy family (10 portal slots) — unlike cropper,
 * "declares no slots" becomes a positive slot-name assertion, and we keep an
 * explicit ROZ127 guard (no slot name may collide with a prop name).
 *
...
const SRC = resolve(HERE, '..', 'src', 'FullCalendar.rozie');
...
const EXPECT = {
  name: 'FullCalendar',
  props: [
    'events', 'view', 'weekends', 'editable', 'selectable', 'height',
    'defaultColor', 'locale', 'firstDay', 'slotDuration', 'nowIndicator',
  ...
  slots: [
    'event', 'dayCell', 'dayHeader', 'slotLabel', 'weekNumber',
    'nowIndicatorContent', 'moreLink', 'allDayContent', 'slotLaneContent',
    'noEventsContent',
  ],
  expose: [
    'getApi', 'changeView', 'addEvent', 'removeEvent', 'today', 'prev',
    'next', 'gotoDate', 'getDate', 'getEvents', 'scrollToTime', 'updateSize',
    'prevYear', 'nextYear', 'selectRange', 'clearSelection',
  ],
} as const;

const sorted = (a: readonly string[]) => [...a].sort();

describe('FullCalendar.rozie surface gate', () => {
  ...
  it('model:true props match (view)', () => {
  ...
  it('slots surface matches (10 portal slots)', () => {
  ...
  it('no slot name collides with a prop name (ROZ127)', () => {
    ...
    const propNames = ir.props.map((p: { name: string }) => p.name);
    expect(propNames.filter((p: string) => slotNames.has(p))).toEqual([]);
  });
