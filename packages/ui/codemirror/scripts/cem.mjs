 ...
 * custom element (`<rozie-code-mirror>`). With no manifest, neither VS Code nor
 ...
 *
 * Ported from `packages/ui/rete/scripts/cem.mjs` (D-05) — codemirror is a
 ...
 * repo's emitter uses, verified against the emitted CodeMirror.ts:
 ...
 *   - source events   = ir.emits, dispatched verbatim as CustomEvents (empty
 *     for codemirror — D-08, no events)
 *
...
/**
 * Build the Custom Elements Manifest document for the Lit leaf.
 *
 * @param {object}  opts
 * @param {object}  opts.ir            once-parsed IR (props/emits/slots/expose)
 * @param {string}  opts.componentName class / symbol name (e.g. CodeMirror)
...
export function buildCustomElementsManifest({
  ...
  // Events: source emits (dispatched verbatim, empty for codemirror — D-08) +
