 ...
 * v-model is inferred by the IDE from the `value` prop + the `update:value`
 * event (both emitted below) — no separate `vue-model` object needed.
 *
 * Ported from `packages/ui/rete/scripts/web-types.mjs` (D-05) — codemirror is a
 * single-component family, so no multi-component merge is needed at the call
 * site (unlike rete's FlowCanvas/NodeType/Port splice).
 *
 ...
 * @param {string}  opts.pkgName       leaf package name (e.g. @rozie-ui/codemirror-vue)
 * @param {string}  opts.version       leaf version (from its package.json)
 * @param {string}  opts.componentName imported symbol / tag (e.g. CodeMirror)
...
export function buildWebTypes({
  ...
  // Events: the source emits (empty for codemirror, D-08) + the model writeback
  // event (drives v-model:value).
