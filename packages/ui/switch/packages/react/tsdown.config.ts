...
export default defineConfig({
  ...
  dts: true,
  clean: true,
  // The generated barrel re-exports both the named component (`Switch`) and its
  ...
  // The generated component does a side-effect `import './Switch.css'`; mark it
  // external and copy the file into dist so the relative specifier resolves at
  // the consumer's bundler.
  copy: [{ from: 'src/Switch.css', to: 'dist', flatten: true }],
});
