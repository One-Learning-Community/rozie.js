 ...
 *
 * NOTE: the switch's `focus` $expose verb DELIBERATELY overrides the inherited
 ...
 * consumers can `import '@rozie-ui/switch-<fw>/themes/X.css'`.
 *
 * Steps:
 *   1. read src/Switch.rozie
 ...
 * its src/Switch.vue + themes + README (it never cleans the leaf src, so the
...
const ROOT = resolve(import.meta.dirname, '..'); // packages/ui/switch
const REPO_ROOT = resolve(ROOT, '..', '..', '..'); // monorepo root
const SRC = resolve(ROOT, 'src/Switch.rozie');
const FILENAME = 'Switch.rozie';

/** Per-target leaf dir + emitted filename (build mode is informational). */
const TARGETS = {
  react: { dir: 'react', file: 'Switch.tsx', build: 'tsdown' },
  vue: { dir: 'vue', file: 'Switch.vue', build: 'source' },
  svelte: { dir: 'svelte', file: 'Switch.svelte', build: 'source' },
  angular: { dir: 'angular', file: 'Switch.ts', build: 'source' },
  solid: { dir: 'solid', file: 'Switch.tsx', build: 'tsdown' },
  lit: { dir: 'lit', file: 'Switch.ts', build: 'tsdown' },
};
...
function main() {
  ...
  for (const [target, cfg] of Object.entries(TARGETS)) {
    ...
    // DEFAULT export, so the barrel re-exports the default under the named `Switch`
    // the READMEs/consumers import (`export *` would NOT forward a default).
    // React/Solid also emit a named `SwitchHandle` interface (the `$expose`
        ...
        (target === 'react' || target === 'solid') && ir.expose.length > 0
          ? `export { default as Switch } from './Switch';\n` +
            ...
            `/** The \`$expose\` imperative handle received via \`ref\` — { ${ir.expose
              .map((m) => m.name)
              .join(', ')} }. */\n` +
            `export type { SwitchHandle } from './Switch';\n`
          : `export { default as Switch } from './Switch';\nexport { default } from './Switch';\n`;
    ...
    // React-only sidecars.
    if (target === 'react') {
      if (r.css) writeFileSync(resolve(leafSrc, 'Switch.css'), r.css);
      if (r.types) writeFileSync(resolve(leafSrc, 'Switch.d.ts'), r.types);
    }
  ...
  const docsPath = resolve(REPO_ROOT, 'docs/components/switch.md');
  ...
  validateDocsSurfaceNames(ir, 'switch', REPO_ROOT);
