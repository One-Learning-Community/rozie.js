/**
 * compile-switch-check.mjs — the live compile() collision/surface gate for the Switch
...
if (process.exitCode) {
  console.error('\n✗ compile-switch-check FAILED');
} else {
  console.log(`✓ Switch surface OK: ${EXPECT.props.length} props (${EXPECT.models.length} model) / ${EXPECT.emits.length} emits / ${EXPECT.slots.length} slots / ${EXPECT.expose.length} expose; compile()×6 zero-error (focus = accepted warn-only ROZ137).`);
}
