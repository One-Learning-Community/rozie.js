/**
 * surface.test.ts — the Switch.rozie surface gate as a vitest test (so it runs
 * under `turbo run test`, not just the standalone
 * scripts/compile-switch-check.mjs).
 *
...
const SRC = resolve(HERE, '..', 'src', 'Switch.rozie');
const FILENAME = 'Switch.rozie';
const source = readFileSync(SRC, 'utf8');

const EXPECT = {
  name: 'Switch',
  props: ['modelValue', 'disabled', 'readonly', 'ariaLabel'],
  models: ['modelValue'],
  emits: ['change'],
  slots: [''] as string[],
  expose: ['focus', 'toggle'],
...
describe('Switch.rozie surface gate', () => {
  ...
  it('declares the default scoped slot', () => {
    const slotNames = ir.slots.map((s: { name: string }) => s.name);
    expect(sorted(slotNames)).toEqual(sorted(EXPECT.slots));
  });

  it('the default slot name does not collide with any prop key (ROZ127)', () => {
    const propNames = new Set(ir.props.map((p: { name: string }) => p.name));
    const slotNames = ir.slots.map((s: { name: string }) => s.name);
    // The default slot is the empty name '' — and no prop is named '' — so the
    // slot==prop collision (ROZ127, Svelte) cannot occur.
    expect(slotNames.filter((n: string) => propNames.has(n))).toEqual([]);
  });

  it('expose surface matches (focus/toggle)', () => {
  ...
  it('no expose-verb collides with an emit (ROZ121) or the React modelValue-model setter (ROZ524)', () => {
    const exposeNames = ir.expose.map((e: { name: string }) => e.name);
