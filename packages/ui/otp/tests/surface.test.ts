/**
 * surface.test.ts — the Otp.rozie surface gate as a vitest test (so it runs
 * under `turbo run test`, not just the standalone scripts/compile-otp-check.mjs).
 *
...
const SRC = resolve(HERE, '..', 'src', 'Otp.rozie');
...
describe('Otp.rozie surface gate', () => {
  ...
  it('emits surface matches (change/complete)', () => {
