 ...
 *
 * NOTE: the otp's `focus` $expose verb DELIBERATELY overrides the inherited
...
const ROOT = resolve(import.meta.dirname, '..'); // packages/ui/otp
...
const TARGETS = {
  react: { dir: 'react', file: 'Otp.tsx', build: 'tsdown' },
  vue: { dir: 'vue', file: 'Otp.vue', build: 'source' },
  svelte: { dir: 'svelte', file: 'Otp.svelte', build: 'source' },
  angular: { dir: 'angular', file: 'Otp.ts', build: 'source' },
  solid: { dir: 'solid', file: 'Otp.tsx', build: 'tsdown' },
  lit: { dir: 'lit', file: 'Otp.ts', build: 'tsdown' },
};
...
function copyInternal(leafSrc) {
  ...
  if (!existsSync(src))
    throw new Error('codegen: src/internal/ not found (the write model must exist)');
...
function main() {
  ...
  for (const [target, cfg] of Object.entries(TARGETS)) {
    ...
    // DEFAULT export, so the barrel re-exports the default under the named `Otp`
    // the READMEs/consumers import (`export *` would NOT forward a default).
    // React/Solid also emit a named `OtpHandle` interface (the `$expose`
        ...
        (target === 'react' || target === 'solid') && ir.expose.length > 0
          ? `export { default as Otp } from './Otp';\n` +
            ...
            `/** The \`$expose\` imperative handle received via \`ref\` — { ${ir.expose
              .map((m) => m.name)
              .join(', ')} }. */\n` +
            `export type { OtpHandle } from './Otp';\n`
          : `export { default as Otp } from './Otp';\nexport { default } from './Otp';\n`;
    ...
    // React-only sidecars.
    if (target === 'react') {
      if (r.css) writeFileSync(resolve(leafSrc, 'Otp.css'), r.css);
  ...
  const docsPath = resolve(REPO_ROOT, 'docs/components/otp.md');
  ...
  validateDocsSurfaceNames(ir, 'otp', REPO_ROOT);
