/**
 * README rendering + docs-table validation for @rozie-ui/otp.
 *
 * Everything structural is derived from a SINGLE parse of Otp.rozie
...
// `value` model prop IS the control value — an OTP is a form control.
...
export class DemoComponent {
  ...
  solid: {
    lang: 'tsx',
    code: `import { Otp, type OtpHandle } from '@rozie-ui/otp-solid';
...
export function renderReadme(target, ir, eventManifest, pkgName, handleManifest = {}) {
  ...
  lines.push('');
  lines.push(
    `Idiomatic **${target}** \`Otp\` — a headless, fully-accessible (WAI-ARIA) ` +
      `one-time-code / PIN input (segmented native cells, paste-to-distribute, full ` +
      `keyboard navigation, \`autocomplete="one-time-code"\` SMS autofill, and optional ` +
      `masking) compiled from one ` +
      `[Rozie](https://github.com/One-Learning-Community/rozie.js) source. ` +
      `The interaction engine IS the browser's native \`<input>\` cells; ` +
  ...
  lines.push('');
  lines.push(
    'Every visual value is a `--rozie-otp-*` CSS custom property — override any of them at ' +
  ...
  if (target === 'angular' && modelProps.length === 1) {
    ...
    lines.push(
      `The generated class implements \`ControlValueAccessor\` — the \`${modelProp.name}\` model ` +
        'prop is the control value, so an OTP **is** a form control. It binds to template-driven ' +
  ...
  if (ir.expose && ir.expose.length > 0) {
    ...
    lines.push(
      'Beyond props, the component exposes imperative methods (declared once in the Rozie source ' +
        ...
        'first empty cell) — on the Lit custom element this is an accepted ROZ137 warn-only ' +
