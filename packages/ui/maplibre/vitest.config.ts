// Vitest config for @rozie-ui/maplibre.
//
...
// re-asserts the MapLibre.rozie compile()/lowerToIR surface (the same contract
// the standalone scripts/compile-maplibre-check.mjs checks), so a drift in the
// 25-prop / 22-emit / 3-slot / 8-expose surface or a new compile() error
