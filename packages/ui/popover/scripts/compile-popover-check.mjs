/**
 * compile-popover-check.mjs — the live compile() collision/surface gate for the
 * Popover family (run BEFORE any leaf work, the otp/slider/cropper discipline).
 *
...
const EXPECT = {
  name: 'Popover',
  props: ['open', 'placement', 'trigger', 'offset', 'disableFlip', 'disableShift', 'arrow', 'disabled', 'modal', 'strategy', 'bare', 'disablePositioning', 'keepMounted', 'matchWidth', 'disableDismiss'],
  models: ['open'],
  emits: ['change'],
  slots: ['', 'anchor'],
...
if (!setEq(propNames, EXPECT.props))
...
// `update` is a reserved Lit lifecycle method — assert we never expose it bare.
if (exposeNames.includes('update')) fail('expose verb `update` clobbers the Lit ReactiveElement.update lifecycle — use `reposition`.');
...
if (process.exitCode) {
  console.error('\n✗ compile-popover-check FAILED');
} else {
  console.log(
    `✓ Popover surface OK: ${EXPECT.props.length} props (${EXPECT.models.length} model) / ${EXPECT.emits.length} emit / ${EXPECT.slots.length} slots / ${EXPECT.expose.length} expose; compile()×6 zero-error.`,
  );
}
