/**
 * compile-pdf-check.mjs — the live compile() collision/surface gate for the PDF.js
 * port (run BEFORE any leaf work, the cropper/maplibre discipline).
 *
...
// event⇄verb (ROZ121) + model-setter (ROZ524) collision guards asserted directly.
...
if (process.exitCode) {
  console.error('\n✗ compile-pdf-check FAILED');
} else {
  console.log(`✓ PdfViewer surface OK: ${EXPECT.props.length} props (${EXPECT.models.length} model) / ${EXPECT.emits.length} emits / ${EXPECT.slots.length} slots / ${EXPECT.expose.length} expose; compile()×6 zero-error.`);
}
