/**
 * compile-tags-check.mjs — the live compile() collision/surface gate for the
 * Tags family (run BEFORE any leaf work, the otp/captcha discipline).
 *
...
const EXPECT = {
  name: 'Tags',
  props: ['modelValue', 'delimiters', 'allowDuplicates', 'max', 'disabled', 'readonly', 'validate', 'placeholder', 'ariaLabel'],
  models: ['modelValue'],
...
// slot==prop (ROZ127), event⇄verb (ROZ121) + model-setter (ROZ524) collisions asserted directly.
...
if (process.exitCode) {
  console.error('\n✗ compile-tags-check FAILED');
} else {
  console.log(`✓ Tags surface OK: ${EXPECT.props.length} props (${EXPECT.models.length} model) / ${EXPECT.emits.length} emits / ${EXPECT.slots.length} slots / ${EXPECT.expose.length} expose; compile()×6 zero-error.`);
}
