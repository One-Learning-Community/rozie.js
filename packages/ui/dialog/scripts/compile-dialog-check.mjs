/**
 * compile-dialog-check.mjs — the live compile() collision/surface gate for the
 * Dialog family (run BEFORE any leaf work, the slider/captcha/otp discipline).
 *
 ...
 *      component deliberately wraps the native <dialog> element, so each target
 *      emits one WARN-only ROZ922 ("`<dialog>` looks like component `<Dialog>`")
 *      which is filtered out by the error-severity gate (accepted + intended).
...
// `open` is BOTH the model prop AND a tempting verb name; assert the verbs avoid it.
const verbModelClash = exposeNames.filter((v) => modelNames.includes(v));
if (verbModelClash.length) fail(`expose-verb == model-prop collision: [${verbModelClash.join(', ')}] (use show/hide, not open/close)`);
...
if (process.exitCode) {
  console.error('\n✗ compile-dialog-check FAILED');
} else {
  console.log(`✓ Dialog surface OK: ${EXPECT.props.length} props (${EXPECT.models.length} model) / ${EXPECT.emits.length} emits / ${EXPECT.slots.length} slot / ${EXPECT.expose.length} expose; compile()×6 zero-error (ROZ922 native-<dialog> wrap = accepted warn-only ×6).`);
}
