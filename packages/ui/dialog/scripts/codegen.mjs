 ...
 *
 * NOTE: the component DELIBERATELY wraps the native `<dialog>` element, so each
 * target emits exactly one ROZ922 WARNING ("`<dialog>` looks like component
 * `<Dialog>` but is lowercase"). ROZ922 is warn-only — the severity filter below
 * keeps only `error`-severity diagnostics, so the intentional native-element
 * wrap does NOT throw codegen (the slider/otp `focus`-warn precedent). We do NOT
 ...
 * its src/Dialog.vue + themes + README (it never cleans the leaf src, so the
...
const ROOT = resolve(import.meta.dirname, '..'); // packages/ui/dialog
const REPO_ROOT = resolve(ROOT, '..', '..', '..'); // monorepo root
const SRC = resolve(ROOT, 'src/Dialog.rozie');
const FILENAME = 'Dialog.rozie';

/** Per-target leaf dir + emitted filename (build mode is informational). */
const TARGETS = {
  react: { dir: 'react', file: 'Dialog.tsx', build: 'tsdown' },
  vue: { dir: 'vue', file: 'Dialog.vue', build: 'source' },
  svelte: { dir: 'svelte', file: 'Dialog.svelte', build: 'source' },
  angular: { dir: 'angular', file: 'Dialog.ts', build: 'source' },
  solid: { dir: 'solid', file: 'Dialog.tsx', build: 'tsdown' },
  lit: { dir: 'lit', file: 'Dialog.ts', build: 'tsdown' },
};
...
function main() {
  ...
  for (const [target, cfg] of Object.entries(TARGETS)) {
    ...
    // `Dialog` the READMEs/consumers import (`export *` would NOT forward a
    // default). React/Solid also emit a named `DialogHandle` interface (the
    ...
    // React-only sidecars.
    if (target === 'react') {
      if (r.css) writeFileSync(resolve(leafSrc, 'Dialog.css'), r.css);
