/**
 * surface.test.ts — the Dialog.rozie surface gate as a vitest test (so it runs
 * under `turbo run test`, not just the standalone
 * scripts/compile-dialog-check.mjs).
 *
 ...
 *      DELIBERATE native-<dialog> wrap is warn-only ROZ922 (accepted ×6), so it
 *      does not trip the error gate.
 *
...
const SRC = resolve(HERE, '..', 'src', 'Dialog.rozie');
...
describe('Dialog.rozie surface gate', () => {
  ...
  it('model:true props match (open)', () => {
  ...
  it('emits surface matches (close)', () => {
  ...
  it('expose surface matches (show/hide)', () => {
  ...
  it('verbs avoid the `open` model prop and `@close` event (use show/hide)', () => {
    const exposeNames = ir.expose.map((e: { name: string }) => e.name);
    expect(exposeNames).not.toContain('open');
    expect(exposeNames).not.toContain('close');
  });

  it('no expose-verb collides with an emit (ROZ121) or the React open-model setter (ROZ524)', () => {
    const exposeNames = ir.expose.map((e: { name: string }) => e.name);
