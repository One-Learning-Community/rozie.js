/**
 * compile-cropper-check.mjs — the live compile() collision/surface gate for the
 * Cropper.js port (run BEFORE any leaf work, the TipTap/Chart.js/MapLibre
 * discipline).
 *
...
if (process.exitCode) {
  console.error('\n✗ compile-cropper-check FAILED');
} else {
  console.log(`✓ Cropper surface OK: ${EXPECT.props.length} props (${EXPECT.models.length} model) / ${EXPECT.emits.length} emits / ${EXPECT.slots.length} slots / ${EXPECT.expose.length} expose; compile()×6 zero-error.`);
}
