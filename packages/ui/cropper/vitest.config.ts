// Vitest config for @rozie-ui/cropper.
//
...
// re-asserts the Cropper.rozie compile()/lowerToIR surface (the same contract the
// standalone scripts/compile-cropper-check.mjs checks), so a drift in the 20-prop
// / 6-emit / 0-slot / 18-expose surface or a new compile() error diagnostic fails
...
// a load-tolerant FAILSAFE (the maplibre analog).
