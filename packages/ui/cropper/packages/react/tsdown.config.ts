...
export default defineConfig({
  ...
  dts: true,
  clean: true,
  // The generated barrel re-exports both the named component (`Cropper`) and its
  ...
  // (`./Cropper.css` — the attribute-scoped component styles). tsdown's css-guard
  ...
  // resolves at the consumer's own bundler. (Cropper ships no `:root` engine-DOM
  // escape-hatch rules, so there is no `Cropper.global.css` — unlike maplibre.)
  external: ['react', 'react-dom', '@rozie/runtime-react', 'cropperjs', /\.css$/],
  copy: [{ from: 'src/Cropper.css', to: 'dist', flatten: true }],
});
