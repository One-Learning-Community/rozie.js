 ...
 *
 * This is the first @rozie-ui family with NO third-party vanilla engine, so
 ...
 * into each leaf so consumers can `import '@rozie-ui/listbox-<fw>/themes/X.css'`.
 *
 * Steps:
 *   1. read src/Listbox.rozie
 ...
 *        (React only: also write Listbox.css + Listbox.d.ts)
 *   4. copy src/themes/ → each leaf src/themes/
 *   5. render each leaf README from the IR + the hand-kept event/handle manifests
 *   6. ENFORCE validateDocsPropsTable against docs/components/listbox.md
...
const ROOT = resolve(import.meta.dirname, '..'); // packages/ui/listbox
const REPO_ROOT = resolve(ROOT, '..', '..', '..'); // monorepo root
const SRC = resolve(ROOT, 'src/Listbox.rozie');
const FILENAME = 'Listbox.rozie';

/** Per-target leaf dir + emitted filename (build mode is informational). */
const TARGETS = {
  react: { dir: 'react', file: 'Listbox.tsx', build: 'tsdown' },
  vue: { dir: 'vue', file: 'Listbox.vue', build: 'source' },
  svelte: { dir: 'svelte', file: 'Listbox.svelte', build: 'source' },
  angular: { dir: 'angular', file: 'Listbox.ts', build: 'source' },
  solid: { dir: 'solid', file: 'Listbox.tsx', build: 'tsdown' },
  lit: { dir: 'lit', file: 'Listbox.ts', build: 'tsdown' },
};
...
function main() {
  ...
  for (const [target, cfg] of Object.entries(TARGETS)) {
    ...
    // `Listbox` the READMEs/consumers import (`export *` would NOT forward a
    // default). React/Solid also emit a named `ListboxHandle` interface (the
        ...
        (target === 'react' || target === 'solid') && ir.expose.length > 0
          ? `export { default as Listbox } from './Listbox';\n` +
            ...
            `/** The \`$expose\` imperative handle received via \`ref\` — { ${ir.expose
              .map((m) => m.name)
              .join(', ')} }. */\n` +
            `export type { ListboxHandle } from './Listbox';\n`
          : `export { default as Listbox } from './Listbox';\nexport { default } from './Listbox';\n`;
    ...
    // React-only sidecars.
    if (target === 'react') {
      if (r.css) writeFileSync(resolve(leafSrc, 'Listbox.css'), r.css);
      if (r.types) writeFileSync(resolve(leafSrc, 'Listbox.d.ts'), r.types);
    }
  ...
  const docsPath = resolve(REPO_ROOT, 'docs/components/listbox.md');
  ...
  validateDocsSurfaceNames(ir, 'listbox', REPO_ROOT);
