/**
 * README rendering + docs-table validation for @rozie-ui/listbox.
 *
 * Everything structural is derived from a SINGLE parse of Listbox.rozie
...
export function Demo() {
  ...
  lit: {
    lang: 'ts',
    code: `import '@rozie-ui/listbox-lit';
...
// source has exactly one `model: true` prop (the emitter's CVA gate).
...
export class DemoComponent {
  ...
  solid: {
    lang: 'tsx',
    code: `import { Listbox, type ListboxHandle } from '@rozie-ui/listbox-solid';
...
export function renderReadme(target, ir, eventManifest, pkgName, handleManifest = {}) {
  ...
  lines.push('');
  lines.push(
    'Every visual value is a `--rozie-listbox-*` CSS custom property — override any of them at ' +
  ...
  if (target === 'angular' && modelProps.length === 1) {
    ...
    lines.push(
      `The generated class implements \`ControlValueAccessor\` — the \`${modelProp.name}\` model ` +
        ...
        'directly, with no wrapper directive:',
    );
    lines.push('');
