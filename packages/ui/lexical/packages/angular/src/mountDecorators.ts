...
//
// AOT NOTE: the leading `@` in the pill text is written as the `&#64;` HTML entity.
// Angular v17+ reserves a bare `@` for control-flow blocks (`@if`/`@for`/…), so
// `@{{ label }}` fails ngtsc with NG5002 ("Unclosed block"). `&#64;` renders the
// literal "@" and sidesteps the control-flow parser (surfaced at the first real
// ng-packagr AOT build in 76-06 — the 76-04 gate was compile-only).
