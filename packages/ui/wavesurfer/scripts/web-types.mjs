/**
 * web-types.mjs — build a JetBrains `web-types.json` for the Vue leaf from the
 ...
 * v-model is inferred by the IDE from the model prop + its `update:<prop>`
 * event (both emitted below) — no separate `vue-model` object needed.
 *
 * Ported from `packages/ui/codemirror/scripts/web-types.mjs` (D-05) —
 * wavesurfer is a single-component family, so no multi-component merge is
 * needed at the call site (unlike chartjs's nine-component or rete's
 * FlowCanvas/NodeType/Port splice).
 *
...
/** Build the scoped-slot `type` string from a slot's params (`{ view: any; pos: any }`). */
...
/**
 * Build the web-types document object for the Vue leaf.
 *
 * @param {object}  opts
 * @param {object}  opts.ir            once-parsed IR (props/emits/slots)
 * @param {string}  opts.pkgName       leaf package name (e.g. @rozie-ui/wavesurfer-vue)
 * @param {string}  opts.version       leaf version (from its package.json)
 * @param {string}  opts.componentName imported symbol / tag (e.g. Waveform)
...
export function buildWebTypes({
  ...
  // Events: the source emits + the model writeback event(s) (drives v-model:<prop>).
