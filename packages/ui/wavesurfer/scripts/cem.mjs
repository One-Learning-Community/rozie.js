 ...
 * custom element (`<rozie-waveform>`). With no manifest, neither VS Code nor
 ...
 *
 * Ported from `packages/ui/codemirror/scripts/cem.mjs` (D-05) — wavesurfer is a
 * single-component family, so no multi-component merge is needed at the call
 * site (unlike chartjs's nine-component or rete's FlowCanvas/NodeType/Port
 * splice).
 *
 * Everything here is derivable from the IR + the Lit emit conventions this
 * repo's emitter uses, verified against the emitted Waveform.ts:
 ...
 * @param {string}  opts.componentName class / symbol name (e.g. Waveform)
