/**
 * surface.test.ts — the Waveform.rozie surface gate as a vitest test (so it runs
 * under `turbo run test`, not just the standalone scripts/compile-wavesurfer-check.mjs).
 *
...
describe('Waveform.rozie surface gate', () => {
  ...
  it('no expose-verb collides with an emit (ROZ121) or the React currentTime-model setter (ROZ524)', () => {
    const exposeNames = ir.expose.map((e: { name: string }) => e.name);
