/**
 * compile-pagination-check.mjs — the live compile() collision/surface gate for
 * the Pagination family (run BEFORE any leaf work, the otp/captcha discipline).
 *
...
const EXPECT = {
  ...
  models: ['modelValue'],
  emits: ['change'],
  slots: ['prevControl', 'nextControl', 'item', 'ellipsis'],
...
if (process.exitCode) {
  console.error('\n✗ compile-pagination-check FAILED');
} else {
  console.log(`✓ Pagination surface OK: ${EXPECT.props.length} props (${EXPECT.models.length} model) / ${EXPECT.emits.length} emits / ${EXPECT.slots.length} slots / ${EXPECT.expose.length} expose; compile()×6 zero-error.`);
}
