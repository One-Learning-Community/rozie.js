/**
 * surface.test.ts — the Captcha.rozie surface gate as a vitest test (so it runs
 * under `turbo run test`, not just the standalone scripts/compile-captcha-check.mjs).
 *
...
const SRC = resolve(HERE, '..', 'src', 'Captcha.rozie');
...
describe('Captcha.rozie surface gate', () => {
  ...
  it('emits surface matches (verify/expire/error)', () => {
  ...
  it('expose surface matches (reset/execute/getResponse)', () => {
