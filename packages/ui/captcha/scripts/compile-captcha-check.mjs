/**
 * compile-captcha-check.mjs — the live compile() collision/surface gate for the
 * Captcha port (run BEFORE any leaf work, the Cropper/TipTap/MapLibre discipline).
 *
...
const SRC = resolve(ROOT, 'src/Captcha.rozie');
const FILENAME = 'Captcha.rozie';
const source = readFileSync(SRC, 'utf8');

const EXPECT = {
  name: 'Captcha',
  props: ['provider', 'sitekey', 'token', 'theme', 'size', 'tabindex', 'options'],
  models: ['token'],
...
if (process.exitCode) {
  console.error('\n✗ compile-captcha-check FAILED');
} else {
  console.log(`✓ Captcha surface OK: ${EXPECT.props.length} props (${EXPECT.models.length} model) / ${EXPECT.emits.length} emits / ${EXPECT.slots.length} slots / ${EXPECT.expose.length} expose; compile()×6 zero-error.`);
}
