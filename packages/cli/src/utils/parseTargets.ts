...
export type Target = 'vue' | 'react' | 'svelte' | 'angular';

export const VALID_TARGETS = new Set<Target>(['vue', 'react', 'svelte', 'angular']);
  ...
  for (const t of tokens) {
    if (!VALID_TARGETS.has(t as Target)) {
      throw new InvalidArgumentError(
        `[ROZ850] unknown target '${t}' (expected vue|react|svelte|angular)`,
      );
    }
  }
