    ...
    if (
      ...
      diagnostics.push({
        ...
        message: componentHost
          ? "The template's only root is a component tag, so auto-fallthrough has no HTML element to attach the inherited listeners to — they are dropped."
        ...
        loc,
        hint: componentHost
          ? 'Move the r-match onto a wrapping HTML element, or set inherit-listeners="false" on the <rozie> tag — a component-tag root owns its own fallthrough surface, so the spread would land as a prop on the inner component rather than as a DOM attribute.'
          : 'Move the gating condition onto a child so the root element is unconditional, or set inherit-listeners="false" on the <rozie> tag and apply the manual r-on="$listeners" spread inside the branch.',
      });
    }
  }
