...
export const RozieErrorCode = {
  ...
  // ROZ144 (STYLE_BINDING_ARRAY_UNSUPPORTED) was RETIRED by quick task
  // 260717-uvk: array-form `:style="[a, b]"` merge is now uniformly supported
  // on all 6 targets (react/solid `parseInlineStyle`, lit/svelte `rozieStyle`,
  // angular `[attr.style]="__rozieMergeStyle(...)"`, vue native), so the
  // fail-loud guard that used to point authors at the object/`$computed`
  // workaround is gone. The code is intentionally left RETIRED (not reused)
  // to avoid confusing historical diagnostics/logs that recorded ROZ144.

  // ROZ145 is the next free code after ROZ143 in the 100 semantic-binding
  ...
  RESERVED_IDENTIFIER_COLLISION: 'ROZ202', // error — <data> field or r-for loop var named $el / $props / $data / $refs / $slots / $emit / $event / $attrs / $listeners / $restoreFocus / $model / $expose / $provide / $inject / $clone / $slotted / $snapshot / $classSelector. ($event is the closure-param name for event-handler emits — see emitTemplate's `($event) =>` convention in target-{react,svelte,solid,lit}.) Wired in semantic/validators/reservedIdentifierValidator.ts — keep RESERVED_SIGILS there in sync with this list.
  ...
  // NOTE: ROZ980 (RFOR_SLOT_NAME_COLLISION) was RETIRED. The r-for-loop-var ==
  // slot-name collision is now AUTO-FIXED in the Svelte emitter (rename the
  // emitter-generated snippet binding to `<name>$$slot`) rather than warned —
