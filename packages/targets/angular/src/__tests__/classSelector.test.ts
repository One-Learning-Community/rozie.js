// Phase 13 Plan 13-01 (Wave 0 RED scaffold) — Angular `$classSelector` emit test.
//
...
//
// Angular keeps authored class names LITERAL in the emitted DOM (style isolation
...
/** Compile `examples/ClassSelectorProbe.rozie` to Angular standalone-component output. */
function compileProbe(): { code: string; diagnostics: Diagnostic[] } {
  ...
  const emitted = emitAngular(ir, { filename, source, modifierRegistry: registry });
  return {
    code: emitted.code,
...
describe('$classSelector emit (Angular) [Wave 0 RED — implemented in Wave 2]', () => {
  ...
  it('R2: :attr-position $classSelector(\'panel\') lowers to the literal `.panel`', () => {
    ...
    // The lowered literal is single-quoted (`'.panel'`) in the :attr path: an
    // Angular `[attr]="..."` binding double-quotes its expression, so the
    // literal must use single quotes to avoid colliding with that wrapper.
    // Quote style is incidental to lowering correctness — match either.
