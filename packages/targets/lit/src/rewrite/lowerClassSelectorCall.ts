/**
 * lowerClassSelectorCall — Phase 13 Plan 13-05 (Lit target).
 *
 * Lowers a `$classSelector('<class>')` `CallExpression` to Lit's compile-time
 * CSS-selector form: the string literal `".<class>"`.
 *
 * Lit keeps authored class names LITERAL in the emitted DOM (style isolation
 * via a `[data-rozie-s-<hash>]` attribute — the original class name is left
 * intact), so a class passed to a third-party engine matches the rendered DOM
 * as written. The helper therefore lowers to a plain `StringLiteral` — no
 * runtime lookup. (React is the lone exception — it lowers to a runtime
 * `"." + styles.<class>` expression because CSS Modules hash its class names.)
 *
