 ...
 *
 * Mirrored byte-identically (in logic) from the Angular target, matching the
...
const TARGET = 'lit' as const;

/**
 * Drives the REAL emitter entrypoint from SOURCE (parse -> lowerToIR -> emitLit).
...
$onMount(() => { go() })
  ...
  return emitLit(lowered.ir, { filename: 'Probe.rozie' }).code;
}
