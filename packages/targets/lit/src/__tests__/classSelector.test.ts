// Phase 13 Plan 13-01 (Wave 0 RED scaffold) — Lit `$classSelector` emit test.
//
...
/** Compile `examples/ClassSelectorProbe.rozie` to Lit `.ts` output. */
function compileProbe(): { code: string; diagnostics: Diagnostic[] } {
  ...
  const emitted = emitLit(ir, { filename, source, modifierRegistry: registry });
  return {
    code: emitted.code,
...
describe('$classSelector emit (Lit) [Wave 0 RED — implemented in Wave 2]', () => {
