// Phase 9 Plan 09-04 Task 1 — Lit `<script lang="ts">` author-annotation
// survival + statement-position type hoisting (OQ-3 / RESEARCH Pattern 5).
//
// Lit emits a class-based custom element. Its `partitionUserImports` already
// hoists `import` statements out of the class body, but a
// `TSInterfaceDeclaration` / `TSTypeAliasDeclaration` declared in `<script>`
// would fall through `partitionScript` → `classBodyFromStatements` into the
// `freeChunks` bucket, landing inside `firstUpdated()` — and an
// `interface`/`type` declaration inside a method body is a TypeScript syntax
// error (TS1184).
//
// This file asserts:
...
//       above the `class extends ...LitElement...` — NOT in the class body,
...
/** Compile an inline `.rozie` source string to Lit `.ts` output. */
function compileSource(source: string, filename = 'TsPassthrough.rozie'): string {
  ...
  return emitLit(ir, { filename, source, modifierRegistry: registry }).code;
}

/** Compile a flat `examples/*.rozie` fixture to Lit `.ts` output. */
...
describe('Lit <script lang="ts"> passthrough — Plan 09-04', () => {
  it('emits a typed component where author annotations survive verbatim', () => {
    ...
    expect(code).toMatch(/describe\s*\(\s*item\s*:\s*Item\s*\)\s*:\s*string/);
  ...
  it('hoists a <script>-declared interface to MODULE scope, not the class body', () => {
    ...
    const classIdx = code.indexOf('class TypedCounter extends');
    ...
    // module scope. If it landed in `firstUpdated()` it would appear AFTER
  ...
  it('does not leave an interface/type declaration inside the class body', () => {
    const code = compileSource(TYPED_SOURCE);
    // Slice the text from the class opening to end-of-file and confirm no
    // statement-position type declaration leaked into the class body.
    const classOpen = code.indexOf('class TypedCounter extends');
