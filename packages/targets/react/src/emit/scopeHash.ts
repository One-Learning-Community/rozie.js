 ...
 *
 * Why this exists: React's CSS Modules pipeline only hashes class names.
 * Bare element selectors (`button { ... }`) survive un-prefixed and apply
 * globally. To match Vue/Svelte/Angular/Lit component-scoped semantics,
 * the React emitter rewrites every selector to include a unique
 * attribute (`button[data-rozie-s-<hash>]`) AND injects the same
 * attribute on every host element rendered by the component.
 *
 * Hash derivation:
 ...
 *     babel-plugin sibling-write) byte-parity-equivalent, which the
 *     dist-parity strict-bytes test enforces (D-93).
 *   - Fall back to the component's IR name when no filename is provided
 *     (test-only paths that synthesize IR without a real file).
 *
