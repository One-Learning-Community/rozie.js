// Phase 16 Plan 16-03 — React `$restoreFocus` emit test.
//
// `$restoreFocus(selector, idx)` is the Phase 16 author-surface sigil. React's
// keyed reconciler MOVES the existing DOM element on reorder, so focus
// survives natively; the sigil lowers to `void 0` (no-op) on React per the
...
describe('$restoreFocus emit (React) [Phase 16]', () => {
  ...
  it("lowers to `void 0` (no-op) — React's keyed reconciler preserves DOM identity natively", () => {
