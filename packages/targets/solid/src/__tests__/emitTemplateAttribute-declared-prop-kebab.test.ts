 ...
 * `aria-*`/`data-*` attributes on composed component tags (Solid target).
 *
 ...
 * that prop — the emitted key is the DECLARED camelCase name (the child's
 * `splitProps` key list is built from its `ir.props`). Before this task,
 * `colonPropToSolidName`'s aria-/data- preservation ran UNCONDITIONALLY
 * (before any tagKind check). Angular's component-tag camelization is the
 * prior art (already correct); this is the declaration-gated form of it —
 * natives and genuine passthrough attributes keep the hyphen preservation
 * exactly (Solid's "KEBAB PASSTHROUGH" file-header rule still applies there).
 *
...
describe('emitTemplateAttribute (Solid) — declared-prop kebab resolution (260812-2ur)', () => {
