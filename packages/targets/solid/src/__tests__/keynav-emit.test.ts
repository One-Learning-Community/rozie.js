/**
 * Plan 71-07 Task 2 — Solid `r-keynav` emitter wiring (red-first per seam).
 *
 ...
 * SEPARATE subtrees) fixtures the React/Vue/Svelte references (71-04/05/06)
 * used through the full `parse -> lowerToIR -> emitSolid` pipeline and
 * asserts on the emitted `.tsx` text — one `it()` per emitter seam
 * (item-marker, id, aria, tabindex, root-hook), per the plan's own
...
describe('Solid r-keynav emitter (Plan 71-07 Task 2)', () => {
  ...
  it('SEAM: id — item id is namespaced by a component-unique group id (T-71-07-02)', () => {
    const ir = compile(MENU_SRC, 'KeynavMenu.rozie');
    const { code } = emitSolid(ir, { filename: 'KeynavMenu.rozie', source: MENU_SRC });
  ...
  it('SEAM: aria — combobox emits aria-activedescendant on the input bound to the active <li> id', () => {
    ...
    const { code } = emitSolid(ir, { filename: 'KeynavCombobox.rozie', source: COMBOBOX_SRC });
  ...
  it('SEAM: root-hook — a createKeynav(...) call wires the root ref, config, source, active get/set, and commit', () => {
    const ir = compile(MENU_SRC, 'KeynavMenu.rozie');
    const { code } = emitSolid(ir, { filename: 'KeynavMenu.rozie', source: MENU_SRC });
    expect(code).toContain('createKeynav(() => __rozieKeynavRootRef, {');
    ...
    expect(code).toContain('onCommit: (i) => { run(local.items[active()]); },');
    // r-keynav-active-class — additive activeClass option.
    expect(code).toContain("activeClass: 'is-active',");
    expect(code).toMatch(/import \{[^}]*\bcreateKeynav\b[^}]*\} from '@rozie\/runtime-solid';/);
    expect(code).toContain('let __rozieKeynavRootRef: HTMLElement | null = null;');
    expect(code).toContain('ref={(el) => { __rozieKeynavRootRef = el as HTMLElement; }}');
  });
  ...
  it('SEAM: source synthesis — the menu getSource maps the synthesized r-for array to { label, disabled }', () => {
    ...
    const { code } = emitSolid(ir, { filename: 'KeynavMenu.rozie', source: MENU_SRC });
    expect(code).toContain(
      'getSource: () => (local.items).map((it) => ({ label: it.label, disabled: it.disabled })),',
    );
  });
  ...
  it('SEAM: source (explicit) — the combobox getSource maps the explicit :source array to { label }', () => {
    const ir = compile(COMBOBOX_SRC, 'KeynavCombobox.rozie');
    const { code } = emitSolid(ir, { filename: 'KeynavCombobox.rozie', source: COMBOBOX_SRC });
    expect(code).toContain('getSource: () => (local.results).map((r) => ({ label: r.label })),');
  });

  it('SEAM: no per-item keydown listeners are ever emitted (delegation lives entirely inside createKeynav)', () => {
    const menuIr = compile(MENU_SRC, 'KeynavMenu.rozie');
    const menu = emitSolid(menuIr, { filename: 'KeynavMenu.rozie', source: MENU_SRC });
  ...
  it('NO-REGRESS: a component with no r-keynav directive is byte-identical to pre-Phase-71 emit (no createKeynav/keynav attrs)', () => {
    const ir = compile(COUNTER_SRC, 'Counter.rozie');
    const { code } = emitSolid(ir, { filename: 'Counter.rozie', source: COUNTER_SRC });
    expect(code).not.toContain('createKeynav');
...
// enclosing loop" mechanic, which is not what this fixture is testing. Both
// loops' `:key` references their OWN index alias (`w`/`d`), which falls back
// to the `<For>` path (not `<Key>` — see emitLoop's `keyDependsOnIndex`
// guard), so `w`/`d` are Solid `<For>` Accessors, exactly the shape the
// explicit-index `invokeAccessors` threading is meant to prove.
...
describe('Solid r-keynav emitter — multi-root, grid, page, explicit index (Plan 77-04 Task 3)', () => {
  it('multi-root: two r-keynav roots emit TWO independent createKeynav(...) controller calls', () => {
    ...
    const callCount = (code.match(/createKeynav\(/g) ?? []).length;
  ...
  it('multi-root: each root gets its own suffixed root-ref/group-id identifiers', () => {
    ...
    const { code } = emitSolid(ir, { filename: 'KeynavTwoGroups.rozie', source: TWO_ROOT_SRC });
    expect(code).toContain('createKeynav(() => __rozieKeynavRootRef, {');
    expect(code).toContain('createKeynav(() => __rozieKeynavRootRef1, {');
    expect(code).toContain('let __rozieKeynavRootRef: HTMLElement | null = null;');
    expect(code).toContain('let __rozieKeynavRootRef1: HTMLElement | null = null;');
    expect(code).toContain('__rozieKeynavGroupId = `keynav-${Math.random()');
  ...
  it('grid: the config literal is UNCHANGED (no grid key) — the columns getter is a SIBLING gridColumns option', () => {
    ...
    const { code } = emitSolid(ir, { filename: 'KeynavGridPage.rozie', source: GRID_PAGE_SRC });
    expect(code).toContain(
  ...
  it('page: an arbitrary @keynav-page expression is wrapped in a (detail) => { ...; } arrow', () => {
    ...
    const { code } = emitSolid(ir, { filename: 'KeynavGridPage.rozie', source: src });
    expect(code).toContain('onPage: (detail) => { onBoundary(local.cells); },');
  });
