...
export default function TodoList(_props: TodoListProps): JSX.Element {
  const [local, rest] = splitProps(_props, ['items', 'title']);
  ...
  return (
    ...
    <div class={"todo-list"}>
      <header>
        {_props.headerSlot ? _props.headerSlot({ remaining, total: items().length }) : <h3>{local.title} ({remaining} remaining)</h3>}
      </header>
      ...
      {<Show when={items().length > 0} fallback={<p class={"empty"}>
        ...
        <For each={items()}>{(item) => <li class={{ done: item.done }}>
