...
export default function Modal(_props: ModalProps): JSX.Element {
  const [local, rest] = splitProps(_props, ['open', 'closeOnEscape', 'closeOnBackdrop', 'lockBodyScroll', 'title']);
  const resolved = children(() => local.children);

  const [open, setOpen] = createControllableSignal(_props, 'open', false);
  onMount(() => {
    const _cleanup = (lockScroll)();
  ...
  return (
    ...
    <>
    {<Show when={open()}><div class={"modal-backdrop"} ref={(el) => { backdropElRef = el as Element; }} onClick={(e) => { if (e.target !== e.currentTarget) return; local.closeOnBackdrop && close(); }}>
      <div ref={(el) => { dialogElRef = el as Element; }} class={"modal-dialog"} role="dialog" aria-modal="true" aria-label={local.title || undefined} tabIndex={-1}>
