...
export default function Dropdown(_props: DropdownProps): JSX.Element {
  const [local, rest] = splitProps(_props, ['open', 'closeOnOutsideClick', 'closeOnEscape']);
  ...
  createEffect(() => {
    if (!(open())) return;
    window.addEventListener('resize', _rozieThrottleLReposition, { passive: true });
    onCleanup(() => window.removeEventListener('resize', _rozieThrottleLReposition, { passive: true }));
  });
  ...
  return (
    ...
    <>
    <div class={"dropdown"}>
      <div ref={(el) => { triggerElRef = el as Element; }} onClick={toggle}>
        {_props.triggerSlot?.({ open: open(), toggle })}
      </div>

      {<Show when={open()}><div ref={(el) => { panelElRef = el as Element; }} class={"dropdown-panel"} role="menu">
        {resolved()}
      </div></Show>}</div>
    </>
    </>
