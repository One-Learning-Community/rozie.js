// Phase 13 Plan 13-01 (Wave 0 RED scaffold) — Vue `$classSelector` emit test.
//
...
/** Compile `examples/ClassSelectorProbe.rozie` to Vue SFC output. */
function compileProbe(): { code: string; diagnostics: Diagnostic[] } {
  ...
  const emitted = emitVue(ir, { filename, source, modifierRegistry: registry });
  return {
    code: emitted.code,
...
describe('$classSelector emit (Vue) [Wave 0 RED — implemented in Wave 2]', () => {
