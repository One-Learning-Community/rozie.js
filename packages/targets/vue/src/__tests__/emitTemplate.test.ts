...
//
// `part` is a standard HTML static attribute and flows through the Vue
// static-attr branch verbatim into the emitted <template>. On Vue the
...
`;
  ...
  it('emits the producer part="body" attribute verbatim into the Vue template', () => {
    const { template, diagnostics } = emitTemplate(lowerInline(PRODUCER), createDefaultRegistry());
