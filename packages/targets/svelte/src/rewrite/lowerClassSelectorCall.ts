/**
 * lowerClassSelectorCall — Phase 13 Plan 13-04 (Svelte target).
 *
 * Lowers a `$classSelector('<class>')` `CallExpression` to Svelte's
 * compile-time CSS-selector form: the string literal `".<class>"`.
 *
 * Svelte keeps authored class names LITERAL in the emitted DOM (style
 * isolation via an appended `svelte-<hash>` class — the original class name is
 * left intact), so a class passed to a third-party engine matches the rendered
 * DOM as written. The helper therefore lowers to a plain `StringLiteral` — no
