// Phase 13 Plan 13-01 (Wave 0 RED scaffold) — Svelte `$classSelector` emit test.
//
...
//
// Svelte keeps authored class names LITERAL in the emitted DOM (style isolation
// via an appended `svelte-<hash>` class), so `$classSelector('grip')` lowers to
// the compile-time string literal `".grip"`. (React is the lone exception — it
// lowers to a runtime `"." + styles.grip` expression.)
//
...
/** Compile `examples/ClassSelectorProbe.rozie` to Svelte SFC output. */
function compileProbe(): { code: string; diagnostics: Diagnostic[] } {
  ...
  const emitted = emitSvelte(ir, { filename, source, modifierRegistry: registry });
  return {
    code: emitted.code,
...
describe('$classSelector emit (Svelte) [Wave 0 RED — implemented in Wave 2]', () => {
