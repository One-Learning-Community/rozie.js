/**
 * `rozieContext` — globalThis-backed React context registry (Phase 36, R11).
 *
 * The `$provide` / `$inject` sigils lower to React Context. A provider
 * component and a (possibly deeply nested) consumer component are compiled
 * into SEPARATE modules, yet must share the SAME `React.Context` object for a
 ...
 * the same realm.
 *
 * The `__rozieCtx` namespace is shared with the Solid + Angular registries;
...
/**
 * Look up — or create-and-store — the React Context object for `key`. Two
