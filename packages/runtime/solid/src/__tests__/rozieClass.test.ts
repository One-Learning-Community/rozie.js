/**
 * Quick task 260620-kby — rozieClass normalizer tests for @rozie/runtime-solid.
 *
...
describe('rozieClass', () => {
  ...
  it('never emits a token for a non-enumerable __proto__ literal key (prototype-pollution-safe)', () => {
    // A `__proto__:` literal key sets the prototype, not an own-enumerable key,
    // so Object.keys never sees it and no token is emitted.
    const obj = { active: true, __proto__: { polluted: true } } as Record<string, unknown>;
    expect(rozieClass(obj)).toBe('active');
    // Explicit own-enumerable '__proto__' (via defineProperty) WOULD be a key,
    // but a plain object literal cannot produce one — confirm the literal form.
  ...
  it('is pure/stateless — re-evaluation with different inputs yields the current string each call', () => {
    // Proves a reactive re-eval at the binding site always reflects current state.
    let cond = false;
