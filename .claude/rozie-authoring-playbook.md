...
| `$memo(fn, keyFn)` | call | memoized plain function, uniform ×6 (reference-keyed) | top-level `const` + EXACTLY 2 arrow args or ROZ146; `keyFn` must read every reactive input `fn` depends on, UNCONDITIONALLY, before the cache-hit check (subscribe-first — the fine-grained Solid/Svelte/Vue subscription surface); call the result with `()` like a plain function |
...
10. **`$memo(fn, keyFn)`** for an O(N)-avoidance re-map derived from engine data (windowed rows, filtered/grouped option lists) — the uniform ×6 answer to the hand-rolled reference-keyed cache-object pattern (`const fooCache = {…}; const foo = () => { … }`) that recurs across engine wrappers. Combobox's `filteredOptions()` is the canonical dogfood: `keyFn` reads every reactive input (`$props.options`, `$data.query`, …) unconditionally and returns them as an array; `fn` is the MISS-path computation. See docs/guide/features.md `## $memo(fn, keyFn)`.

**Collision classes catalogued so far** (name a thing two ways that collapse on some target): prop==slot name (ROZ127, Svelte), expose==event name (ROZ121), event⇄verb (focus/blur, TipTap), model-prop==emit-name (zoom/pitch dropped, MapLibre [[project_next_port_maplibre]]), import-name==component-name (alias the engine import, Cropper), expose-verb==inherited-DOM-method on Lit (`scrollTo`→`scrollToIndex`, Embla), local-const==inherited-DOM-property on Lit (`const nodeType`→`injectedType` shadows `Node.nodeType` TS2416, rete `Port`), **reserved-word prop name** (`<Port in=>` → Svelte `$props()` destructure `let { in } = $props()` is illegal; Dan renamed the attr `in`/`out`→`input`/`output` over a Svelte emitter fix — rete Phase 41), r-for-loop-var==slot-name on Svelte (loop var shadows the slot snippet → `{@render name()}` renders a non-function, Svelte-only runtime crash; Embla `slide`, slider `mark`, toast `toast`) — **now AUTO-FIXED on Svelte** (683b2a24): the svelte emitter renames the colliding snippet binding `<name>`→`<name>$$slot` (binding + `{@render}`/`{#if}` sites; the author's loop var + slot-arg values untouched), conditional so non-colliding components stay byte-identical. Detection = `findRForSlotNameCollisions(ir)` (core). The earlier ROZ980 warning was superseded + removed. (The `#body` slot-param shadow variant — rete `node` — is NOT auto-fixed; still rename manually.) **local-name==ref-name self-shadow** (`const X = $refs.X` → `const X = X(.current)` TDZ on React+Svelte — now AUTO-FIXED by the deconflict pre-pass renaming the local `X$local`; the `$refs` analog of the `$props` self-shadow; refs-lowering-cross-target). **`$data`-key==`$expose`-verb** (`$data.open` + `$expose({ open })` → both collapse onto React's `useState` `open` ident; rename the data key → `expanded`, listbox), **local-helper-name==`Object.prototype`-member** (`const valueOf = …` becomes a class field on Lit/Angular → breaks the legacy `@property` decorator's `Object`-assignability → cascades **TS1240/TS1271 to EVERY decorator** on the Lit class, 38 errors from one name; rename `valueOf`→`readValue` — also watch `toString`/`hasOwnProperty`, listbox), **bare boolean attr on a CHILD COMPONENT** (`<Child combobox />` emits `combobox=""` on all 6 but only Vue+Lit coerce ""/attr-presence→`true`; React/Svelte/Angular/Solid pass falsy `""` → use `:combobox="true"`, listbox), **local-helper/const/PROP-name==inherited-DOM-*property* on Lit** (a top-level `const inputMode` / a prop named `id`/`title`/`lang`/`hidden`/`tabIndex` becomes a Lit class field colliding with the inherited `HTMLElement.inputMode`/`.id`/etc. → hard TS2416/TS1238/TS1240, NOT warn-only — the *property* sibling of the warn-only ROZ137 *method* override; rename `inputMode`→`cellInputMode`, prop `id`→`idBase`; otp+combobox, 2026-06), **local-helper-name==Angular-CVA-method** (a PLAIN — non-exposed — helper named `writeValue`/`registerOnChange`/`registerOnTouched`/`setDisabledState` in a single-`model:true` component collides with the generated ControlValueAccessor → `TS2300` duplicate identifier at the angular `ng-packagr` build; ROZ124 only catches the `$expose`-verb form, a plain helper slips past it; rename `writeValue`→`commitValue`; otp, 2026-06). These two only bite at the per-leaf typecheck/`ng-packagr` build, NOT at `compile()×6` — run gates d (typecheck ×4) + e (build ×6), the surface gate alone misses them ([[project_pure_rozie_collision_classes_2026_06]]). When adding a port, watch for a new one.

**EMITTER GAP — no `dialog` (and other unmapped elements) in the per-target ref-type map:** a `ref=` on a native `<dialog>` types to generic `HTMLElement` on tsdown/vue leaves (no `.open`/`.showModal()`/`.close()` → TS2339/2551/2322). Source-only workaround (keeps the additive scope fence): ref the inner `<div>` and reach the dialog via `panelEl.parentElement as HTMLDialogElement` (dialog family, 2026-06). Proper fix is non-additive (add a `dialog→HTMLDialogElement` case to all 6 ref-type maps + snapshot/dist-parity rebless). See [[project_pure_rozie_collision_classes_2026_06]].
...
- **RENDER PURITY: NEVER write `$data`/`$model` from a template expression** (codemirror UAT 2026-08-10 — collision-class, 4/6 targets reject it): a side-effect binding like `:data-cap="captureFn(scope)"` runs DURING render, and a `$data.x = …` inside it is a render-time reactive mutation — Svelte 5 throws `state_unsafe_mutation`, Angular throws NG0600, Solid re-enters render to stack overflow (a `$data.x = $data.x + 1` read-write subscribes the effect to its own write), Vue intermittently re-enters engine dispatch. In an engine-wrapper portal fill the throw can escape the engine's setup (CM6 `EditorState.create`) and kill the WHOLE editor mount. TipTap's `captureEditor` idiom is safe ONLY because it writes a non-reactive `window` global. **FIX pattern:** capture into a plain module object + flush into `$data` via `setTimeout(0)` (macrotask — outside every framework's flush; microtask is NOT enough on Angular), and gate the capture on an actual value change (Angular re-evaluates template expressions every CD pass — an unconditional counter bump re-triggers CD forever at ~2/s). See CodeMirrorDemo's `capBuf`/`queueCapFlush`.
- **DRIVING-PROP IDENTITY: computed-per-render array/object props re-trigger the component's prop `$watch` every render** (codemirror UAT 2026-08-10, Vue bite): `:gutterLines="fn()"` returning a fresh `[1,3]` each render makes the identity-based watcher queue a reconfigure per parent re-render; on Vue those PRE-flush watchers get flushed synchronously by a portal fill's nested `render()` — INSIDE the engine's own update → "Calls to EditorView.update are not allowed while an update is in progress". Author driving props as `$computed` (identity-stable until deps change). Component-side: engine-wrapper `$watch` reconfigures should go through a microtask-deferred batcher (CodeMirror's `scheduleReconfigure`) because a consumer changing TWO reconfigure props in one tick reproduces the same re-entrancy legitimately.
- **CROSS-TREE LEAF-SRC IMPORTS split engine module identity on Angular** (codemirror UAT round 2, 2026-08-10): a demo/consumer file importing a LEAF's src module by relative path (`../../packages/ui/codemirror/packages/react/src/languages`) resolves that module's `@codemirror/*` imports through the LEAF's own node_modules symlink context — the Angular sub-build gives those a second module identity, so the page carries TWO `@codemirror/state` copies and CM6 rejects the extension with "Unrecognized extension value … multiple instances of @codemirror/state" (instanceof breaks). The other 5 targets realpath-dedupe. Use a BARE specifier (declared in the consumer's package.json) so resolution shares the component's path; keep leaf-module coverage in family unit tests (node resolution realpaths).
...
- **React** (the fragile one — re-runs the body every render):
  ...
  - **module-`let` referenced ONLY inside an `$expose` verb is NOT hoisted to `useRef`** (toast, 2026-06): `hoistModuleLet` persists a reassigned top-level `let` only when it escapes into a hook context it scans (useEffect from `$onMount`/`$onUnmount`, handlers) — a `useImperativeHandle` (`$expose`) closure is NOT scanned, so a counter like `let nextId` mutated only in `show()` resets to 0 every render → duplicate ids. `timers` in the same component survived only because it ALSO escapes into `$onUnmount`'s effect. WORKAROUND (source): keep such a counter in reactive `$data` (real `useState`, persists; the refreshed-each-render `$expose` handle reads current state). Proper fix = teach hoistModuleLet to scan `$expose`-verb bodies (emitter, needs rebless). A VR mount-and-drive cell caught it; compile/typecheck/build did not.
  - **STALE re-read of just-written `$data.x` WITHIN one handler** (listbox): `$data.q = e.target.value; emit('search', $data.q)` reads the PRE-write `q` on React (setState is async; the closure binds the old value) — the other 5 assign reactively/synchronously. FIX: capture the fresh local (`const q = e.target.value; … emit('search', q)`). Hits any "write then read the same `$data` key" in a single function.
- **`$computed` access form is TARGET-SPLIT** (listbox): read it BARE (`visibleOptions`, never `visibleOptions()`) AND never alias it (`const o = comp` → on Solid `o` is the memo ACCESSOR fn, `.length`/`.findIndex`/`[i]` → TS2339; on React it's the value). The Solid emitter rewrites bare reads to `comp()`, but NOT an assignment RHS. So: keep `$computed` for template-bare + simple-expression reads only; for a derived value used+aliased in script logic, make it a PLAIN FUNCTION called `()` everywhere (uniform on all 6). **Public-facing writeup (ITEM-6, Phase 46-04): `docs/guide/features.md` → the `## $computed(() => ...)` section** (value-vs-accessor table, bare-read idiom, plain-function workaround) — this §8 note is the source of that port. Also a `setX` helper collides with React's generated `setX` setter for `$data.x`/model:`x` → ROZ524 (rename, e.g. `setExpanded`→`applyExpanded`).
- **aria-* bound to a boolean (JSX targets)** (listbox): `:aria-selected="isSelected(opt)"` (a CALL → not provably-primitive) wraps in `rozieAttr`→`string`→ React/Solid JSX want `Booleanish` (TS2322). FIX: `!!` prefix (`:aria-selected="!!isSelected(opt)"`) → emitter emits unwrapped `aria-selected={!!…}` (boolean). A provably-boolean prop (`$props.x`) is already unwrapped. Also `ul`/`li` template refs type as generic `HTMLElement` (emitter tag-map lacks them) → React ref TS2322 vs `HTMLUListElement`; use `div role="listbox"/"option"` (standard headless practice).
