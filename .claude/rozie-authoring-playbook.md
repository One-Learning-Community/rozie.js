...
| `$snapshot(x)` | call | non-reactive deep copy (Svelte) / identity (others) | only for libs that `Object.defineProperty` the value (Chart.js). Narrow use — see §6 |
...
**Collision classes catalogued so far** (name a thing two ways that collapse on some target): prop==slot name (ROZ127, Svelte), expose==event name (ROZ121), event⇄verb (focus/blur, TipTap), model-prop==emit-name (zoom/pitch dropped, MapLibre [[project_next_port_maplibre]]). When adding a port, watch for a new one.
...
- **Angular**: `$onMount` → `ngAfterViewInit` + `inject(DestroyRef)` ([[project_rozie_angular_onmount_emit_bug]]). NgClass import was latent-missing in 13 files (fixed). `::ng-deep` for `:deep`.
- **Svelte 5**: snippets+props share `$props()` (→ ROZ127). `$state.snapshot` for `$snapshot`. a11y warnings on demo event patterns are status-quo ([[project_example_a11y_warnings_status_quo]]).
...
- Shipped engine-wrapper ports: `packages/ui/{sortable-list,flatpickr,chartjs,codemirror,tiptap,fullcalendar,maplibre}/src/*.rozie` (+ per-framework pre-compiled `packages/<fw>/` leaves, colocated helpers, `codegen.mjs` doc-automation — [[project_rozie_ui_distribution_model]]).
