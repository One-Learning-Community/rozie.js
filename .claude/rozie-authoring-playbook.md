...
- **`structuredClone()` THROWS on a Vue `reactive()` / Svelte `$state` proxy** (rete undo, Phase 44 — new collision-class). A bare `structuredClone(someReactiveObj)` silently fails ("could not be cloned") on Vue + Svelte ONLY, leaving snapshot/history empty on those two targets while React/Solid/Lit work — a brutal target-asymmetric trap. **Deep-clone any reactive object JSON-first** (`JSON.parse(JSON.stringify(x))` — also strips functions) with `structuredClone` only as a fallback for non-reactive inputs. Mirror FlowCanvas's `cloneGraph` helper for any graph/state snapshotting.
