...
| `$clone(x)` | call | independent DEEP clone, safe on a reactive object on all 6 | lowers `structuredClone(toRaw(x))` (Vue) / `$state.snapshot(x)` (Svelte) / `structuredClone(x)` (R/Solid/Ng/Lit); preserves Date/Map/Set (vs lossy JSON); `$clone(null)`→`null`; throws on fns/DOM (author error). Use for undo/history/scratch snapshots. Distinct from `$snapshot` (unwrap-only). ROZ135 warns on bare `structuredClone(<reactive>)` |
