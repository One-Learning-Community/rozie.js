...
- **Compiler-driven diagnostics via the native platform LSP API** (Phase 85, superseding LSP4IJ) — the shared `@rozie/language-server` (`packages/language-server/`, rebuilt on Volar) serves both TypeScript type intelligence and `@rozie/core` ROZ diagnostics over LSP, including at a caret **inside an injected fragment** — the one thing the retired LSP4IJ client could never do (it has zero concept of language injection anywhere in its codebase). The native `platform.lsp.serverSupportProvider` extension point unwraps injections centrally and is the same one JetBrains' own Vue plugin uses. The pre-existing PSI-based completion/reference/navigation contributors (Phase 08.2, ~4,821 lines) keep running alongside the LSP's own answers — not because the LSP cannot reach injected carets (it now can), but because D3 (Phase 85 CONTEXT.md) deliberately defers retiring that semantic Kotlin layer until the LSP is trusted in daily use.
