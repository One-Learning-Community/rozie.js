...
/**
 * RESEARCH Open Question 4 resolved Option A — paint `$props`/`$data`/`$refs`/
 * `$emit`/`$el`/`$slots`/`$onMount`/`$onUnmount`/`$onUpdate`/`$computed`/
 * `$watch` with `ROZIE_MAGIC_IDENT` inside any `.rozie`-hosted injected JS.
 * Restores the visual cue that retired from the JFlex side (Plan 01 removed
 ...
 * `referenceName` is one of the 11 Rozie magic identifiers — a top-level
...
class RozieJsAnnotator : Annotator {
    ...
    override fun annotate(element: PsiElement, holder: AnnotationHolder) {
        ...
        if (name !in MAGIC_IDENTS) return
    ...
    companion object {
        /**
         * The 11 Rozie magic identifiers (matches the retired JFlex MAGIC_IDENT_RE
         * macro from Plan 01). Stored as a Set for O(1) membership lookup on each
         * candidate JSReferenceExpression.
         */
        private val MAGIC_IDENTS: Set<String> = setOf(
            "\$props",
            "\$data",
            "\$refs",
            "\$slots",
